 ...
 *
 * **Que hace ese codigo de salida** (tarea 3.2). Tres cosas, y las tres
 * hacen falta porque cada una falla de una forma distinta:
 *
 *   · `projection_divergence_total` sube y la regla `absent()` sobre el sello de
 *     tiempo delata que la pasada dejo de ejecutarse.
 *   · Las jornadas que la pasada encontro y **no pudo corregir** salen en
 *     `projection_reconciliation_last_failures`, y `ReconciliacionConFallos`
 *     (`infra/observability/prometheus/rules/projection.yml`) la evalua con
 *     `> 0` hacia el IT del cliente, con `divergencia-proyeccion.md`.
 *   · El propio codigo de salida deja `scheduler.command_failed` en el log, con
 *     `command` y `exit_code`, desde el `->onFailure()` de
 *     `routes/console.php`. Hace falta porque la tarea corre con
 *     `runInBackground()` y en ese camino un codigo distinto de cero **no**
 *     produce excepcion ni fila en `error_events`: sin esa linea, la unica
 *     constancia seria la metrica, que no existe si el comando ni siquiera
 *     llego a arrancar.
 *
