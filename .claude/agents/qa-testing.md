...
- **En Feature, Integration y Contract el reloj se detiene con `FrozenTime::at('…')` (`tests/Support/Time`), nunca con `app()->instance(Clock::class, FixedClock::at(…))`.** Esas suites tienen dos relojes —el puerto `Clock` del dominio y Carbon, que leen Sanctum, Eloquent y el limitador— y detener solo el primero hace que el resultado dependa del día en que se ejecuta (el 13-09-2026 `main` se puso en rojo por un token de quiosco emitido bajo un `FixedClock` de junio y caducado para el reloj real). `FrozenTimeTest` (Architecture) lo exige. En Unit no hay framework: allí `FixedClock` inyectado a mano sigue siendo lo correcto.
