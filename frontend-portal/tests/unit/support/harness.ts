...
//
// Monta la vista con lo mismo que le da la aplicacion real -Pinia, router e
// i18n- y sustituye `fetch` por un doble explicito. Las pruebas describen
// respuestas de la API, no interioridades del cliente: si mañana cambia como
// se envia una peticion, siguen valiendo.
...
export async function mountView<Props extends Record<string, unknown>>(
  ...
  // dispara `app.use(router)` o un `push`. Sin este empujon la espera no
  // termina nunca. `/login` no pide datos a nadie.
  ...
  const mountingOptions = {
    props: options.props,
    global: {
      plugins: [pinia, router, createAppI18n(options.locale ?? 'es')],
    },
...
/** Espera a que se resuelvan las promesas pendientes y se repinte el DOM. */
