...
- **Alcance de la decisión: la vía de la tarjeta.** El fichaje por PIN (RF-AT-11) no se puede validar en local —el PIN viaja sellado y solo el servidor lo verifica—, así que una confirmación inmediata sería una afirmación que el quiosco no puede hacer. Desde el 28 de agosto de 2026 el PIN muestra un estado propio, «Comprobando…», mientras hay red y hasta un plazo corto (`PIN_VERIFY_TIMEOUT_MS`); si el servidor no contesta a tiempo o no hay red, cae a «pendiente de validar» y el sobre sigue encolado. La regla dura 19 no cambia: nada se bloquea ni se pierde; solo se evita enseñar un éxito provisional que un rechazo puede desmentir un segundo después.
