...
## §4 STRIDE × las catorce filas del doc 01 §8.1
...
| 14 | Divulgación | Enlace de descarga de un informe en diferido filtrado —historial de un navegador compartido, un reenvío, un registro de proxy— o adivinado, que entrega un fichero con las horas de la plantilla **sin pasar por ninguna sesión** (fila añadida el 22-09-2026 en la tarea 3.9) | El enlace no existe hasta que alguien consulta el estado desde el panel con su sesión, y cada consulta rota el anterior; 32 bytes de `random_bytes` de los que en la fila solo vive su `sha256`, comparado con `hash_equals`; se consume en la primera descarga (`FOR UPDATE` sobre la fila, así que un doble clic no entrega dos veces) y caduca a los `REPORTING_EXPORT_LINK_TTL_MINUTES` (15 de serie). El fichero se purga a los `REPORTING_EXPORT_RETENTION_DAYS` y al purgarlo la fila pierde `employee_uuid` y el alcance. Zona propia por IP `throttle:report-download`. Cada descarga deja asiento `report_export.downloaded` **antes** de entregar el fichero (RS-05, RL-15). El correo nunca lleva el enlace ([ADR-041](../adr/ADR-041-descarga-de-ficheros-diferidos-con-enlace-de-un-solo-uso.md)). | **Cubierta** (mitigación) · **parcial** (detección) | **Sin detección a propósito**: un enlace consumido por quien no debía es indistinguible en el borde de uno consumido por su dueño. Queda el asiento de la descarga, que permite reconstruirlo después, y el `410 …link-used` que ve el legítimo cuando alguien se le adelantó. Sin hallazgo. |
