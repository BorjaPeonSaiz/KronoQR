...
absences_metrics_day_seconds                             gauge
...
compliance_metrics_week_start_seconds                    gauge
...
**`pattern_detection_last_run_timestamp_seconds` y `pattern_detection_last_failures` son las mismas dos hermanas, para `attendance:detect-patterns`** (tarea 3.11, RF-PR-06/RN-16). El comando corre a diario a las 04:35 UTC —entre `attendance:detect-incidents` (04:30) y `reporting:compliance-metrics` (04:45)— y publica las dos series por *textfile* (`kronoqr_pattern_detection.prom`), también cuando no hay hallazgos ni centro que revisar: sin eso, «no hay ningún patrón anómalo» y «nadie lo ha buscado todavía» se leerían igual. Las alertas que las sostienen —`DeteccionDePatronesConFallos` y `DeteccionDePatronesAusente`, `infra/observability/prometheus/rules/incidents.yml`, mismo umbral de 26 h y mismo destinatario que sus hermanas de incidencias— están dirigidas al IT del cliente, no a RRHH: lo que vigilan es que el planificador siga corriendo, no el contenido del indicio. **Los hallazgos en sí (`anomalous_patterns_detected_total{pattern}`) no llevan alerta, a propósito**: un indicio de posible préstamo de credencial señala a dos personas concretas y se revisa en la bandeja de incidencias por quien conoce el turno, no se enruta a un canal de guardia como si fuera una avería (§8.4 no admite alerta sin runbook que la sostenga, y aquí el procedimiento es `docs/runbooks/patron-anomalo-credencial.md`, pero el runbook es para revisar el indicio, no para que suene un teléfono a las 06:30).

**`compliance_findings_last_week{rule}` y `compliance_employees_affected_last_week` son *gauges* de fichero `.prom`, publicados cada noche por `reporting:compliance-metrics`** (tarea 3.4, paso 7 y decisión 10 de su ficha), después de `attendance:detect-incidents`, sobre la **última semana completa del perfil de cumplimiento** y con la **misma consulta** que la pantalla «Cumplimiento» del panel de administración, para que el cuadro de Negocio (§8.3) y esa pantalla no puedan discrepar. `rule` toma los cuatro valores de `ComplianceFinding` (`insufficient_rest`, `daily_excess`, `missing_break`, `weekly_excess`); `missing_break` sale a cero mientras RN-12 siga suspendida (`ComplianceRuleSuspension`, hasta la tarea 3.5). Es un indicador de RRHH, no un fallo de operación: **no lleva alerta de Prometheus** (§8.4, «una alerta sin runbook es ruido»). `compliance_metrics_week_start_seconds` dice **de qué semana hablan las otras dos** —el primer día de la semana medida, en segundos—: sin ella, unas cifras congeladas porque la tarea programada dejó de ejecutarse se leen en el cuadro exactamente igual que una semana tranquila, y estos *gauges* se reescriben idénticos los siete días siguientes. Mismo papel que `incidents_metrics_timestamp_seconds`, con la diferencia de que aquí lo que importa no es cuándo se calculó sino qué periodo describe.
...
| **Impacto y adopción** | Dirección y el propio fabricante en la venta | Jornadas con registro completo, reparto de fichajes por origen, ratio de correcciones, tiempo hasta resolver incidencias, credenciales pendientes. Es el cuadro que responde a *"¿esto está sirviendo para algo?"* (RF-IN-08). **Desde la tarea 3.13 el panel tiene su propio cuadro de impacto** (`GET /api/v1/reports/adoption`, roles `admin` y `rrhh`), con **las mismas definiciones** que las series del §8.2 —jornada completa, fichajes aceptados por origen, correcciones sobre fichajes, resolución por tipo— pero **distinta ventana**: Grafana mira los últimos 7 días de contadores; el panel, un periodo civil cerrado comparado con el mismo número de días anterior. Por eso pueden no coincidir al céntimo sin que haya error. **El cuadro del panel es el que sostiene la renovación de la licencia**: es exportable con sello y hash, su exportación se audita y añade lo que Grafana no mide (disponibilidad del acto de fichar de RNF-D-01 y la línea base declarada de horas en hojas). **Cierre de la Fase 3:** el panel «Ratio de correcciones manuales / fichajes» de `impacto-adopcion.json` dividía por `scans_total` sin filtrar (todo intento, incluidos los rechazados) en vez de por los aceptados —la misma definición que `AdoptionIndicators`—, una fracción distinta de la que explica solo la ventana; corregido a `scans_total{result!~"rejected_.*"}` |
...
| SBOM | CycloneDX vía `trivy fs --format cyclonedx` (`make sbom`), `sbom/kronoqr-<versión>.cdx.json` | No es una puerta de calidad: es un artefacto que se sube en cada ejecución del job `security` y, al publicar una versión, el job `paquete` de `release.yml` lo genera de nuevo sobre el commit de la etiqueta y `publicar` lo adjunta a la *release* junto con el paquete de entrega y sus sumas SHA-256 |
