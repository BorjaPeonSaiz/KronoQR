...
11. **La navegación del panel es una columna lateral fija** (10-09-2026): `<header>` de primer nivel —sigue siendo el landmark `banner`— de 16 rem a la izquierda, sobre `kq-surface-raised` con `kq-border` a la derecha, pegajosa a toda la altura; dentro, la marca arriba, las secciones en vertical (`bg-kq-primary-strong text-kq-on-primary` en la activa, con `aria-current="page"`; `hover:bg-kq-surface-alt` solo en el resto, y un hueco de al menos 6 px entre secciones para que el anillo de foco —del mismo tono que la activa— nunca caiga sobre la vecina) y quién ha entrado con «Cerrar sesión» abajo, separados por `kq-border`. Por debajo de `md` se apila sobre el contenido y las secciones vuelven a una fila que envuelve: **sin menú plegable** —lo que la sesión alcanza está siempre en el DOM y visible—. Sin tokens nuevos ni parejas nuevas: todas las combinaciones son las medidas en §2.

## 7. Relación con la marca blanca (tarea 5.8, RF-PD-08)

Lo diseñado aquí es **el valor por defecto**; la tarea 5.8 (07-09-2026) lo hizo configurable **sin tocar `theme.css`**. Cómo funciona, en `packages/web-kit/src/branding.ts`, que las tres SPA consumen igual:

- Las tres aplicaciones reciben la marca de `GET /api/v1/branding` (público: el quiosco y el portal la necesitan antes de identificar a nadie) y la aplican **en tiempo de ejecución** con `applyBranding`, que escribe *custom properties* `--kq-*` en `:root`. Como las utilidades son `@theme inline`, la sobreescritura llega a todas las clases `*-kq-*` sin más. No se recompila nada por cliente (ADR-017).
- **El cliente elige un solo color de acento.** Mientras no elige ninguno, `accent_color` es `null` y no se toca ningún token: lo que se ve es exactamente este documento. Con un color, `accentOverrides` deriva **solo la familia primaria** de cada contexto:
  - Claro (panel y portal): `primary` y `primary-strong` = el acento; `on-primary` = blanco o `text`, el que más contraste; `primary-soft` = 10 % de acento sobre `surface-raised`; `on-primary-soft` = el acento oscurecido hasta 4,5:1 sobre `primary-soft`; `focus` = el acento oscurecido hasta 3:1 sobre `surface-raised` (el anillo de foco no es marca, es saber dónde está el teclado, WCAG 2.4.11: con un acento casi blanco se oscurece solo).
  - Quiosco (oscuro): igual que hace esta paleta con el naranja (§1.2, más claro en la tablet que en el panel), el acento **se aclara**: `kiosk-primary` hasta 3:1 y `kiosk-primary-strong` hasta 4,5:1 sobre `kiosk-surface-raised`; `kiosk-on-primary` = `kiosk-surface` o `kiosk-text`, el que más contraste.
- **Contraste avisado, no impuesto** (paso 9 de 5.8): `contrastWarnings` pasa el acento por las parejas de `themePairs.ts` en las que interviene un token derivado y devuelve las que no alcanzan su mínimo; la pantalla «Marca» del panel las enseña antes de guardar. Los tonos derivados llegan siempre por construcción; lo que puede fallar es **el propio acento como primer plano** sobre las superficies del producto (un amarillo pálido como enlace o como anillo de foco). `branding.spec.ts` lo verifica con siete acentos, del blanco al granate, contra los valores reales de `theme.css`.
- **Qué no es marca y no se configura**: superficies, texto y bordes; los cinco colores de confirmación del quiosco (§1.2); los tokens de accesibilidad del quiosco (tamaños mínimos y objetivo táctil); la salvia decorativa; los colores de estado `success`/`warning`/`danger` en su papel semántico; y los identificadores técnicos (`FH1`, nombres de tabla, rutas, y el `name` del manifiesto PWA, que es de compilación).
- El logotipo y el nombre de la aplicación no son parte de este documento: son la clave `BRANDING_APP_NAME` y el fichero de `BRANDING_LOGO_PATH` (validado al guardar: PNG o SVG por contenido, ≤ 512 KiB, ≤ 2048 px, dentro del directorio de marca del contenedor). `applyBranding` pone además el nombre en el título de la pestaña y el logotipo como favicon. Ningún componente lleva el nombre del producto escrito a mano: `PRODUCT_BRANDING` de `web-kit` es el único sitio del frontend donde vive el valor por defecto, y coincide con el catálogo de `installation_settings`.
- **El papel sigue la misma regla.** El color de acento de serie del catálogo es `primary-strong` (`#b8542a`), así que la tarjeta de credencial y el informe sellado llevan de serie el filete del producto, y el del cliente cuando lo configura. La marca **no desplaza ni encoge el QR** de la tarjeta (`CredentialCardLayoutTest`).
