...
| You grant **support access** (panel → "Support", or `support:grant`) | You set the reason, the scope and the duration; you can revoke it at any time; you see every use in your audit log | **Processor for that specific case** (RL-18), with the same obligations; the access expires on its own and cannot extend itself |
