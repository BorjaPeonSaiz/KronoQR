...
- Catálogo: `docs/requisitos.yaml`, **139 requisitos**.
- Fase en curso (`CURRENT_PHASE`): **0**. Orden real de ejecución: 0 -> 1 -> 2 -> 5 -> 3 -> 4.
- Pruebas etiquetadas: **40** (Pest 40, Playwright 0).
...
| `RQ-13` | 0 | `html/tests/Architecture/QualityGatesTest.php:174` — bloquea la integracion si un requisito implementado no tiene prueba | Trazabilidad requisito ↔ prueba, verificada por la CI. Cada prueba declara qué requisitos cubre mediante una etiqueta (RF-, RN-, RL-, RS-) |
...
- `RNF-D-01`: 2 prueba(s) la citan y no figura en `docs/requisitos.yaml`.
- `RQ-07`: 1 prueba(s) la citan y no figura en `docs/requisitos.yaml`.
