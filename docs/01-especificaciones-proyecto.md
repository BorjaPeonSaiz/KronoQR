...
| RF-QR-05 | El QR se genera con **corrección de errores nivel Q** y tamaño mínimo garantizado. | M |
...
> **Sobre el nivel Q de RF-QR-05: qué es y qué no promete el producto** (decisión del usuario, 18-09-2026). El nivel Q es una **propiedad del símbolo** —la proporción de redundancia de Reed-Solomon que lleva dentro—, no una garantía de cuánto deterioro aguanta una tarjeta. Lo que el decodificador recupera de verdad depende de dónde caiga el daño, y sobre todo del **soporte físico**, que elige el cliente: el producto no controla si la tarjeta se imprime en papel, en cartulina o en PVC plastificado. Por eso **el producto no promete ninguna tolerancia al desgaste, ni aquí ni en la oferta comercial**: la respuesta a una tarjeta que ya no se lee es **reponerla** —revocar, reemitir e imprimir en el día ([`runbooks/tarjeta-perdida-o-rota.md`](runbooks/tarjeta-perdida-o-rota.md))— y, mientras llega la nueva, esa persona ficha con su código y su **PIN de respaldo** (RF-AT-11), de modo que no hay ninguna jornada sin registro. **Las cifras de desgaste y de oclusión no viven en este documento**: son la evidencia de dos pruebas E2E con cámara simulada y están en [`frontend-kiosk/e2e/fixtures/README.md`](../frontend-kiosk/e2e/fixtures/README.md), junto al generador que produce los vídeos. Ese es su sitio: describen lo que la suite ejercita como **regresión** —que el lector siga decodificando un símbolo dañado y que deje de hacerlo cuando el daño es masivo—, no un compromiso con el cliente.
...
>
> **Definición operativa de «coincidencia» (tarea 3.11).** Los tres patrones que enumera RF-PR-06 son **dos hallazgos**: los dos primeros son el mismo —«dos fichajes consecutivos separados por segundos» es la unidad de medida y «coincidencias sistemáticas» el umbral— y el tercero es RN-16. Una *coincidencia* es un par de escaneos **aceptados en el mismo quiosco** de **dos personas distintas** separados por menos de `ATTENDANCE_PATTERN_WINDOW_SECONDS` (10 s de serie; límite estricto: 9 s cuenta, 10 s no; `0` desactiva el hallazgo). Se cuenta **como mucho una por par de personas y día civil del centro**: la cola del quiosco al cambio de turno produce pares de segundos todos los días entre compañeros que llegan juntos, y dos coincidencias la misma mañana no dicen más que una. Hay hallazgo cuando el par acumula **al menos `ATTENDANCE_PATTERN_MIN_REPEATS` días** con coincidencia (3 de serie) dentro de la ventana revisada (`COMPLIANCE_PATTERN_LOOKBACK_DAYS`, 30 de serie): «sistemático» es «varios días», que es lo que dice el escenario del §11. Se abre **una incidencia por persona y patrón, no por pareja**: si una persona alcanza el mínimo con varias contrapartes, recibe una sola, que lleva la contraparte principal (`counterpart_employee_uuid`: la de más días; en empate, la de la primera coincidencia), cuántas personas distintas (`counterpart_count`), los días de la principal (`coincidence_days`) y **cuatro escalares sobre todas las coincidencias** en lugar de una lista de momentos —`first_coincidence_at`, `last_coincidence_at`, `last_gap_seconds` y `min_gap_seconds`; el hueco mínimo es el dato que sostiene el indicio—; `work_date` es el último día con coincidencia de la persona. Cada persona del grupo tiene la suya, asignada al responsable de su departamento, porque el indicio es simétrico y pueden ser de departamentos distintos; el grupo se reconstruye leyendo las de sus miembros. **La detección se de-duplica contra la bandeja**: no emite `kiosk_coincidence` para una persona con una incidencia de ese patrón **abierta**, y tras una resolución solo cuenta los días de coincidencia **posteriores** a ella —hacen falta `ATTENDANCE_PATTERN_MIN_REPEATS` días nuevos para que vuelva a abrirse—; una colisión entre dos hallazgos distintos de la misma persona y fecha no se descarta en silencio: cuenta como fallo de la pasada y deja log. Entran los escaneos de quiosco —tarjeta y PIN de respaldo—, aceptados o rechazados por el anti-rebote (`rejected_debounce`: el anti-rebote de RF-AT-06 es por persona y no por quiosco, y sin él RN-16 quedaría ciega entre 0 y `ATTENDANCE_DEBOUNCE_SECONDS`); los rechazos por credencial desconocida, revocada o con mala firma no son un uso de credencial, y los manuales y los importados no pasan por ningún quiosco. **RN-16 excluye solo por desfase de reloj**: un escaneo cuyo desfase supera la tolerancia de RF-AT-10 —el mismo criterio de `ReviewPolicy`, no la marca `flagged_for_review`, que es verdadera para todo fichaje por PIN— se quita de la lista **antes** de formar pares, de modo que sus vecinos limpios sí se comparan entre sí; sobre una hora que ya está en duda no se sostiene una imposibilidad. Ningún hallazgo mira ni toca `shift_entries`: nada cambia de estado, nada se marca, nada se anula. El procedimiento de revisión es el runbook `patron-anomalo-credencial.md`.
...
| RN-05 | La **jornada laboral** (`work_date`) es la fecha civil, en la zona del centro, del `clocked_in_at` del tramo **que abre la jornada**. Un turno 22:00→06:00 pertenece íntegramente al día de inicio. Los tramos que **continúan** una jornada abierta —la vuelta de una pausa (RF-AT-12)— **heredan su `work_date`** y no abren jornada nueva, aunque empiecen en otro día natural. **La continuación tiene una cota**: la vuelta solo continúa la jornada si desde el inicio de la pausa han pasado **menos horas que el descanso mínimo entre jornadas del perfil** (`min_rest_hours`, 12 h en `ES-hosteleria`); con ese tiempo o más, el escaneo abre jornada nueva. Sin la cota, quien pulsa «Pausa» y se va a casa vería su entrada del día siguiente pegada a la jornada anterior. |
...
| RN-10 | **Descanso entre jornadas**: se alerta si entre el fin de un turno y el inicio del siguiente median menos horas que el **descanso mínimo** del perfil de cumplimiento del centro (`min_rest_hours`; **12 h** en el perfil `ES-hosteleria` que se entrega de serie, art. 34.3 ET). El límite es abierto: con 12 h configuradas, 11 h 59 min alerta y 12 h exactas no. |
| RN-11 | **Jornada diaria ordinaria**: se alerta si la suma de los tramos de una jornada supera la **jornada diaria ordinaria** del perfil de cumplimiento del centro (`max_daily_hours`; **9 h** en `ES-hosteleria`). El límite es abierto: 9 h exactas no alertan y 9 h 1 min sí. |
| RN-12 | **Descanso en jornada continuada**: se alerta si un tramo continuo supera el **máximo sin pausa** del perfil de cumplimiento del centro (`break_required_after_hours`; **6 h** en `ES-hosteleria`). El límite es abierto: 6 h exactas no alertan y 6 h 1 min sí. **Abre incidencia `missing_break` cuando el fichaje de pausa está activado en la instalación, y solo entonces** — ver la nota de más abajo. |
...
>
> **Ese caso no está cubierto por el producto, y hay que decirlo así.** El motivo por el que no se evalúa es operativo, no jurídico: en la jornada partida —el turno típico de hostelería— ese hueco es la pausa, y medirlo contra el descanso mínimo convertiría cada jornada partida en un aviso. Una bandeja con un aviso por cada jornada partida es una bandeja que se deja de mirar, y el aviso que sí importa se pierde dentro. La pausa declarada (tarea 3.5, decisión 9) **no cambia esto**: lo que aporta es RN-12 —un tramo continuo deja de serlo cuando la pausa lo parte— y el detalle del registro, que distingue en el panel y en el portal una pausa de un hueco mudo (`opened_by`/`closed_by` en el Anexo B).
>
> **El producto no dictamina si una jornada cumple el art. 34.3 ET.** Ni aquí ni en ninguna otra regla: lo que hace es aplicar el umbral que el cliente ha puesto en su perfil de cumplimiento y abrir incidencia cuando se cruza. **Qué convenio aplica, cómo se computa el descanso en una jornada partida y si un caso como el de arriba es o no conforme lo decide la asesoría laboral del cliente, no el fabricante** (RL-16, RL-21; `docs/cliente/obligaciones-legales.md` §7). Por eso el hueco no evaluado se declara por escrito en la guía de RRHH (§4 bis) en lugar de dejarlo implícito: quien revisa el registro tiene que saber qué mira el sistema y qué tiene que mirar él.

> **Sobre RN-12: se evalúa cuando el fichaje de pausa está activado, y solo entonces** (tarea 3.5). La regla se evalúa en el dominio con el umbral del perfil de cumplimiento (6 h en `ES-hosteleria`) y tiene su prueba unitaria en los límites 5:59 / 6:00 / 6:01. Lo que decide si además **abre incidencias `missing_break`** es un ajuste de la instalación, `ATTENDANCE_BREAK_CLOCKING` (RF-AT-12, RF-PD-01), que se entrega **desactivado**.
>
> El motivo de la condición es el mismo que la mantuvo suspendida desde el 30-08-2026, y no ha cambiado: con [ADR-024](adr/ADR-024-la-pausa-son-dos-tramos.md) la pausa son dos tramos, así que **en una instalación donde la plantilla no ficha la pausa cada turno de más de 6 h abriría una incidencia** —cientos a la semana sobre la semilla de desarrollo— sin que nadie pueda distinguir «no descansó» de «descansó y no lo fichó», y esa avalancha taparía las incidencias que sí importan. Con el fichaje de pausa activado esa distinción ya existe: un tramo continuo largo es, literalmente, un tramo que nadie partió.
>
> **La suspensión sigue declarándose en un único sitio** —`Shared\Domain\ValueObject\ComplianceRuleSuspension`— y de ahí la derivan la pasada de detección, el asiento de auditoría del cambio de umbral y la pantalla del perfil; lo que cambia en la 3.5 es que **deja de ser una constante y se construye a partir del ajuste**, de modo que las tres siguen diciendo lo mismo sin que nadie las sincronice. La razón que se publica es la de verdad: `break_clocking_disabled`, «el fichaje de pausa está desactivado en esta instalación», y viaja en `ComplianceRuleStatus.suspension_reason` (RF-PA-06).
>
> **Activarlo no reprocesa el pasado.** La revisión diaria empieza a abrir `missing_break` en su siguiente pasada y **solo dentro de su ventana** (`COMPLIANCE_INCIDENT_LOOKBACK_DAYS`, 7 días de serie), como cualquier otro cambio de criterio: nada anterior a esa ventana se recalcula. **Desactivarlo vuelve a suspender la regla y no cierra ninguna incidencia ya abierta** (regla dura 5, RN-13). Mientras esté desactivado, cambiar `break_required_after_hours` **no altera ninguna incidencia**, y así lo dicen el asiento (`affects_incident_detection: false` con `detection_suspended: true`) y la pantalla. El tipo `missing_break` permanece en el catálogo de `incidents.type` en los dos casos.
>
> **Una pausa sin vuelta no abre incidencia en esta versión.** Quien pulsa «Pausa», pasa la tarjeta y se va a casa deja la jornada cerrada en ese instante, con la marca de pausa y sin tramo de vuelta; el registro dice exactamente lo que ocurrió y se corrige como cualquier otro tramo (RN-13). No se inventa una salida, no se cierra nada por debajo y **no se crea un tipo de incidencia nuevo**: haría falta migración, catálogo y bandeja para un caso que ya se ve en el registro y que RN-08 —turno abierto prolongado— no cubre, porque aquí no queda ningún turno abierto. La cota de RN-05 es lo que impide que esa pausa se coma la jornada siguiente.

> **Sobre RN-17: es informativa y no abre incidencia** (tarea 3.4). El art. 34.1 ET fija la jornada semanal ordinaria **en cómputo anual** —«cuarenta horas semanales de trabajo efectivo de promedio en cómputo anual»—, así que una semana por encima de las cuarenta **no es por sí sola un incumplimiento**: puede compensarse con otra más corta dentro del año, y muchos convenios de hostelería reparten así los turnos de temporada. Por eso la vista de cumplimiento (RF-PA-06) la **señala** para que RRHH la contraste con el convenio, y la bandeja de incidencias no la recibe: una incidencia es trabajo pendiente para una persona, y «esta semana fueron 41 h» es información, no una tarea. Abrirla convertiría la bandeja en un calendario de horas extra y taparía lo que sí hay que corregir.
>
...
>
> El motivo es el mismo que acota la ventana: una incidencia abierta hoy sobre una jornada de hace dos años no describe nada que nadie pueda corregir, y **cerrar** automáticamente incidencias que dejaron de serlo destruiría el rastro de una decisión que tomó una persona (regla dura 5, RN-13). Lo que sí queda es la **traza del cambio**: cada umbral modificado deja su asiento en `audit_log` con el valor anterior, el nuevo, el autor y el momento, que es lo que permite explicar dentro de dos años por qué una jornada de marzo no generó alerta y una de abril sí.
>
> **La vista de cumplimiento (RF-PA-06) se comporta al revés, y hay que saberlo antes de comparar las dos pantallas.** No conserva nada: **vuelve a medir** las jornadas del periodo cada vez que se consulta, siempre con el umbral vigente **en ese momento**. Así que bajar `min_rest_hours` hace aparecer en la vista jornadas de hace tres semanas que ayer no salían, **sin** abrir ni una incidencia nueva en la bandeja — y subirlo las hace desaparecer de la vista dejando abiertas las incidencias que ya existían.
>
> No es una incoherencia: son dos cosas distintas. La bandeja es **trabajo pendiente** —alguien decidió mirar esto, y esa decisión no se borra al mover un número—; la vista es una **lectura** del registro contra el criterio de hoy. Por eso la vista publica en cada respuesta el perfil con el que ha medido y los cuatro umbrales aplicados (`meta.profile`, `meta.rules[]`): sin ellos, dos capturas de la misma pantalla en fechas distintas serían imposibles de explicar ante un empleado, y es justo la explicación que hace falta cuando alguien pregunta por qué su jornada aparece ahora y antes no.
...
**`devices`** — `id`, `uuid`, `site_id`, `name`, `token_hash`, `app_version`, `last_seen_at`, `pending_queue_size`, `status`, `paired_at`

> `uuid` faltaba en esta lista aunque la decisión es de la tarea 1.5 y el resto del documento ya lo nombra (ver la nota de `credentials`): el `BIGINT` no sale nunca de la base de datos, y `POST /api/v1/devices/{uuid}/unpair` no podría cumplirse sin él.
>
> **`paired_at` entra con la tarea 5.6** (RF-PD-06). Es cuándo se vinculó el quiosco, o cuándo se **re**vinculó por última vez si la fila se reactivó con el mismo nombre ([ADR-028](adr/ADR-028-limites-del-plan-no-bloquean.md)). No es redundante con `created_at`: la fila es «el quiosco de Recepción» y sobrevive a la tablet que la atiende, así que su fecha de creación no dice desde cuándo está el aparato que hay hoy — que es la primera pregunta cuando algo deja de funcionar. Es `NULL` en los dispositivos dados de alta antes de que existiera el emparejamiento por código: no se les inventa una fecha.

**`device_pairing_requests`** — solicitud de emparejamiento de un quiosco (RF-PD-06, tarea 5.6)
`id`, `uuid`, `code_hash`, `secret_hash`, `status`, `app_version`, `device_id`, `confirmed_by_user_id`, `expires_at`, `confirmed_at`, `claimed_at`
*Estados `pending` → `confirmed` → `claimed`; «caducada» es derivado de `expires_at` y no se escribe. Índice único parcial sobre `code_hash` `WHERE status = 'pending'`, que es lo que garantiza que un código señale una sola solicitud. Código y secreto solo en hash (SHA-256). Sin datos personales: las filas consumidas o caducadas se purgan a las 24 h.*
...
**`license`** — `id`, `signed_key`, `license_id`, `customer_name`, `plan`, `max_employees`, `max_devices`, `features` (JSONB), `valid_from`, `valid_until`, `issued_at`, `activated_at`, `activated_by_user_id`, `last_verified_at`

> **Una fila, garantizada por el esquema** (tarea 5.3). Hay una licencia por instalación, como hay un centro (ADR-040): lo impone un índice único sobre una expresión constante, no el adaptador. **Nace vacía**, y «sin licencia» es un estado normal del producto: se ficha, se consulta el registro, se exporta para la Inspección y se hacen copias exactamente igual (regla dura 15, ADR-019).
>
> **`signed_key` es la afirmación; el resto es proyección legible.** La única columna con valor es la clave firmada. Los campos descompuestos existen para que una consulta de diagnóstico o una copia se puedan leer sin verificar nada, y **el estado que gobierna el producto se recalcula siempre desde `signed_key`**. Si alguien edita `valid_until` con `psql`, la firma deja de cuadrar, el estado pasa a `unverifiable` y el sistema sigue funcionando degradado: es el comportamiento correcto para un control **comercial** (§8.1), que no se refuerza a costa de bloquear el registro.
>
> **`license_id`, `valid_from` e `issued_at`** los añade la tarea 5.3: el primero identifica la licencia en el asiento de auditoría y por teléfono; los otros dos permiten activar con antelación una renovación cuya vigencia empieza más adelante, y saber cuándo se emitió.
>
> **Renovar sobrescribe la fila y la historia queda en `audit_log`** (`license.activated`), con actor, momento, plan, límites y vigencia. Conservar aquí también las claves antiguas daría dos fuentes de verdad sobre cuál está vigente.
>
...
**`report_exports`** — informes y exportaciones generados en diferido (RF-IN-06, RF-IN-07, tarea 3.9)
`id`, `uuid` (v7, identificador público), `kind` (`period`|`payroll`), `format` (`csv`|`xlsx`|`pdf`; `payroll` solo `csv`|`xlsx`), `parameters` (JSONB: `from`, `to`, `granularity`, `group_by`, `include_open_shifts`, `department_id`, `employee_uuid`), `scope` (JSONB, instantánea del alcance del solicitante), `requested_by_user_id` (NOT NULL), `requested_at`, `started_at`, `completed_at`, `failed_at`, `failure_reason` (catálogo cerrado, solo la clase del fallo), `status` (`pending`|`running`|`completed`|`failed`|`purged`, con `CHECK`), `file_path`, `file_name`, `size_bytes`, `sha256`, `row_count`, `criteria` (JSONB, los criterios ya traducidos al idioma del solicitante), `expires_at`, `purged_at`, `download_token_hash`, `download_token_expires_at`, `downloaded_at`, `download_count`, `notified_at`, `notification_channel` (`mail`|`panel`), `created_at`, `updated_at`
*Ciclo de vida calcado del de `data_exports`, con una diferencia deliberada: la restricción de «una sola en curso» es **por solicitante**, no por instalación, porque dos responsables pidiendo su informe a la vez no se estorban. El fichero caduca a los `REPORTING_EXPORT_RETENTION_DAYS` y lo purga una tarea diaria. Pedir, generar y descargar dejan asiento (`report_export.requested|generated|downloaded`, familia `PersonalDataAccess`).*

> **Del enlace de descarga solo se guarda su `sha256`, y rota** ([ADR-041](adr/ADR-041-descarga-de-ficheros-diferidos-con-enlace-de-un-solo-uso.md)). El token en claro se emite al consultar el estado y no se almacena nunca; cada consulta emite uno nuevo, que invalida el anterior; caduca a los `REPORTING_EXPORT_LINK_TTL_MINUTES` y **se consume en la descarga**. Un token filtrado —en un historial de navegador, en un chat— abre como mucho una descarga y durante minutos, y no lleva sesión: no es una credencial del panel.
>
> **`scope` es una instantánea, no una consulta.** El alcance del solicitante se congela al pedir la exportación y el trabajo lo aplica tal cual. Si entre la petición y la generación alguien amplía el ámbito de esa cuenta, el fichero sigue conteniendo lo que se autorizó al pedirlo (RF-ID-03), y el asiento de la generación puede compararse con el de la petición.
>
> **La fila queda `purged`, no se borra** (regla dura 5). Lo que desaparece es el fichero; la constancia de que se generó una exportación con datos de terceros, quién la pidió y quién se la descargó tiene que sobrevivir al fichero, porque es lo que permite acotar una brecha (RL-15) mucho después de que el CSV haya caducado.
>
> **Pero al purgar, la fila se adelgaza:** pierde `parameters.employee_uuid`, `parameters.department_id` y `scope` entero. Es minimización, no olvido — **el hecho completo, con el alcance y el sujeto, sigue en `audit_log`** (`report_export.requested`), donde tiene el plazo de RL-02 y la protección de ADR-010. Conservar el mismo dato indefinidamente en una tabla operativa, con `UPDATE` y `DELETE` disponibles y sin plazo, sería guardarlo dos veces y peor la segunda. Por eso **`report_exports` no entra en `RetentionScope`**: después de la purga no queda en ella ningún dato personal que retener.
>
> **Entra, en cambio, en la exportación íntegra** (`DataExportCatalog`, RF-PD-14): el cliente se lleva qué informes se generaron, cuándo, por quién y con qué resultado. **Sin `file_path` ni `download_token_hash`**: el primero es una ruta del servidor y el segundo, aunque sea un hash, es material de autenticación y no tiene por qué viajar en un ZIP.
>
> **`criteria` existe porque el fichero de nómina no los lleva dentro.** El informe por periodo imprime sus criterios debajo de la tabla; un CSV que entra en un programa de nómina no admite una línea de comentario sin romper la importación. Los criterios se conservan aquí, se enseñan en la pantalla y viajan en la cabecera `X-Kronoqr-Export-Criteria` de la descarga síncrona.

**`weekly_summary_deliveries`** — envíos del resumen semanal por correo (RF-PR-05, tarea 3.12)
`id`, `manager_user_id` (FK → `users`), `week_start` (DATE, lunes de la semana ISO enviada, en el calendario civil del centro), `sent_at` (TIMESTAMPTZ), `employee_count`, `row_count`, `created_at`
*`UNIQUE (manager_user_id, week_start)` es la idempotencia del envío semanal: `reporting:weekly-summary` —los lunes a las 06:00 UTC, o a mano con `--week=AAAA-Www`— **reclama** la fila en una transacción corta antes de componer nada, envía el correo **fuera de toda transacción** —ninguna E/S de red bajo el candado de la cadena de auditoría— y, solo si salió, escribe el asiento en otra transacción corta; si no salió, retira la reclamación y cuenta el fallo sin asiento, y la semana queda libre para el reintento; si ya estaba entregada, una segunda ejecución la salta, y dos pasadas simultáneas no duplican porque la segunda tropieza con el `UNIQUE` y omite. No hay estado ni `notified_at`: la fila existe o no existe. Es rastro operativo, no legal —el hecho con relevancia legal, que salieron por correo datos de N personas a una cuenta, es el asiento `personal_data.accessed` con conjunto `weekly_summary` (RS-05), con `manager_user_id`, `week_start` y, solo cuando el alcance tiene 50 personas o menos, la lista de `employee_uuid` (por encima, `employees` y `scope`), y nunca nombres—. La tabla guarda recuentos y ningún dato personal más allá de la cuenta destinataria, así que, como `report_exports`, no entra en `RetentionScope`. La activación es el ajuste `WEEKLY_SUMMARY_EMAIL` de `installation_settings` (`disabled` de serie) y exige `Feature::WeeklyEmailSummary` en la licencia; sin ella la pasada se omite, nunca falla (ADR-019).*
...
>
> **`missing_break`** es el tipo que faltaba para RN-12. Con [ADR-024](adr/ADR-024-la-pausa-son-dos-tramos.md), «sin pausa registrada» significa exactamente «un solo tramo continuo por encima del umbral»; colapsarlo en `long_shift` haría indistinguible en la bandeja «ha trabajado 9 h y media hoy» de «lleva 6 h y media sin parar», y se resuelven de forma distinta. **`out_of_order_scan`** es el de RN-18: lo abre la revisión diaria leyendo `scan_events.result`, describe **la jornada** y no un tramo —`shift_entry_id` nulo, una por empleado y jornada— y se resuelve creando o rectificando el tramo que falta (RN-13). **`anomalous_pattern`** lo abre la detección de patrones de RF-PR-06 (tarea 3.11): describe **la jornada** en la coincidencia entre dos personas —`shift_entry_id` nulo, `work_date` el día en que el par alcanzó el mínimo de repeticiones— y **el tramo** del segundo escaneo en la secuencia imposible de RN-16; su `context` lleva el patrón, los quioscos por identificador y nombre de dispositivo, los momentos y los umbrales aplicados, y la otra credencial del par por `employee_uuid`, nunca por nombre. **`missing_clock_out`** sigue en el catálogo sin detector: describe el olvido ya corregido a mano, no lo que la detección automática ve.

**`absences`** — `id`, `uuid` (UUID v7 UNIQUE, identificador público), `employee_id` (FK `RESTRICT`), `type` (`vacation`|`sick_leave`|`leave`|`other`), `starts_on`, `ends_on` (fechas civiles **inclusivas**, `CHECK ends_on >= starts_on`), `note` (TEXT NULL, ≤ 500), `status` (`active`|`superseded`|`voided`), `version` (≥ 1), `supersedes_id`, `superseded_by_id` (FK a la propia tabla, nulas), `change_reason`, `voided_at`, `voided_by_user_id`, `void_reason`, `created_at`, `created_by_user_id`

> **Las once columnas que el enunciado original no tenía** las añade la tarea 3.10, que es la que crea la tabla. Seis de ellas existen por la **regla dura 5** —nada se borra ni se sobrescribe (RN-13, RL-04)— y las otras cinco por lo que el registro tiene que poder explicar:
>
> - **`status`, `version`, `supersedes_id` y `superseded_by_id`** son el versionado, con la misma técnica que `shift_entries` ([ADR-026](adr/ADR-026-la-correccion-supersede.md), [ADR-035](adr/ADR-035-la-correccion-estrena-identificador-y-no-cambia-de-jornada.md)): corregir una ausencia **crea una fila nueva** con `version + 1`, `uuid` nuevo y `supersedes_id` apuntando a la anterior, y lo único que se escribe sobre la fila anterior son su `status` y su `superseded_by_id`. Tipo, fechas, nota, autor y momento no se tocan jamás. Una ausencia cambia el resultado de un informe de absentismo, que tiene consecuencias laborales: sin las versiones, la pregunta «¿por qué el informe de marzo dice hoy otra cosa?» no tiene respuesta en la tabla.
> - **`change_reason`** es obligatoria a partir de la segunda versión (`CHECK ((version = 1) = (change_reason IS NULL))`): una corrección sin motivo no es explicable ante una inspección. Es texto libre de 3 a 500 caracteres, **sin catálogo cerrado** a diferencia de las correcciones del registro horario (Anexo C): ahí hay nueve causas tipificadas que defender, aquí no las hay.
> - **`voided_at`, `voided_by_user_id` y `void_reason`** hacen de la anulación **un hecho con nombre**, no un `DELETE`: `CHECK ((status = 'voided') = (voided_at IS NOT NULL))`. Anular **no** crea versión —no hay versión posterior de un hecho que no pasó—, y la fila anulada sigue siendo consultable, que es lo que permite explicar por qué un informe de la semana pasada traía un día que el de hoy ya no trae. Nunca `DELETE`, en ninguna capa.
> - **`created_at` y `created_by_user_id`**, como en `employment_contracts`, dejan legible en la propia tabla quién anotó la ausencia y cuándo, sin cruzar el trail. `created_by_user_id` es NULL en importación y semilla.
> - **`uuid`** es el identificador público, por la misma razón que en `credentials` y `devices`: el Anexo B expone `/api/v1/absences/{uuid}` y el `BIGINT` no sale nunca de la base de datos.
>
> **El solape lo impide PostgreSQL, no el código**: `absences_no_overlap EXCLUDE USING gist (employee_id WITH =, daterange(starts_on, ends_on, '[]') WITH &&) WHERE (status = 'active')`, la misma técnica que `employment_contracts_no_overlap` y que RN-02. La restricción es **parcial** a propósito: solo las activas se excluyen entre sí, porque una versión supersedida o una ausencia anulada conservan sus fechas y chocarían con su propia sucesora. La comprobación equivalente en PHP existe solo para poder explicar el choque con un `409`; la garantía es de la base de datos.
>
> **`note` puede contener datos de salud, y por eso tiene un tratamiento distinto del resto de columnas.** Es donde alguien escribirá «operación de rodilla» aunque la guía pida que no lo haga. Consecuencias: **no entra nunca en `audit_log`** —el asiento lleva `has_note` como booleano, y con el tipo, las fechas y el autor ya describe el hecho—, no aparece en ningún log técnico ni en `error_events` (**regla dura 21**), y no viaja en el paquete de diagnóstico. El **tipo** sí entra en el asiento: sin él la anotación no describe qué cambió, y `audit_log` es el registro legal con acceso restringido, no un log técnico. En la API, `note` solo la reciben `admin` y `rrhh`; para el responsable de departamento el campo **no viaja**, para que no se confunda con «sin nota».
...
> **Dos acciones para el ciclo de vida de la instalación** (`system.updated` y `system.restored_from_backup`, RF-PD-10, tarea 5.7). La actualización asistida y su vuelta atrás automática dejan asiento, con actor `system` y sujeto `installation`. El motivo es que **una vuelta atrás no deja ningún hueco visible en la cadena**: lo que queda tras restaurar es la cadena íntegra de la copia, `compliance:verify-audit-chain` sale en verde, y el intervalo descartado —que puede contener fichajes reales de un turno de noche— es invisible desde el propio registro. Por eso `system.restored_from_backup` **se escribe sobre la cadena ya restaurada**: su `prev_hash` es la huella del último hecho que sobrevivió, y el asiento significa «lo que hay antes de mí es la copia de las HH:MM». Los dos payloads tienen **lista cerrada de claves**, sin datos personales, sin secretos y sin rutas absolutas del servidor; `reason` y `failed_step` son vocabularios cerrados, no texto libre (regla dura 21). Los escribe el instalador con `php artisan compliance:record-system-event`, y **ningún actor humano puede firmarlos**.
...
| **Suplantación** | Préstamo de la tarjeta a un compañero (*buddy punching*) | Fraude **autolimitado**: prestarla deja al titular sin la suya, exige entrega y devolución, y solo funciona si el titular no piensa fichar. Mitigación: supervisión presencial y detección automática de patrones anómalos **(RF-PR-06, tarea 3.11)**: coincidencias sistemáticas de dos personas en el mismo quiosco separadas por segundos, y secuencia imposible de una misma credencial entre dos quioscos (RN-16). La señal es la incidencia `anomalous_pattern` en la bandeja, **sin alerta a propósito** y sin ninguna acción automática: la revisa una persona con el runbook `patron-anomalo-credencial.md`. | — |
...
| **Manipulación** | Reloj de la tablet adelantado o atrasado —por avería, por configuración o a propósito— para que un fichaje conste a una hora distinta de la real | `recorded_at` es la hora del servidor y la tablet no la controla (RF-AT-09); `occurred_at` se admite con un desfase tolerado hasta el umbral configurado y, por encima, el fichaje **se registra igual**, con aviso en el quiosco e incidencia `clock_skew` para revisión humana (RF-AT-10). El anti-rebote (RF-AT-06) y la secuencia imposible entre dispositivos (RN-16) acotan lo que un reloj movido puede producir. **Nunca se rechaza un fichaje por desfase** (regla dura 19). | `T1070.006 Eliminación de indicadores: manipulación de marcas de tiempo` |
...
| **Divulgación** | El registro de ausencias expone **dato de salud** (art. 9 RGPD): el tipo `sick_leave` de una persona identificada, y la nota que alguien escriba junto a él | Lectura acotada a `rrhh` y `admin`; el `responsable_departamento` ve **solo su departamento** y **sin el campo `note`** —omitido, no `null`, para que no se confunda con «sin nota»— (RF-GP-04, RF-ID-03). Toda página leída deja asiento `personal_data.accessed` con conjunto `absence_register`, con el alcance y **sin el dato** (RS-05). `note` **no entra nunca** en `audit_log` —el asiento lleva `has_note` como booleano—, ni en logs técnicos, ni en `error_events`, ni en el paquete de diagnóstico (regla dura 21). El informe por periodo **no desglosa por tipo** a propósito: una columna «días de baja médica» por departamento sería dato de salud agregado. El `responsable_departamento` ve el tipo de su departamento sin la nota; **ningún acceso de soporte del fabricante alcanza el registro**, con ningún alcance (cerrado en el cierre de la Fase 3: `AbsencePolicy` rechaza a todo actor de soporte, con prueba por alcance). | `T1213 Datos de repositorios de información` |
| **Divulgación** | El paquete de diagnóstico contiene datos personales | Anonimizado por defecto (RF-PD-09, RL-19). | — |
| **Divulgación** | Un enlace de descarga de un informe en diferido se filtra —historial de un navegador compartido, un reenvío, un registro de proxy— o se adivina, y entrega un fichero con las horas de la plantilla sin pasar por ninguna sesión | El enlace **no existe hasta que alguien consulta el estado desde el panel con su sesión**, y cada consulta **rota** el anterior: no hay una URL viva desde que se encola el trabajo. Son 32 bytes de `random_bytes` de los que en la fila solo vive su `sha256`, comparado con `hash_equals`; **se consume en la primera descarga** y caduca a los `REPORTING_EXPORT_LINK_TTL_MINUTES` (15 de serie). El fichero se purga a los `REPORTING_EXPORT_RETENTION_DAYS` y **al purgarlo la fila pierde `employee_uuid` y el alcance**. Zona propia por IP `throttle:report-download`. Cada descarga deja asiento `report_export.downloaded` **antes** de entregar el fichero (RS-05, RL-15). **El correo nunca lleva el enlace** ([ADR-041](adr/ADR-041-descarga-de-ficheros-diferidos-con-enlace-de-un-solo-uso.md)). | `T1528 Robo de token de acceso de aplicación` |
| **Divulgación** | Datos personales de la plantilla **salen del servidor por SMTP, a diario y sin que nadie pulse nada**, hacia buzones que el producto no controla: el aviso de incidencias pendientes (RF-PR-01, con fecha, **nombre**, tipo y severidad de cada línea —desde la 3.11 también las de `anomalous_pattern`, que es una sospecha de fraude junto a un nombre—) y el resumen semanal de horas (RF-PR-05, opt-in). El relevo de correo suele ser de un tercero, y el correo entregado se puede reenviar, imprimir y archivar fuera del sistema | Lo mínimo por línea y nunca el registro horario: el aviso diario corta a **20 líneas** detalladas (`IncidentDigestNotification::DETAILED_LINES`) y el resumen semanal a **50** (`WeeklySummary::MAXIMUM_DETAILED_LINES`), con recuentos de ausencias y de festivos pero **nunca el tipo de ausencia**, de modo que por aquí no sale dato del art. 9. El resumen semanal **viene apagado** (`WEEKLY_SUMMARY_EMAIL = disabled`), encenderlo es un cambio auditado de impacto `data_disclosure` **reservado al cliente** (el soporte del fabricante recibe `403`), y lo reciben solo los responsables de departamento, cada uno con el mismo `AccessScope` del panel aplicado en el `WHERE`: ni `admin`, ni `rrhh`, ni `auditor`. Cada envío **entregado** deja asiento `personal_data.accessed` con conjunto `incident_digest` o `weekly_summary` —con los `employee_uuid` afectados; en el semanal, uno a uno cuando el alcance tiene 50 personas o menos y por recuento y alcance por encima— y el sello de aviso se escribe **después** de entregar, así que un correo no enviado no se da por avisado (RS-05, RL-15, RF-PR-01, RF-PR-05). El cifrado del canal es del cliente y está escrito donde lo lee: `MAIL_SCHEME=smtps` **falla en vez de degradarse**, mientras vacío o `smtp` es STARTTLS oportunista y expone el contenido en tránsito (`T1557`); si el relevo es de un tercero, ese tercero es **encargado del tratamiento del art. 28 RGPD** y el contrato de encargo es del cliente (`docs/cliente/endurecimiento.md` §7, `obligaciones-legales.md` §2). **Lo que no se mitiga**: el plazo de conservación del correo ya entregado lo fija ese buzón y no `RetentionScope` (RL-11) | `T1114 Recolección de correo` |
...
| **Elevación** | Acceso de soporte usado fuera del incidente que lo motivó | Concesión expresa, temporal, de alcance limitado, revocable y auditada de forma visible para el cliente (RF-PD-11). El alcance de lectura queda **acotado a lo que hace falta para diagnosticar un cálculo de horas** —plantilla y registro horario de una persona, cada lectura con asiento de divulgación— y **cerrado por policy** al registro de ausencias (dato de salud), a la presencia en vivo y al resumen de cumplimiento por persona, con `SupportScopeRoutesTest` recorriendo las rutas registradas (decisión del 24-09-2026, tras el cierre de la Fase 3). | `T1199 Relación de confianza` |

> **Para qué sirve la columna ATT&CK.** STRIDE dice qué se mitiga; MITRE ATT&CK (matriz Enterprise; el identificador es el canónico, el nombre corto va traducido) nombra la técnica tal como la buscaría un analista en un SIEM o en un informe de incidente, y obliga a mirar el otro lado del control: la **detección**. Cada técnica cuya mitigación figura como resuelta debería tener además una señal observable, porque un control sin señal no distingue «nadie lo intentó» de «lo intentaron y falló». Hoy la tienen `T1565.001` y `T1070` sobre `audit_log` —verificación diaria de la cadena y alerta `RoturaDeCadenaDeAuditoria` (RS-07)—, `T1070.006` desde la tarea 3.5 —la señal es la propia incidencia `clock_skew` en la bandeja (`incidents_open{type="clock_skew"}`), **sin alerta a propósito** porque la juzga una persona, y `recorded_at` conserva la hora real de llegada aunque la tablet mienta— y `T1110.001` la está recibiendo con la auditoría de autenticación y las alertas `KronoqrAuth*` que se incorporan en `infra/observability` (doc 07 §5; hasta que queden en verde, la señal es solo el bloqueo por intentos y la telemetría del portal, sin alerta). **Con detección desde la 3.8:** `T1606` (los rechazos de firma se cuentan en `scans_total{result="rejected_signature"}` y la alerta `RechazoDeFirmaQr` avisa a seguridad), `T1499.002` (el borde responde `429`; la alerta `SaturacionDelBordeEnElFichaje` mide la capa de aplicación, no hay exportador de Nginx). **Con detección desde la 3.10:** `T1213` (cada página leída del registro de ausencias deja asiento `personal_data.accessed` con conjunto `absence_register` y el alcance de la consulta, RS-05; **sin alerta a propósito**, porque la lectura por RRHH es el uso normal de la pantalla y lo que el asiento permite es reconstruir después quién miró qué, no avisar de que alguien mira). **Sin detección a propósito:** `T1550.001` (un `403` por ámbito de token no deja asiento, a propósito —ADR-037, tarea 3.8—: la señal es la métrica `http_requests_total{status="403"}` y la prueba de autorización negativa de cada endpoint, y el asiento `access.denied` queda para la denegación por alcance), `T1528` (un enlace consumido por quien no debía es indistinguible en el borde de uno consumido por su dueño; queda el asiento de la descarga, que permite reconstruirlo después, y el `410 …link-used` que ve el legítimo cuando alguien se le adelantó), `T1199` (la concesión auditada llega con la tarea 5.9) y `T1005` (por naturaleza solo es mitigable: cifrado y purga). Las dos filas con «—» no describen a un adversario informático sino a una persona con acceso legítimo o a un proceso propio: el préstamo de tarjeta es un fraude físico que ATT&CK no modela y cuya señal es el patrón anómalo (RF-PR-06); el paquete de diagnóstico con datos personales es un fallo de minimización, no un ataque. Cuatro técnicas relevantes no tienen fila porque su mitigación vive fuera de este modelo: `T1195.002 Cadena de suministro: software` (RS-10), `T1557 Intermediario` (TLS 1.3 obligatorio, doc 02 §7.1), `T1552 Credenciales no protegidas en ficheros` (RS-08) y `T1200 Hardware adicional` en la VLAN de quioscos (segmentación por `KIOSK_VLAN_CIDR`). La madurez y la cobertura técnica → detección se siguen en `docs/07-seguridad-madurez-y-amenazas.md`.
...
> **Los hallazgos de RF-PR-06 no están en este catálogo, a propósito** (tarea 3.11). Una incidencia `anomalous_pattern` es un indicio sobre dos personas concretas y se revisa **en la bandeja**, por el responsable del departamento que conoce el turno, con el runbook `patron-anomalo-credencial.md`; enrutarla a un canal de guardia convertiría el indicio en aviso y el aviso en acusación. Lo que sí alerta es **que la pasada no corra o falle**, que es operación y no personas: `DeteccionDePatronesAusente` (más de 26 h sin ejecutarse `attendance:detect-patterns`, Media) y `DeteccionDePatronesConFallos` (`pattern_detection_last_failures > 0`, Alta), las dos al IT del cliente y con el mismo runbook, §«Si la detección no corre». Son las hermanas de `DeteccionDeIncidenciasAusente`/`DeteccionDeIncidenciasConFallos`, que tampoco figuran aquí por la misma razón: vigilan la detección, no el registro.
...
Escenario: Jornada continuada sin pausa
  ...
  Y el fichaje de pausa activado en la instalación
  ...
  Y con el fichaje de pausa desactivado no se crea ninguna en ningún caso
...
Escenario: Semana por encima de la jornada ordinaria
  Dado un perfil con 40 horas de jornada semanal ordinaria que empieza la semana el lunes
  Y un empleado con cinco jornadas de 8 horas y 12 minutos entre el lunes 9 y el viernes 13
  Cuando el responsable consulta la vista de cumplimiento del jueves 12
  Entonces la semana del 9 al 15 se señala con 41 horas frente a 40
  Y se evalúa completa aunque el periodo consultado abarque un solo día
  Y no se abre ninguna incidencia, porque el cómputo del art. 34.1 ET es anual
```
...
POST   /api/v1/kiosk/pair/claim            Recoger el token vinculado   [público, un solo uso]
POST   /api/v1/kiosk/pair/confirm          Confirmar código y vincular  [rol: admin]
GET    /api/v1/devices                     Quioscos vinculados          [rol: admin]
POST   /api/v1/devices/{uuid}/unpair       Desvincular un quiosco       [rol: admin, auditado]
...
POST   /api/v1/reports/exports             Generar exportación async    [rol: manager+; kind payroll, rrhh+]
GET    /api/v1/reports/exports             Mis exportaciones en diferido[rol: manager+, solo las propias]
GET    /api/v1/reports/exports/{id}        Estado y enlace de descarga  [rol: manager+, caducable]
GET    /api/v1/reports/exports/{id}/download Descarga del fichero       [sin sesión: token de un solo uso]
...
GET    /api/v1/compliance-profile          Perfil de cumplimiento       [rol: admin]
PATCH  /api/v1/compliance-profile          Modificar umbrales legales   [rol: admin, auditado]
GET    /api/v1/license                     Estado de la licencia        [rol: admin]
POST   /api/v1/license/activate            Activar clave                [rol: admin, auditado]
...
GET    /api/v1/setup/status                Estado del asistente         [público, throttle 10 r/m]
POST   /api/v1/setup/administrator         Primer administrador         [público, solo sin cuentas de gestión]
POST   /api/v1/setup/site                  Alta del centro (una vez)    [rol: admin]
GET    /api/v1/setup/steps                 Pasos y su estado            [rol: admin]
PUT    /api/v1/setup/steps/{step}          Marcar paso hecho u omitido  [rol: admin]
...
| `ScanIntent` (`/scan`, `/scan/pin` y cada elemento de `/scan/batch`) | **El servidor honra la intención declarada siempre**, esté o no activado el fichaje de pausa en la instalación. `break_start` con tramo abierto lo cierra como `break_start`; sin tramo abierto, `break_end` —y también `auto`— **continúa la jornada** si el último escaneo aceptado de esa persona fue un `break_start` **y desde él ha pasado menos tiempo que `min_rest_hours`** (RN-05), y abre jornada como `clock_in` si no. Una intención que contradice el estado se resuelve por la estructura, se registra tal cual y **nunca se rechaza** | RF-AT-12, ADR-024, regla dura 19, tarea 3.5. **El ajuste `ATTENDANCE_BREAK_CLOCKING` gobierna la pantalla de la tablet y la evaluación de RN-12, no la verdad de lo que la persona declaró**: un fichaje ya encolado en la tablet no puede quedar reinterpretado porque alguien cambiara un ajuste del panel entre el escaneo y su sincronización, y una cola offline sincronizada dos horas después tiene que producir la misma atribución que si hubiera llegado en línea. **Que `auto` continúe la pausa** es lo que permite que volver del descanso no añada ningún paso: solo el inicio de la pausa se declara, la vuelta es pasar la tarjeta |
| `POST /api/v1/kiosk/heartbeat` | **La respuesta gana `break_clocking_enabled` y `clock_skew_tolerance_seconds`**, los dos obligatorios | RF-AT-12, RF-AT-10, regla dura 13, tarea 3.5. El latido es **el único canal autenticado que la tablet repite cada minuto** —precedente de `service_code_hash`, tarea 3.3—, y la tablet guarda los dos valores en local para que el botón «Pausa» y el aviso de desfase funcionen **sin red**, que es cuando más falta hacen (regla dura 19). **La tolerancia viaja y no se compila**: la tablet avisa del desfase con el mismo umbral con el que el servidor marca el fichaje para revisión (`ATTENDANCE_MAX_CLOCK_SKEW_MINUTES`), en vez de con una constante propia que un cliente no podría cambiar. Aditivo sobre la v1 (ADR-012) |
| `WorkDayShiftEntry` (`/employees/{uuid}/workdays` y `/me/workdays`) | **Gana `opened_by` (`clock_in` \| `break_end`) y `closed_by` (`clock_out` \| `break_start` \| `null`)**, derivados del resultado de los escaneos que abrieron y cerraron el tramo. Un tramo declarado o corregido a mano sin escaneo detrás va como `clock_in`/`clock_out` | ADR-024, tarea 3.5. Sin ellos la pausa es **indistinguible de un hueco mudo**: dos tramos seguidos se ven igual venga de un descanso declarado o de un olvido de fichaje, y es justo la distinción que RF-AT-12 existe para crear. Es lo que permite que el panel y el portal enseñen «Pausa» entre dos tramos —«los informes… la tratan como pausa», consecuencia de ADR-024— y que un empleado entienda su propia jornada. **Sin cambio de esquema**: salen de `scan_events.result`, que ya admite los cuatro valores desde la tarea 1.3 |
| `ComplianceRuleStatus.suspension_reason` | Pasa de `awaiting_declared_break` a **`break_clocking_disabled`** | RF-PA-06, RF-AT-12, tarea 3.5. La razón que se publica tiene que ser **la de verdad**: RN-12 no está esperando a que el fabricante entregue nada, está desactivada porque esta instalación no ficha la pausa, y eso lo arregla el cliente desde su panel en un minuto. Una razón que culpa al calendario del fabricante deja al lector sin acción posible |
...
| `POST /api/v1/kiosk/pair`, `/pair/claim` y `/pair/confirm` | **Tres pasos, iniciados por la tablet** (tarea 5.6). **1)** La tablet sin vincular llama a `/pair` y recibe `{ pairing_id, pairing_secret, code, expires_at, poll_interval_seconds }`; muestra `code` en pantalla. **2)** El administrador lo teclea en el panel, que llama a `/pair/confirm` con `{ code, name }`: crea —o **reactiva**— la fila de `devices` y deja la solicitud confirmada. **3)** La tablet, que sondea `/pair/claim` con `{ pairing_id, pairing_secret }`, recibe `{ status: paired, device, token }` y la solicitud queda consumida | Reconcilia RF-PD-06 («código mostrado en la tablet e introducido en el panel») con el carácter público y de un solo uso de `/pair`. **Hacen falta tres y no dos**: el token es de la tablet, y sin un tercer paso tendría que salir por la respuesta del administrador y llegar allí por algún medio manual, que es justo el paso que RF-PD-06 existe para eliminar. **`/pair` y `/pair/claim` son públicas** porque quien las llama todavía no tiene token, y por eso sus rechazos son genéricos y de tiempo constante entre sí (regla dura 17): `pairing_id` desconocido, secreto incorrecto y solicitud caducada o consumida devuelven la misma respuesta. `kiosk:pairing-code` se conserva como vía alternativa de consola, coherente con «el cliente no tiene por qué usar SSH» |
| Formato y caducidad del código de emparejamiento | **Seis dígitos decimales** (`^[0-9]{6}$`), generados con CSPRNG y únicos entre las solicitudes pendientes; se muestran agrupados («483 921»). **Caducidad de 10 minutos** y **un solo uso**. Código y `pairing_secret` (32 bytes, base64url) se almacenan **hasheados** (SHA-256), como `devices.token_hash` y `credentials.secret_hash`. Las dos cifras son configuración de la instalación (`config/kiosk.php`), no constantes | Cierra lo que quedaba abierto de C-3 (tarea 5.6). **Seis dígitos** porque se lee de lejos y se teclea a mano: lo que protege el emparejamiento no es la entropía de lo que se enseña en una pantalla colgada de una pared, sino el **secreto de recogida** —que no sale de la tablet— y la caducidad corta. Solo dígitos porque un alfabeto mixto a dos metros convierte la O en un cero. **Hasheados** porque un código en claro en la base de datos es una credencial de alta de quiosco al alcance de cualquier volcado. **Configurables** por la regla dura 13: un cliente con un procedimiento más lento sube el TTL sin tocar el repositorio |
| `GET /api/v1/devices` y `POST /devices/{uuid}/unpair` | **Pasan de «manager+» a `admin`**, con ámbito `settings:*` y policy propia. Sin `POST /devices` ni `PATCH /devices/{uuid}`: un dispositivo no se da de alta a mano, nace de un emparejamiento. `unpair` es `POST` y no `DELETE`, devuelve el dispositivo ya revocado y es idempotente | Tarea 5.6. **Gestionar dispositivos es la misma potestad que configurar la instalación**: quien puede ver la flota es quien puede desvincularla y dejar un hotel sin quiosco en pleno cambio de turno, así que comparte ámbito con `/settings` en lugar de estrenar uno que ningún rol usaría por separado (§7.3 del documento 02, nota 5). **Sin alta ni edición manual** porque el nombre y el centro se fijan al confirmar el código, y un `PATCH` que renombrase un quiosco rompería la reactivación por nombre de [ADR-028](adr/ADR-028-limites-del-plan-no-bloquean.md). **Nada se borra** (regla dura 5): un quiosco revocado conserva su fila, porque los fichajes que registró tienen que poder atribuirse a algo |
...
| `GET` y `PATCH /api/v1/compliance-profile` | **Recurso propio y singular** (tarea 5.2), no una clave más de `PATCH /settings`. Devuelve el perfil **vigente para el centro** —el asignado o el de `is_default`— con `source` diciendo cuál de los dos. `jurisdiction`, `is_default` e `id` son de solo lectura, y no hay alta ni baja de perfiles | RF-PD-07. Un umbral **legal** lo fija la jurisdicción y uno **operativo** lo fija el hotel (§4): son dos tablas, dos potestades y dos consecuencias. En un mapa de clave y valor serían indistinguibles «he bajado el anti-rebote» de «he bajado los años que hay que conservar el registro», y el asiento de auditoría perdería el matiz. Es singular como `/site` por [ADR-040](adr/ADR-040-un-centro-por-instalacion-y-por-licencia.md): con un centro por instalación hay exactamente un perfil vigente, así que un segundo perfil no lo leería nadie. Comparte el ámbito `settings:*` porque el §7.3 del documento 02 no declara uno propio y crear uno que ningún rol usa por separado sería superficie sin dueño |
| `GET /api/v1/license` y `POST /api/v1/license/activate` | **Ámbito propio `license:*`** (§7.3 del documento 02), no `settings:*`. El `GET` **nunca devuelve `404`**: «sin licencia» es el estado `absent` del recurso. `POST /license/activate` recibe un solo campo, `signed_key`, y **admite una clave ya caducada**, que se guarda con su estado en `audit_log`. No hay `DELETE` | RF-PD-04, tarea 5.3. **Dos ámbitos y no uno** porque un ajuste operativo y un hecho comercial no son lo mismo, y porque el asistente de puesta en marcha (RF-PD-03) debe poder activar la clave sin poder tocar los umbrales con los que se calculan las horas. **Sin `404`** porque el caso más común de una puesta en marcha es no tener licencia, y tratarlo como error lo haría indistinguible de una ruta mal escrita. **Se admite una clave caducada** porque un hotel que renueva con retraso recibe una clave cuya vigencia empezó el día 1: rechazarla le obligaría a pedir otra por una diferencia de calendario. **Sin `DELETE`** porque no se «desactiva» una licencia, se activa otra: un endpoint para dejar la instalación sin licencia solo serviría para equivocarse. **Ninguna de las dos rutas se degrada jamás** ([ADR-019](adr/ADR-019-la-licencia-nunca-bloquea-el-registro.md)): son la pantalla desde la que se arregla el problema |
| Funcionalidad accesoria no disponible | **`402` con `application/problem+json`** y `type: urn:kronoqr:problem:feature-not-licensed`. Hoy lo devuelven `GET /api/v1/reports/period` y su exportación. El cuerpo lleva `feature`, `restriction` y `since`, y el `detail` **nombra lo que sigue disponible** | RF-PD-05, [ADR-019](adr/ADR-019-la-licencia-nunca-bloquea-el-registro.md), [ADR-023](adr/ADR-023-frontera-registro-legal-y-funcionalidad-accesoria.md). **`402` y no `403`**: un `403` mezclaría «no tienes permiso» con «tu empresa no ha renovado», que son dos problemas de dos personas distintas —lo arregla quien administra los roles frente a quien firma el contrato— y en un log serían indistinguibles. **Este código no puede acompañar jamás al registro legal**: la excepción que lo produce solo se construye a partir del catálogo de funcionalidades accesorias, y ahí no hay ningún caso legal. La presencia en tiempo real **no** usa este código: degrada a sondeo dentro del `200` (ADR-011) |
| `GET /api/v1/health` | **Gana el campo `license`** con el estado de la licencia, además de `status` y `version` | §10.5 del documento 02 y RF-PD-09: `doctor` y el paquete de diagnóstico tienen que poder informarlo. **Una palabra y nada más** —ni cliente, ni plan, ni fechas—: la sonda es pública y el resto es información comercial del cliente (ADR-020). Sale de una copia en caché y **no consulta la base de datos**, porque una sonda de vida que lo hiciera haría reiniciar el contenedor de PHP cuando lo caído es PostgreSQL; si esa copia no está, responde `unknown`, que no es lo mismo que `absent`. **No cambia el código de respuesta**: una licencia caducada devuelve `200` igual que una vigente |
...
| `GET /api/v1/attendance/live` | **Sin paginación** y con **tres filtros**: `department_id`, `status` (`present` \| `absent`, `present` por omisión) y `q` (nombre, apellidos, nombre completo y código, igual que `GET /employees`). La respuesta lleva `data[]` con la fila de cada persona del alcance y un `meta` con `generated_at`, `time_zone`, los dos recuentos y el bloque **`realtime`** | RF-PA-01 y RF-PA-02. **No se pagina** porque una instalación es un hotel (ADR-040) con hasta 500 empleados: paginar obligaría al panel a reconciliar cada mensaje del WebSocket contra una página que quizá no contiene a esa persona, que es como una vista en vivo acaba mostrando a alguien dos veces. **`generated_at` no es decorativo**: el tiempo transcurrido se calcula contra él y no contra el reloj del navegador (regla dura 3). **`realtime` viaja en la respuesta y no compilado en la SPA** —clave pública, ruta del WebSocket, endpoint de firma, evento, canales permitidos y periodo de sondeo—: el panel se compila una vez y se instala en el servidor de cada cliente (ADR-017, regla dura 13). Con `enabled: false` la vista **degrada a sondeo** y no se apaga (ADR-011, ADR-023) |
| Canal de presencia | **No es un endpoint, pero es contrato.** Canales privados `presence.all` y `presence.department.{id}`, evento `presence.updated`, firma en `POST /api/v1/broadcasting/auth` con el mismo token Bearer y el ámbito `attendance:read`. El mensaje se describe en `webhooks` del `openapi.yaml` | ADR-011. El cuerpo del mensaje es **exactamente** una fila del listado más el instante del cambio, para que el panel pinte igual venga del sondeo o del canal. La autorización del canal es tan obligatoria como la del endpoint (regla dura 18): un `responsable_departamento` firma sus departamentos y **no** el canal global |
...
| `/api/v1/setup/*` (seis rutas) | **Prefijo propio** para el asistente de puesta en marcha, con **dos rutas públicas**: `GET /setup/status` y `POST /setup/administrator`. Los controladores siguen en el módulo dueño del recurso —el primer administrador en `Identity`, el centro en `Workforce`—, y el prefijo agrupa el asistente **en el contrato**, no mueve la lógica de módulo | RF-PD-03, tarea 5.5. **Prefijo y no rutas repartidas** porque estas son de **un solo uso** y se cierran a la vez, mientras que los recursos que configuran viven para siempre: con `POST /api/v1/site` habría un alta de centros *permanente* —la primera pieza del multicentro que [ADR-040](adr/ADR-040-un-centro-por-instalacion-y-por-licencia.md) cerró, y que este mismo anexo niega por escrito— en lugar de un acto irrepetible. **Públicas por necesidad**: se llaman cuando no existe ninguna cuenta con la que autenticarse, que es literalmente el estado que describen |
| `POST /api/v1/setup/administrator` | Crea la **primera** cuenta de gestión, con rol `admin` fijo, y **solo mientras no exista ninguna cuenta de gestión** —activa o desactivada—. Devuelve `201` con un **`TwoFactorChallenge`**, no una sesión; `409` para siempre en cuanto hay una cuenta, y el `detail` remite a `POST /auth/login` | RS-06, RF-PD-03. **Es la única escritura pública del producto**, y existe porque el instalador **no crea cuentas** (tarea 5.4, doc 07 §6): una contraseña generada por un script acaba en el historial del shell. **No devuelve sesión** porque `admin` es un rol de alcance global y RS-06 le exige segundo factor: el TOTP se da de alta y se confirma con `/auth/2fa/enrol` y `/auth/2fa/confirm`, que ya existen y son los únicos por los que sale un secreto TOTP. **Cuenta también las cuentas desactivadas** porque, si no, dar de baja a la única persona con acceso reabriría la creación pública de un administrador. **El rol no viaja en el cuerpo**: dejar elegir permitiría crear un `auditor` como primera cuenta y dejar la instalación sin nadie capaz de configurarla |
| `POST /api/v1/setup/site` | Alta del **único** centro, `201` con el esquema `Site`, `409` si ya existe. **Sin `compliance_profile_id`**: el centro nace sin perfil asignado, que es como se dice «usa el de la instalación» | ADR-040, RN-05. `PATCH /api/v1/site` sigue sin tener alta ni lista y `POST /api/v1/site` sigue respondiendo `405`. **`timezone` es obligatorio y sin valor por defecto en el servidor** aunque la columna lo tenga: es el dato del que depende RN-05, y un campo omitible aquí es un turno de noche mal atribuido dentro de seis meses. **Sin perfil en el cuerpo** porque con un centro por instalación hay exactamente un perfil vigente, que `GET /compliance-profile` resuelve por `is_default`: un campo para elegirlo sería un parámetro que ningún cliente necesita distinto |
| `GET /api/v1/setup/steps` | Los ocho pasos con su estado **y `completed_at`**. **Autenticada**, al contrario que `GET /setup/status`, que es pública y devuelve **solo `available`** | RF-PD-03, tarea 5.5. **La lista de pasos es un inventario de la postura de la instalación**: dice si hay un administrador **sin segundo factor confirmado**, si no se ha activado licencia y si no hay ningún quiosco vinculado; `completed_at` dice además desde cuándo está montado el servidor (minimización, corrección del cierre de la Fase 5). Ninguna de las cuatro cosas hace falta para lo único que el panel decide sin credenciales —si lleva al asistente o a la pantalla de acceso— y todas son útiles para quien está mirando si merece la pena insistir. **Dos rutas y no una que cambie de forma según quién pregunte**: una respuesta cuyo cuerpo depende de las credenciales es un contrato que un cliente generado no sabe expresar. Con el asistente cerrado, `steps` viaja **vacío** |
| `PUT /api/v1/setup/steps/{step}` y `POST /api/v1/setup/complete` | Marcan un paso como `completed` o `skipped` y cierran el asistente. **`administrator` y `site` no se marcan**: su estado se deduce del dato (`422`). **El perfil de convenio no admite `skipped`** (`422`). Cerrar exige que ningún paso quede en `pending`, y el `409` **nombra los que faltan** | RF-PD-03, RL-21, regla dura 15. **`PUT` porque es idempotente**: repetirlo deja el mismo estado, y el panel puede reintentar sin pensar cuando se le cae la red a media puesta en marcha. **Lo derivado no se declara a mano** para que ni una fila perdida en una restauración ni un `PUT` mal dirigido puedan hacer que el asistente afirme que hay administrador cuando no lo hay. **La licencia sí se omite** —un asistente que la exigiera para terminar la convertiría en requisito de arranque del registro horario ([ADR-019](adr/ADR-019-la-licencia-nunca-bloquea-el-registro.md))— y **el perfil de convenio no**, porque RL-21 obliga a contrastar los umbrales con el convenio aplicable y eso es un acto de alguien. **El asistente no se reabre**: sería una vía para reconfigurar la instalación —zona horaria incluida— sin el asiento que RL-04 exige, y por eso **el cierre deja su propio asiento** `setup.completed`, con los pasos que se omitieron y sin ningún dato personal. Un acto que se justifica por el *trail* tiene que estar **en** el *trail* |
| `POST /api/v1/employees/import` | **Único endpoint con cuerpo `multipart/form-data`.** Dos fases: `mode: validate` no escribe nada y devuelve el informe línea a línea; `mode: apply` exige `confirm_checksum` con el `sha256` que devolvió la validación y **el fichero se vuelve a subir**. `200` también con líneas rechazadas; `409` si la huella no cuadra; `422` si el fichero no se puede leer o venía truncado. Ámbito `employees:*`, y la policy admite `rrhh` **y `admin`** | RF-GP-05, tarea 5.5. **Multipart y no JSON** porque el fichero se lee en *streaming* desde disco y un base64 en un cuerpo JSON obliga justo a lo contrario. **La confirmación por huella y no un fichero en servidor**: guardar entre las dos fases un fichero con los nombres y los documentos de identidad de la plantilla es superficie de datos personales en reposo —con su borrado, su cuota y su fuga— y además así se garantiza que se aplica **exactamente lo que se revisó**. **`200` con rechazos** porque el informe es el resultado esperado del endpoint, no un error de la petición. **`admin` además de `rrhh`**, al contrario de lo que la línea del anexo sugiere: la importación es el paso de plantilla del asistente, y en ese momento la única cuenta que existe es el primer administrador — con `rrhh` a secas ese paso sería inalcanzable el día de la instalación |
| `POST /api/v1/employees/import` (reglas del fichero) | La identidad de una línea es `national_id` y, si no hay, `email`; sin ninguno de los dos se rechaza. **`employee_code` no se lee del fichero.** **`hired_at` de quien ya existe no se modifica** y se avisa. El delimitador y la codificación **se detectan**; el mapa de columnas es configuración (`WORKFORCE_IMPORT_COLUMN_ALIASES`) | Reglas duras 5, 12 y 13; RL-08. **Documento o correo** es lo que permite que reimportar el mismo fichero no duplique a nadie; no contradice la regla dura 12 —el fichero puede no traer columna de correo— porque lo que no pueden faltar son **los dos**. **El código lo genera el servidor** porque es opaco (§5.5): uno tomado del sistema anterior sería un dato con significado impreso en una tarjeta. **La fecha de alta no se reescribe** porque mueve el punto desde el que corre la conservación de RL-02. **Detección y no configuración** del delimitador y la codificación porque son las dos causas número uno de incidencia con un CSV exportado por un Excel español, y un parámetro más sería uno que alguien tiene que acertar a ciegas |
| `site.created` y `site.updated` en `audit_log` | **Acciones nuevas del catálogo.** El alta del centro y cualquier cambio de su nombre o su zona horaria dejan asiento, en la familia «cambia roles, permisos o parámetros del cálculo» | Regla dura 6, RN-05. **Se audita por `timezone`**: es el parámetro con el que se decide a qué día van las horas de toda la plantilla, y cambiarlo las mueve de un día a otro sin tocar un solo fichaje. **`site.updated` cierra una deuda de la tarea 1.6**: `UpdateSiteHandler` afirmaba por escrito que el cambio quedaba auditado y no publicaba ningún evento — no había ni oyente ni asiento |
| `employee.imported` en `audit_log` | **Acción nueva.** Asiento **del lote**, con las cifras y la huella del fichero; **nunca nombres, correos, documentos ni el nombre del fichero**. Familia de acceso a datos personales | Reglas duras 6 y 21. Cada alta deja además su propio rastro; lo que este asiento responde es «¿quién cargó la plantilla, cuándo y con qué fichero?», que es lo que se pregunta cuando aparecen doce altas que nadie recuerda haber hecho. **El nombre del fichero no se audita** porque lo pone quien sube y puede llevar dentro el de una persona |
| `system.updated` y `system.restored_from_backup` en `audit_log` | **Dos acciones nuevas del catálogo** y una familia nueva, «ciclo de vida de la instalación». La actualización asistida y su vuelta atrás automática dejan asiento, con actor `system`, sujeto `installation` y **payload de lista cerrada**: versiones, recuento de migraciones, huella de la copia, huellas de la cadena antes y después (en la vuelta atrás, `chain_before` es **la punta de la cadena que se descarta**, leída justo antes de restaurar: no encaja con el `prev_hash` de la fila, y esa discrepancia es la prueba), y —en la vuelta atrás— nombre del fichero de copia (**nunca su ruta**), momento en que se tomó, paso que falló y motivo **en código, no en texto libre**. Los escribe el instalador con `php artisan compliance:record-system-event` | RF-PD-10, RL-04, RS-07, reglas duras 6 y 21. **Sin ellas el registro afirma una continuidad que no existió**: tras una vuelta atrás la cadena que queda es la de la copia, es íntegra, `compliance:verify-audit-chain` sale en verde y el intervalo descartado —que puede contener fichajes reales— no deja hueco visible. Por eso `system.restored_from_backup` **se escribe sobre la cadena ya restaurada**, no antes: su `prev_hash` es el último hecho que sobrevivió, y escribirlo antes de restaurar habría sido escribirlo en la cadena que se tira. **Ningún actor humano puede firmarlas** —lo rechaza el dominio al construir el asiento—: un asiento firmado por una persona afirmaría que esa persona decidió descartar un intervalo del registro. **Familia propia y no `retention.*`**: aquella describe una pérdida planificada, autorizada y sellada (ADR-027); esta, una imprevista. Que la actualización falle en escribir su asiento **no deshace la actualización**: se anota en el informe y se sale con el código que corresponda |
| `GET /api/v1/reports/exports` y `GET /reports/exports/{uuid}/download` | **Dos rutas aditivas** sobre las tres que este anexo ya listaba (tarea 3.9). La lista devuelve las 20 exportaciones más recientes **del solicitante**, sin paginación y sin `DELETE`; la descarga **no lleva cabecera `Authorization`** y se autoriza con un token de un solo uso en `?token=`, con su propia zona de límite de tasa. Solo el solicitante ve y descarga lo suyo, también si es `admin`: cualquier otro recibe `404`, no `403`. `POST /reports/exports` con `kind: payroll` exige rol `rrhh+`, como `GET /reports/payroll-export` | RF-IN-06, RF-IN-07, [ADR-041](adr/ADR-041-descarga-de-ficheros-diferidos-con-enlace-de-un-solo-uso.md). **Una lista hace falta** porque el panel tiene que poder enseñar «lo que pedí» sin guardar identificadores en el navegador, y el sondeo necesita un solo recurso que mirar. **La descarga va sin sesión** porque un enlace que se abre con un clic no lleva cabeceras, y eso es justo lo que obliga a que el token sea corto, de un solo uso y ligado a una fila (ADR-041). **`404` y no `403`** para no confirmar que la exportación de otra persona existe. Aditivas sobre la v1 (ADR-012); la superficie del quiosco no se toca |
