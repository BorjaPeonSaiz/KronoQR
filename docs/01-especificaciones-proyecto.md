...
| `GET /api/v1/setup/steps` | Los ocho pasos con su estado. **Autenticada**, al contrario que `GET /setup/status`, que es pública y devuelve solo `available` y `completed_at` —**sin `steps`**— | RF-PD-03, tarea 5.5. **La lista de pasos es un inventario de la postura de la instalación**: dice si hay un administrador **sin segundo factor confirmado**, si no se ha activado licencia y si no hay ningún quiosco vinculado. Ninguna de las tres hace falta para lo único que el panel decide sin credenciales —si lleva al asistente o a la pantalla de acceso— y las tres son útiles para quien está mirando si merece la pena insistir. **Dos rutas y no una que cambie de forma según quién pregunte**: una respuesta cuyo cuerpo depende de las credenciales es un contrato que un cliente generado no sabe expresar. Con el asistente cerrado, `steps` viaja **vacío** |
