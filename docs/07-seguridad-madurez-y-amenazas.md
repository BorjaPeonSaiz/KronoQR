...
| **Degradación por licencia de la presencia en vivo, sin punto de decisión real** | **Pendiente** (29-08-2026, tarea 2.4): el módulo `Product` todavía no expone el puerto de estado de licencia, así que ADR-023 —«degrada a sondeo, no se apaga»— no se puede aplicar | RF-PD-05, ADR-019, ADR-023 | El punto de extensión existe y es **único**: `Reporting\Http\Support\RealtimeSubscription::isAvailable()`. Cuando la tarea 5.3 cree el puerto, la comprobación entra ahí y en ningún otro sitio; el endpoint de sondeo ya responde `enabled: false` y el panel ya sabe qué hacer con eso. **Ningún `if (license.expired)` fuera de ese método** | 5.3 |
