...
| Requisitos de seguridad | **2** | RS-01..RS-12 (doc 01 §8) y RL-* (§7) etiquetados en pruebas: `QualityGatesTest.php`, `backend/tests/Feature/Identity/PortalPinLockoutTest.php` (RS-12), `backend/tests/Feature/Identity/CredentialStatusDisclosureTest.php` (RS-03); `qa:traceability --check` falla en CI si un requisito implementado no tiene prueba (RQ-13). Requisitos hacia proveedores: no aplica (on-premise, sin subencargados de datos, ADR-016). | **3**: RS-13 para la auditoría de autenticación (§6) y mapa requisito ↔ control ASVS por versión. | 2.1 (RS-06) |
...
| Sin requisito para OWASP A09 (registro y monitorización de fallos de seguridad) | **Propuesta RS-13**: «Toda autenticación deja rastro consultable en los tres canales —gestión, portal y PIN de quiosco—: el acceso, el cierre de sesión y la apertura de un bloqueo dejan asiento en `audit_log`; cada fallo deja apunte estructurado en el log técnico y alimenta `kronoqr_auth_attempts_total`, sobre la que se declaran las alertas de fuerza bruta.» El reparto no es cosmético y ya no vive solo en los docblocks: lo fija **ADR-039**, que además saca el asiento del bloqueo del camino de la respuesta —auditar cada fallo, o auditarlo en línea, metería un ataque de fuerza bruta dentro del candado global de ADR-010, por el que pasa cada fichaje—. Añadirlo a doc 01 §8 exige su prueba y su fila en el Anexo A; **no se hace desde esta rama** y queda a decisión del usuario | A09; `T1110.001` | Implementado (§5); el requisito lo añade quien lo decida | Cierre de esta rama |
