...
KQ_MSG_ES[u_f_verify_doctor]="product:doctor ha encontrado al menos un fallo en la version nueva (RF-PD-13). A diferencia de install.sh, aqui TODA verificacion fallida deshace: se restaura la copia y se relanza la version anterior. El detalle completo de product:doctor esta en el fichero de detalle del informe."
...
KQ_MSG_ES[u_f_verify_doctor_unexpected]="product:doctor ha terminado de forma inesperada (codigo %s) en la version nueva. Se deshace; el detalle completo esta en el fichero de detalle del informe."
...
KQ_MSG_EN[u_f_verify_doctor]="product:doctor found at least one failure in the new version (RF-PD-13). Unlike install.sh, HERE every failed verification rolls back: the backup is restored and the previous version relaunched. The full product:doctor output is in the report's detail file."
...
KQ_MSG_EN[u_f_verify_doctor_unexpected]="product:doctor ended unexpectedly (exit code %s) on the new version. Rolling back; the full detail is in the report's detail file."
