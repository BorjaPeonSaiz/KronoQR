...
KQ_MSG_ES[u_verify_doctor_ok]="Diagnostico completo (product:doctor) sin fallos ni avisos"
KQ_MSG_ES[u_verify_doctor_warn]="Diagnostico completo (product:doctor) con avisos"
KQ_MSG_ES[u_verify_doctor_warn_fix]="ningun aviso deshace la actualizacion, pero conviene leerlos en el detalle tecnico del informe: son la primera senal de un problema que hoy no bloquea y manana si."
KQ_MSG_ES[u_verify_doctor_failed_warn]="Diagnostico completo (product:doctor) termino con codigo %s"
KQ_MSG_ES[u_verify_doctor_failed_fix]="no detiene la actualizacion: la version nueva ya esta verificada por las sondas, la cadena de auditoria, las restricciones RN-01/RN-02 y los privilegios de base de datos, y buena parte de los fallos de product:doctor son ambientales (disco por debajo del umbral, certificado a punto de caducar, APP_DEBUG en produccion), no del esquema ni del codigo desplegado. Revisa el informe completo en el fichero de detalle y repite './doctor.sh' cuando quieras."
KQ_MSG_ES[u_f_verify_doctor_missing_command]="la imagen de la version nueva no incluye 'product:doctor'. Es un defecto de esa version, no de esta instalacion: se deshace y hay que avisar al fabricante con este informe."
...
KQ_MSG_ES[u_report_warned]="correcto, con avisos"
KQ_MSG_ES[u_report_doctor_failed]="con fallos (informativo, no bloquea)"
...
KQ_MSG_ES[u_rollback_audit_entry_skipped]="Asiento 'system.restored_from_backup' NO escrito: la version restaurada (%s) no conoce esa accion y su verificacion nocturna de la cadena fallaria con ella. Los datos del asiento estan en el detalle tecnico de este informe: escribelo con 'compliance:record-system-event' tras la proxima actualizacion."
KQ_MSG_ES[u_rollback_audit_entry_unknown]="Asiento 'system.restored_from_backup' NO escrito: no se ha podido preguntar a la version restaurada (%s) si conoce esa accion. La vuelta atras YA ESTA HECHA y verificada. Los datos del asiento estan en el detalle tecnico de este informe, junto con el error de 'artisan list': comprueba con './doctor.sh' que la instalacion responde y escribe el asiento a mano con 'compliance:record-system-event'."
...
KQ_MSG_EN[u_verify_doctor_ok]="Full diagnostic (product:doctor) with no failures nor warnings"
KQ_MSG_EN[u_verify_doctor_warn]="Full diagnostic (product:doctor) with warnings"
KQ_MSG_EN[u_verify_doctor_warn_fix]="no warning rolls the update back, but they are worth reading in the report's technical detail: they are the first sign of a problem that does not block today and will tomorrow."
KQ_MSG_EN[u_verify_doctor_failed_warn]="Full diagnostic (product:doctor) ended with code %s"
KQ_MSG_EN[u_verify_doctor_failed_fix]="it does not stop the update: the new version is already verified by the probes, the audit chain, the RN-01/RN-02 constraints and the database privileges, and a good part of product:doctor's failures are environmental (disk below the threshold, a certificate about to expire, APP_DEBUG in production), not about the schema or the code just deployed. Check the full report in the detail file and run './doctor.sh' again whenever you like."
KQ_MSG_EN[u_f_verify_doctor_missing_command]="the new version's image does not include 'product:doctor'. It is a defect of that version, not of this installation: rolling back and telling the manufacturer, attaching this report."
...
KQ_MSG_EN[u_report_warned]="ok, with warnings"
KQ_MSG_EN[u_report_doctor_failed]="with failures (informational, non-blocking)"
...
KQ_MSG_EN[u_rollback_audit_entry_skipped]="'system.restored_from_backup' entry NOT written: the restored version (%s) does not know that action and its nightly chain verification would fail on it. The entry data is in the technical detail of this report: write it with 'compliance:record-system-event' after the next update."
KQ_MSG_EN[u_rollback_audit_entry_unknown]="'system.restored_from_backup' entry NOT written: the restored version (%s) could not be asked whether it knows that action. The rollback is ALREADY DONE and verified. The entry data is in the technical detail of this report, together with the 'artisan list' error: check with './doctor.sh' that the installation responds and write the entry by hand with 'compliance:record-system-event'."
