...
export interface paths {
    ...
    "/api/v1/scan": {
         ...
         *
         *     **Que cubre exactamente la garantia de tiempo constante.** Los rechazos **de
         *     credencial**, que son los cinco de arriba: entre ellos no hay diferencia
         *     medible, porque es ahi donde una diferencia diria si un codigo existe o esta
         *     revocado. El rechazo de RN-18 —el fichaje que no puede cuadrar con el
         *     registro— tambien responde este mismo `422`, pero llega **despues** de
         *     resolver la credencial y de cargar la jornada, asi que su latencia se parece
         *     a la del camino aceptado y no a la de los otros rechazos. No abre ningun
         *     oraculo: para provocarlo hacen falta un token de quiosco valido, una
         *     credencial que resuelve y un `occurred_at` fabricado, y lo unico que se
         *     averigua es sobre la jornada de la persona **cuya tarjeta se tiene en la
         *     mano**.
         *
    ...
    "/api/v1/scan/batch": {
         ...
         *
         *     **Un elemento que no puede cuadrar se resuelve de una vez** (RN-18, *fichaje
         *     irreconciliable*). Una cola desordenada o dos tablets con relojes distintos
         *     traen escaneos que ya no caben, y hay **dos formas** de no caber:
         *
         *     - **Al cerrar**: la hora real **no es posterior** a la entrada del turno que
         *       esa persona tiene abierto. RN-03 exige salida estrictamente posterior.
         *     - **Al abrir**: el tramo que la entrada crearia —que nace sin salida, asi que
         *       se extiende hacia delante— **pisaria un tramo ya cerrado** de esa persona,
         *       en cualquier jornada. Lo prohibe RN-02. Es el caso de la tablet que encola
         *       la entrada de las 08:00 mientras la persona ficha de 09:00 a 13:00 en otro
         *       quiosco, y tambien el del turno de noche del dia anterior, que pertenece a
         *       la jornada en que empezo (RN-05) y sigue vivo de madrugada.
         *
         *     Ninguno de los dos puede producir tramo y reintentarlos no lo va a arreglar:
         *     se registran en el log con resultado propio y marcados para revision, su
         *     elemento responde el `422` generico de siempre, y la revision diaria abre una
         *     incidencia `out_of_order_scan` para que una persona lo cuadre con una
         *     correccion (RN-13, RF-PA-04). **El resto del lote se procesa con normalidad y
         *     lo que ya estaba registrado no se toca.** Para el quiosco no hay nada nuevo
         *     que programar: ya descarta lo que responde `422`.
         *
    ...
    "/api/v1/scan/pin": {
         ...
         *     registro— viaja en el mismo `422` pero se decide despues de identificar a la
         *     persona y de cargar su jornada, asi que tarda lo que el camino aceptado.
         *     Exige un PIN correcto, de modo que no dice nada de nadie mas.
         *
    ...
    "/api/v1/credentials/{uuid}/print": {
         ...
         *
         *     **El QR se genera con correccion de errores nivel Q y tamaño minimo
         *     garantizado** (RF-QR-05). El nivel es una propiedad del simbolo, no una
         *     garantia de durabilidad: **el producto no promete ninguna tolerancia al
         *     desgaste**, porque el soporte fisico lo elige el cliente. Una tarjeta que
         *     deja de leerse se repone —revocar, reemitir e imprimir en el dia— y hasta
         *     entonces se ficha con el PIN de respaldo (RF-AT-11).
         *
    ...
    "/api/v1/setup/status": {
         ...
         *
         *     **Y no revela nada mas: la respuesta trae `available` y nada mas.** El
         *     detalle —que pasos estan hechos y cuales no, y **cuando** se cerro la
         *     puesta en marcha— vive en `GET /api/v1/setup/steps`, que exige sesion de
         *     administrador. Esa lista dice si hay un administrador **sin segundo
         *     factor**, si no se ha activado licencia y si no hay ningun quiosco
         *     vinculado, y `completed_at` dice desde cuando esta montado el servidor:
         *     informacion util para quien mira si merece la pena insistir, e
         *     innecesaria para redirigir un navegador.
         *
...
export interface components {
    schemas: {
         ...
         *     permite.
         *
         *     **Solo lo transitorio llega aqui.** Un escaneo que no puede cuadrar por mas
         *     veces que se envie **no** es un `503`: se registra, se responde con el `422`
         *     generico y se abre una incidencia para que lo resuelva una persona. Son las
         *     dos formas de RN-18 —una hora que no es posterior a la entrada del turno
         *     abierto (RN-03) y una entrada que pisaria un tramo ya cerrado de esa persona,
         *     en cualquier jornada (RN-02)—, y las dos caian antes en este esquema: el
         *     resultado era una cola que no vaciaba nunca y un fichaje del que no quedaba
         *     ni una linea.
         *
        ...
        ScanBatchEntry: {
             ...
             *
             *     El `422` cubre **dos** situaciones que para el cliente son la misma y
             *     se tratan igual: la credencial no valia (RS-03) y el escaneo **no
             *     puede cuadrar con el registro y queda como incidencia** (RN-18, en sus
             *     dos formas: su hora real no es posterior a la entrada del turno abierto,
             *     o la entrada pisaria un tramo ya cerrado de esa persona en cualquier
         ...
         *     - `out_of_order_scan`: **fichaje irreconciliable** (RN-18). Llego un
         *       escaneo que no podia encajar en el registro y se guardo con
         *       `rejected_out_of_order`, sin tramo. Son dos situaciones con dos limites
         *       opuestos: su hora real **no era posterior** a la entrada del turno
         *       abierto —RN-03 exige salida estrictamente posterior— o la entrada
         *       **pisaba un tramo ya cerrado** de esa persona, en cualquier jornada
         *       —RN-02, con limites `[inicio, fin)`: entrar a la hora exacta en que se
         *       salio si cuadra—. **El fichaje ocurrio**: alguien paso su
         *       tarjeta, y lo que falta es decidir con una correccion (RN-13, RF-PA-04)
         ...
         *     pinta como pares.
         *
         *     **Nunca lleva datos personales** (regla dura 21.) Nacio admitiendo
         *     **solo enteros**, que era la forma mas corta de garantizarlo, y RN-18
         *     lo amplio a **entero o cadena**: la incidencia `out_of_order_scan` lleva
         *     el `scan_id` del primer escaneo que no cuadro y su `occurred_at`, que es
         *     lo unico con lo que una persona encuentra el fichaje en el log para
         *     corregirlo (RN-13). Ninguno de los dos identifica a nadie: uno es un UUID
         *     generado por la tablet y el otro un instante.
         *
         *     **El cambio es aditivo** (ADR-012): todo contexto que ya existia sigue
         *     siendo valido, y un cliente que solo supiera pintar numeros no deja de
         ...
         *
         *     Y es la razon por la que **ni `completed_at` ni `steps` estan en
         *     `required`**. `GET /api/v1/setup/status` es publico —tiene que serlo: se
         *     llama antes de que exista ninguna cuenta— y `available` es lo unico que
         *     el panel necesita para decidir si lleva al asistente o a la pantalla de
         *     acceso. Lo demas se queda fuera por minimizacion.
         *
         *     La lista de pasos es un **inventario de la postura de la instalacion**:
         *     dice si hay un administrador sin segundo factor, si no se ha activado
         *     licencia y si no hay ningun quiosco vinculado. `completed_at` dice
         *     **cuando** se monto el servidor. Ninguna de las dos cosas se regala sin
         *     autenticar, ni antes ni despues de terminar la puesta en marcha.
         *
         *     Con una sesion de administrador —`GET /api/v1/setup/steps`,
         *     `PUT /api/v1/setup/steps/{step}` y el `status` de
         *     `POST /api/v1/setup/complete`— viajan las tres.
         *
         *     **El panel debe tratar `completed_at` y `steps` como opcionales** y no
         *     asumir su presencia: en la primera llamada, antes de crear la cuenta, no
         *     estaran.
         */
        SetupStatus: {
            /**
             ...
             *     **Solo con sesion de administrador**; ausente en la respuesta
             *     publica de `GET /api/v1/setup/status`.
...
export interface operations {
    ...
    recordScan: {
        ...
        responses: {
            ...
            /**
             * @description **Escaneo rechazado.** Respuesta unica y generica para toda causa de
             *     rechazo, y de tiempo constante entre las de credencial (regla dura 17,
             *     RS-03, documento 02 §5.2; el matiz, arriba en la descripcion del
             *     endpoint).
             ...
             *
             *     **Tambien responde asi el fichaje irreconciliable** (RN-18), en sus dos
             *     formas: un escaneo cuya hora real **no es posterior** a la entrada del
             *     turno abierto —no puede cerrarlo (RN-03)— y una entrada que **pisaria un
             *     tramo ya cerrado** de esa persona, en cualquier jornada —el tramo que
             *     abriria nace sin salida y se extiende hacia delante (RN-02)—. Los dos
             *     quedan registrados, marcados para revision y convertidos en incidencia, y
             *     el cliente recibe este mismo cuerpo. **No se distingue desde fuera**, y no
             *     es un descuido: distinguirlo daria un canal por el que averiguar el
             *     estado de la jornada de otra persona pasando su tarjeta.
             */
            422: {
                headers: {
                    [name: string]: unknown;
