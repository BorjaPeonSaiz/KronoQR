...
//  3. Los PDF de credenciales son instrumentos al portador: se descargan por
//     `requestBlob`, que no los cachea ni los deja en ningun sitio.
import type { Problem, ValidationProblem } from './types'
...
export class ApiError extends Error {
  ...
  /** Clave de i18n del titulo. Los textos viven en `locales/*.json`. */
  get titleKey(): string {
...
export interface RequestOptions {
  ...
  anonymous?: boolean
  /**
   * Tipos aceptados en una descarga binaria. Por defecto, el PDF de una tarjeta.
   *
   * Existe porque la exportacion legal **no es un PDF**: RL-06 exige formato
   * tabular no propietario, asi que viaja como `text/csv`. Pedir un `Accept` que
   * el endpoint no sirve es la forma de recibir un 406 que nadie sabe explicar.
   */
  accept?: string
}
...
async function send(path: string, options: RequestOptions, accept: string): Promise<Response> {
  ...
  try {
    response = await fetch(buildUrl(path, options.query), {
      ...
      headers,
      // Ni el PDF de una tarjeta ni una lista nominal de la plantilla tienen por
      // que quedarse en la cache del navegador.
      cache: 'no-store',
      credentials: 'omit',
...
export interface BinaryDocument {
  blob: Blob
  filename: string
  /** Cabecera `X-Kronoqr-Printed-Count`: cuantas tarjetas lleva el PDF. */
   ...
   * `audit_log`, y sin ellas el panel no puede decir cuanto entrego. Se exponen
   * en crudo en vez de añadir un campo por descarga: este tipo lo comparten
   * todas, y una lista de contadores especificos crece con cada informe.
   */
  headers?: Headers
}
 ...
 * lotes NO es un error: es la forma que toma su idempotencia (ADR-034).
 */
export async function requestBlob(
  ...
  const response = await send(
    path,
    options,
    options.accept ?? 'application/pdf, application/problem+json',
  )
