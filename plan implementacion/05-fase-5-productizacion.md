...
infra/versions.txt                   # matriz de versiones publicadas y frontera de migraciones de cada una (versions.txt del paquete)
infra/scripts/package.sh             # arma el paquete de entrega; lo usan los dos jobs de la etapa 8
infra/scripts/lib/checks.sh          # comprobaciones comunes a install.sh y update.sh
infra/scripts/lib/messages-update.sh # catalogo bilingue del actualizador
...
```
**Decisiones tomadas al ejecutar la tarea (07-09-2026).** Las que los documentos dejaban abiertas o que la implementación obligó a fijar; cada una está escrita también donde la lee quien la necesita (cabecera de `update.sh`, runbook, `versions.txt`):

1. **El punto de control entre versiones (punto 11) es una marca de versión persistida, no una copia por versión.** Cada versión intermedia aplica sus migraciones en un lote propio de la tabla `migrations` y deja una línea `PUNTO DE CONTROL <versión>: N migraciones, S s, lote B` en pantalla y en el informe. Sirve para saber **exactamente** dónde se paró; **no** pretende deshacer solo esa versión. La vuelta atrás es siempre **restaurar la copia verificada del paso 3** con `restore.sh` y relanzar la versión anterior: el `down()` de una migración de contracción no devuelve los datos que quitó, y la restauración es lo único que se ensaya cada trimestre. `migrate:rollback` queda para el desarrollo (y se prueba entero en `MigrationsRoundTripTest`), no para el servidor de un cliente.
2. **La matriz de versiones soportadas es un fichero del paquete, `versions.txt`** (`infra/versions.txt` en el repositorio): una línea por versión publicada con la **última migración que incluye** (`-` si no añadió ninguna; `*` solo en la última línea, la versión en desarrollo). De ahí salen la ventana «vigente y dos anteriores», la cadena de intermedias, a qué versión ir primero y **qué migraciones pertenecen a cada versión** (`migrate --path` con la lista de cada tramo). Lo edita quien publica, en el mismo `chore(release)` que sube `VERSION`; `QualityGatesTest` verifica su contrato y `update.sh` se niega a actualizar hacia una versión que no esté listada.
3. **El mantenimiento va antes de la copia**, y no al revés como enumera el §11.6.4. Con la copia primero, un fichaje aceptado entre la copia y el mantenimiento existiría en la base pero no en la copia, y el quiosco ya lo habría sacado de su cola porque el servidor lo confirmó: una vuelta atrás lo perdería. Con el mantenimiento primero, todo lo que ocurre en la ventana sigue en las colas y entra después, gane o pierda la actualización. Es la única desviación del orden literal y está explicada en el runbook y en doc 08 §2.2.
4. **La versión nueva no recibe tráfico hasta estar verificada.** Se arranca con `--scale nginx=0 --scale horizon=0 --scale scheduler=0 --scale reverb=0`, se sonda por FastCGI desde dentro del contenedor (`cgi-fcgi`, que ya usa la sonda de Docker), se verifican versión, sondas, cadena de auditoría y restricciones de RN-01/RN-02, y solo entonces arranca el borde. Un fichaje aceptado por una versión que después se deshace sería el mismo fallo del punto 3 por otro camino.
5. **Los códigos de salida (punto 6) son los de la tabla común fijada en la 5.4**, sin excepción. `update.sh` **nunca sale con `6`**: toda verificación fallida deshace (RF-PD-10), y el `2` cubre también «copia previa fallida» porque la instalación no se ha tocado (el mantenimiento se retira antes de salir).
6. **Las dos sondas quedan fuera del modo mantenimiento** (`preventRequestsDuringMaintenance(except: ['api/v1/health', 'api/v1/ready'])`) y el 503 de mantenimiento es `problem+json` con tipo propio `urn:kronoqr:problem:maintenance` y `Retry-After`. Sin lo primero, Docker reiniciaría un contenedor sano; sin lo segundo, el 503 saldría como el JSON por defecto de Laravel.
7. **`VERSION` sube a 2.1.0 con esta tarea** y la etapa ⑧b de la CI actualiza **desde el `main` previo** (con versión sintética `X.Y.(Z+1)-ci` cuando `VERSION` no ha cambiado) en lugar de desde una etiqueta: la etiqueta `v2.0.0` es anterior al instalador (5.4), así que hoy no existe ninguna versión etiquetada desde la que se pueda actualizar de verdad. **Decidido (07-09-2026): no se corta ninguna etiqueta 2.0.x; la primera versión instalable y publicada es `v2.1.0`.** La línea 2.0.0 de `versions.txt` es solo la frontera de migraciones de la que parte la 2.1.0, no una versión soportada como origen de clientes. La «actualización desde la versión anterior publicada» por etiquetas del §11.6.5 empieza a existir de verdad con el salto 2.1.0 → 2.2.0; hasta entonces RQ-11 se cubre con el `main` previo, que ejercita el mismo actualizador.
8. **El instalador y el actualizador comparten `lib/checks.sh`** (contadores, Docker, herramientas), `kq_env_set` de `lib/env-file.sh`, y la lista del paquete vive en `infra/scripts/package.sh`, que usan los dos jobs de la etapa ⑧. Ninguna copia inline queda en `ci.yml`.
...
**Decisiones tomadas al ejecutar la tarea (07-09-2026).** Las que los documentos dejaban abiertas o que la implementación obligó a fijar; cada una está escrita también donde la lee quien la necesita (contrato, `packages/web-kit/src/branding.ts`, `configuracion.md`, doc 06 §7):

1. **Precedencia (punto 1 de «no cubiertos»): manda la base de datos**, como fijó la 5.1. La variable de entorno no es un escalón de la cascada, y `BRANDING_NAME` y `BRANDING_ACCENT_COLOR` **se retiran** del Anexo B: la marca la siembra el asistente de puesta en marcha (5.5) y se edita desde el panel. Quedan `BRANDING_LOGO_ROOT` (directorio del contenedor), `BRANDING_PATH` (montaje) y `PRODUCT_BRANDING_RATE_LIMIT`, que son del despliegue, no de la marca.
2. **El valor por defecto es el producto, también en color.** `BRANDING_ACCENT_COLOR` pasa de `#111827` (gris) a `#b8542a`, el `primary-strong` del doc 06: la tarjeta impresa y el informe sellado llevan de serie el filete del producto. Y **`accent_color` público es `null` mientras la fila sea `product_default`**: las SPA no tocan ningún token y el sistema visual del doc 06 queda intacto; solo con un color configurado se derivan tonos.
3. **Un solo color de acento, tonos derivados por cálculo, contraste avisado.** `web-kit/src/branding.ts` deriva la familia primaria de cada contexto (claro y quiosco; en el quiosco el acento se aclara, como hace la paleta del producto) buscando el primer tono que alcanza el mínimo AA de cada pareja, y `contrastWarnings` enseña las parejas que el acento en bruto no cumple. No se impone: guardar sigue permitido (doc 06 §7).
4. **Endpoint público de marca** (`GET /api/v1/branding`, `security: []`, limitado por IP) con **solo** las cinco claves de presentación resueltas (`BRANDING_*` y `LOCALE_*`): el quiosco y el portal necesitan la marca antes de identificar a nadie. Es la primera lectura pública que sale de `installation_settings`, y por eso la proyección es explícita y cerrada (`additionalProperties: false`) y la prueba comprueba que no filtra ninguna otra clave.
5. **El logotipo es un fichero del servidor del cliente dentro de un directorio de marca montado en solo lectura** (`BRANDING_LOGO_ROOT` ← `BRANDING_PATH`), no una subida por HTTP. `BRANDING_LOGO_PATH` **se valida al guardar** (paso 7): ruta absoluta dentro del directorio (por `realpath`, sin `..`), PNG o SVG **por contenido**, ≤ 512 KiB, ≤ 2048 px de lado, sin `<script` en el SVG; fuera de eso, `422`. La lectura posterior sigue siendo tolerante (5.1): un fichero que desaparece deja los documentos sin logotipo, nunca sin fichar. Sin el cerco, `GET /api/v1/branding/logo` —público— sería una lectura de cualquier fichero del servidor.
6. **`GET /api/v1/branding/logo`** sirve el fichero por contenido (nunca por extensión), con `ETag`, `Cache-Control: public, max-age=31536000, immutable` y `?v=<huella>` en la URL publicada; un SVG viaja con CSP `default-src 'none'; sandbox` y `nosniff`. El quiosco lo guarda en la caché del *service worker* (`CacheFirst`): es la **única** excepción a «la API nunca se cachea», acotada a estas dos rutas de lectura, y existe porque sin ella el quiosco sin red saldría sin logotipo (RF-KI-03).
7. **Idiomas unificados**: `LOCALE_DEFAULT` y `LOCALE_AVAILABLE` gobiernan la negociación de idioma del backend (`NegotiateLocale`) y el selector del quiosco; `APP_LOCALE`/`APP_SUPPORTED_LOCALES` quedan como respaldo si la configuración no se puede leer, y las dos sondas no la consultan. Cierra la nota «la versión de la marca blanca los unifica» de `configuracion.md` §2.3.
8. **Qué no se renombra**: `FH1`, tablas, rutas, comandos y el `name` del manifiesto PWA del quiosco (es de compilación y queda documentado como límite: el nombre con el que se instala la PWA en la tablet sigue siendo el del producto).
9. **La marca propia es una funcionalidad del plan** (`white_label`, ADR-023: «vuelve a la marca por defecto del producto»). `Feature::WhiteLabel` pasa a `implemented()` y el punto único de decisión es el decorador `LicensedBrandingProvider`: sin la funcionalidad en el plan o con la licencia caducada, `GET /api/v1/branding`, la tarjeta, el informe y la exportación legal salen con **el color y el logotipo del producto**; **el nombre de la instalación no se degrada nunca** (revisión de `seguridad-cumplimiento`: la primera línea de la exportación para la Inspección y el sello del informe identifican al obligado, RL-03/RL-06, y no pueden cambiar por un motivo comercial ni por un fallo transitorio de verificación de la licencia). Las filas `BRANDING_*` **se conservan, se siguen editando y auditando** (reglas duras 5 y 15) y vuelven a aplicarse solas al renovar. La pantalla «Marca» del panel lo avisa sin bloquear el guardado. Los idiomas no son marca y no se degradan.
...
**Decisiones tomadas al ejecutar la tarea (08-09-2026).** Las que los documentos dejaban abiertas o que la implementación obligó a fijar; cada una está escrita también donde la lee quien la necesita (contrato, `configuracion.md`, `operacion.md`, runbook `incidencia-sin-acceso.md`, doc 07 §6):

1. **`scope` (punto 12 de «no cubiertos»): tres alcances, y cada uno limita por los dos controles del producto** —ámbitos del token y rol ante las policies—: `diagnostics` (por defecto; `diagnostics:*`; paquete anonimizado y errores), `read_only` (`diagnostics:*`, `attendance:read`, `employees:read`, `audit:read`; actúa como `admin`: lo que impide escribir es el ámbito, no el rol, porque el `auditor` no lee el diario de jornadas a propósito —lo suyo es la exportación legal, que ningún alcance concede—) y `configuration` (`diagnostics:*`, `settings:*`; actúa como `admin`; **no** modifica el perfil de cumplimiento —`ComplianceProfilePolicy` lo cierra a todo actor de soporte: los umbrales legales y la retención los fija el cliente con su asesoría, RL-01/RL-02, regla 14— ni activa licencias —`LicensePolicy`— ni completa el asistente —`SetupPolicy`—). Ninguno lleva `license:*`, `support:*`, `employees:*`, `credentials:*`, `attendance:correct` ni `reports:*`, y **ningún actor de soporte puede incluir datos personales en un paquete** (RL-19): esa decisión es del cliente. Cada puerta cerrada tiene su prueba de `403`.
2. **La concesión es un token de API ligado a la fila de `support_grants`**, no a una cuenta: `support_grants` es el cuarto *tokenable* del producto tras `users`, `devices` y `employees`, con el mismo patrón que el token del quiosco (hash en la fila, revocación por el estado de la fila en `authenticateAccessTokensUsing`). `expires_at` del token es el de la concesión, de modo que la **caducidad es efectiva sin que nadie haga nada**; revocar borra el token y marca `revoked_at` (regla dura 5: la fila se conserva). El token en claro viaja **una sola vez** (respuesta del `POST` y salida de `support:grant`). No existe cuenta del fabricante, ni permanente ni dormida (ADR-020).
3. **Cada uso efectivo se audita, con ventana.** Un middleware sobre el grupo autenticado actualiza `accessed_at` y publica `support_grant.used` como máximo **una vez por concesión cada `PRODUCT_SUPPORT_USE_AUDIT_WINDOW_SECONDS` (900)**: sin ventana, una sesión de soporte de una hora dejaría cientos de asientos y la cadena dejaría de ser legible para el cliente, que es a quien sirve. El asiento lleva la petición que abrió la ventana.
4. **Décima familia de auditoría, `support_access`** (`support_grant.granted|used|revoked`, `diagnostics.bundle_generated`, `diagnostics.personal_data_included`) y **actor nuevo `support_grant`** en `audit_log` (CHECK ampliado por migración *expand*). No cabe en `personal_data_access`: describe la **potestad** concedida y su uso, no un dato consultado, y separarla es lo que permite responder «¿cuándo entró soporte y con qué alcance?» sin recorrer los accesos ordinarios.
5. **Formato del paquete (punto 13): un único JSON legible, sin cifrar**, `kronoqr-diagnostics-<version>-<UTC>.json`, con `manifest` (versión de esquema, versión del producto, instante, `anonymized`, `generated_by`, secciones y `sha256` del resto del documento canónico). Sin cifrado porque la ficha exige que el cliente **inspeccione** el paquete antes de enviarlo y porque una clave compartida con el fabricante sería un secreto más que custodiar; el canal es el del contrato de soporte. `product:diagnostics --verify RUTA` recalcula la huella. Tamaño acotado por `PRODUCT_DIAGNOSTICS_MAX_BYTES` (8 MiB): las colecciones grandes se truncan con `truncated: true` y el recuento total.
6. **Contenido por lista de permitidos, sección a sección** (contrato `DiagnosticsBundle`): `installation`, `configuration` (lista blanca de claves del `.env` —sin `LICENSE_KEY`, `QR_SIGNING_KEY_*`, `BACKUP_ENCRYPTION_KEY`, `REVERB_APP_*`, `APP_KEY`, `APP_URL`, `DB_*`, `REDIS_PASSWORD`, `MAIL_HOST|USERNAME|PASSWORD|FROM_*`, `*_CIDR`, `LOKI_URL`, `OTEL_*`, `GRAFANA_*`, `IDENTITY_PIN_SEALING_SECRET_KEY`, que no aparecen ni «redactadas»—, `invalid_keys` y diferencias `.env`/BD sin valores), `services`, `doctor`, `license` (**sin `customer_name`**: cierra el riesgo pendiente de la 5.3 en doc 07 §6), `kiosks` (**sin `name`**), `error_events` (`not_installed` hasta la 5.12), `metrics` (contadores agregados), `updates` (último `update-*.log` **filtrado por las líneas del formato que escribe `update.sh`**, con recuento de las omitidas; **nunca** el `.detalle.log`) y `audit` (**solo recuentos** por familia y día, y verificación de la cadena; ningún payload). Una prueba de integración con volumen (500 empleados, 90 días) busca nombres, correos, DNI, horas de fichaje y la razón social en el paquete completo.
7. **Datos personales, acción distinta** (RL-19): `include_personal_data: true` / `--with-personal-data`, solo `admin` con cuenta de gestión; añade `personal_data` (plantilla con `uuid`, código, nombre, estado y departamento, **solo de quienes tienen actividad en el periodo o una incidencia abierta** —revisión de `seguridad-cumplimiento`: diagnosticar la nómina de una persona no puede comunicar la plantilla entera—; tramos y fichajes de los últimos `period_days` —7, máximo 31— por `employee_uuid` y sin `client_meta`; incidencias abiertas), pone `anonymized: false` y deja `diagnostics.personal_data_included` además del asiento de generación. El panel avisa con `role="alert"` de qué se incluye y de que queda auditado antes de permitir marcarlo. El fichero que deja `product:diagnostics` en `storage/app/diagnostics` es desechable: el propio comando purga los de más de `PRODUCT_DIAGNOSTICS_RETENTION_DAYS` (7) al arrancar, para que la retención de un fichero con datos personales no dependa de que alguien se acuerde.
8. **`product:doctor`: 0 / 1 / 2** (correcto / solo avisos / al menos un fallo), `--json` con la misma estructura que va en el paquete (`DoctorReport`), `--lang`, textos redactados para quien no conoce el sistema y con **qué hacer** en cada rojo. `install.sh` lo invoca tras las sondas y **solo el 2 se traduce** al `6` de la tabla común; el 1 se muestra y no bloquea. En `update.sh` el doctor es **informativo** (revisión de `revisor-codigo`): su resultado va al informe y a pantalla, pero un `failure` ambiental —disco, certificado caducado, `APP_DEBUG`— no deshace una actualización que las sondas, la cadena, las restricciones y los privilegios ya han verificado; solo un paquete sin el comando aborta. El estado de la licencia **nunca pasa de aviso** (regla dura 15). Con la base de datos caída, lo dice y sigue con el resto.
9. **`doctor.sh`** delega en `product:doctor` cuando la aplicación está en pie y hace desde fuera lo que puede cuando no lo está (daemon, `compose ps`, `.env` en 0600, disco, certificados por `openssl x509 -enddate`, puertos). Códigos: 0, 2 (sin Docker), 3 (no hay instalación) y 6.
10. **`error_events` no se crea aquí.** El puerto `ErrorHistoryArchive` ya lo dice: llega con la 5.12, y el paquete lo declara `not_installed` en lugar de afirmar «cero errores» sobre una tabla que no existe.
...
**Decisiones tomadas al ejecutar la tarea (08-09-2026).** Las que los documentos dejaban abiertas o que la implementación obligó a fijar; cada una está escrita también donde la lee quien la necesita (contrato, `configuracion.md`, `operacion.md`, `obligaciones-legales.md`, doc 02 §7.3 y Anexos B/C, doc 07 §6):

1. **La exportación íntegra es un ZIP con un CSV por tabla, JSON para lo estructurado, `manifest.json` y `README.md`**, `kronoqr-export-<version>-<UTC>.zip`. CSV y no XLSX: XLSX topa en 1 048 576 filas y cuatro años de `scan_events` de 500 personas lo superan; el CSV lo abre cualquier hoja de cálculo y cualquier base de datos. El dialecto es el de `Shared\Infrastructure\Export\CsvDialect` (BOM, RFC 4180, `\r\n`), para que las exportaciones del producto no diverjan. Los instantes van en **UTC ISO-8601** (`UtcInstant`, regla dura 3): es una exportación de datos, no un documento de presentación; el `README` lo dice, indica la zona horaria del centro y explica cómo convertirlos. `manifest.json` lleva versión de esquema, versión del producto, instante, quién la pidió, la zona del centro y, por fichero, el recuento de filas y su `sha256`; el `README` explica **cada fichero y cada columna** en el idioma de la instalación (`lang/{es,en}/data-export.php`), porque una exportación que nadie sabe leer no cumple RL-20.
2. **Alcance «íntegro» por lista de permitidos, tabla a tabla** (`FieldAllowlist`, la misma pieza que el paquete de diagnóstico): `site`, `departments`, `employees` (nombre, código, correo, estado, fechas, departamento, idioma; **sin** `pin_hash`, `national_id_hash` ni `photo_path`), `employment_contracts`, `credentials` (**sin** `secret_hash`), `devices` (**sin** `token_hash`), `shift_entries` (**todas las versiones**, incluidas `superseded` y `voided`, con `version` y `superseded_by_uuid`), `shift_corrections` (autor, `before`/`after`, código y texto del motivo), `daily_totals`, `incidents`, `scan_events` (todos, aceptados y rechazados, con `client_meta`), `audit_log` (completo, con `prev_hash` y `hash`: el cliente puede verificar la cadena fuera del producto, RL-04), `audit_chain_anchors`, `users` (nombre, correo, roles, estado; **sin** contraseña ni secretos de 2FA), `support_grants` (**sin** `token_hash`), `installation_settings`, `compliance_profiles` y `license` (**sin** `signed_key`: la clave es del fabricante y el cliente ya la tiene). **Los identificadores internos (`BIGINT`) no salen**: toda referencia entre ficheros es por `uuid` (`employee_uuid`, `device_uuid`, `shift_entry_uuid`, `user_uuid`), como en toda la API (doc 01 §5.5). Quedan fuera por efímeros o secretos: `personal_access_tokens`, `device_pairing_requests`, `setup_progress`, `failed_jobs`, `migrations`. `absences` y `error_events` no existen todavía: el manifiesto los declara `not_installed`, no vacíos. Una prueba unitaria fija la lista y una de integración con volumen busca en el ZIP entero `pin_hash`, `secret_hash`, `token_hash`, `signed_key` y `password`.
3. **Streaming de verdad**: cada tabla se recorre con un **cursor de servidor** dentro de una transacción (`DECLARE … FETCH FORWARD`, la técnica de `DatabaseLegalExportSource`) y se escribe fila a fila en disco; el ZIP lo cierra `ZipArchive` sobre ficheros ya escritos. La prueba de integración con volumen (500 empleados y 90 días, la misma semilla que el paquete de diagnóstico) mide `memory_get_peak_usage()` y exige que quede por debajo de un techo fijo (128 MiB), no «que no reviente»: una implementación que cargara una tabla en memoria lo superaría ya con esa semilla, y el cursor hace que cuatro años cuesten lo mismo que noventa días en memoria.
4. **Desde el panel la generación es asíncrona; desde la consola, síncrona.** `POST /api/v1/data-export` encola un trabajo y responde `202` con la fila de `data_exports`; `GET /api/v1/data-export` lista las 20 más recientes (estado, tamaño, recuentos, caducidad); `GET /api/v1/data-export/{uuid}/download` entrega el ZIP. Asíncrona porque el volumen que exige la ficha no cabe en los 60 s de `fastcgi_read_timeout` ni en la memoria de un navegador que recibe la respuesta de un `fetch`, y una descarga que se corta a la mitad es peor que ninguna. **Una sola en curso por instalación**: un segundo `POST` mientras hay una `pending` o `running` responde `409` (`urn:kronoqr:problem:data-export-in-progress`); descargar una que no ha terminado, `409` (`urn:kronoqr:problem:data-export-not-ready`); una purgada, fallida o inexistente, `404`. `php artisan product:export-all` la genera en el acto, en primer plano, y **también** registra su fila, de modo que el panel la enseña y la deja descargar. Tabla nueva `data_exports` (`uuid`, `requested_by_user_id` NULL en consola, `requested_via` `panel|console`, `status` `pending|running|completed|failed|purged`, `requested_at`, `started_at`, `completed_at`, `failed_at`, `failure_reason` sin PII, `file_path`, `file_name`, `size_bytes`, `sha256`, `row_counts` JSONB, `expires_at`, `purged_at`, `downloaded_at`, `download_count`). **Las filas no se borran** (regla dura 5): purgar marca `purged_at` y borra solo el fichero.
5. **El fichero vive en `PRODUCT_DATA_EXPORT_PATH`** (`storage/app/exports`, `0700`/`0600`) y **caduca**: `product:export-all --purge` (programado cada hora) borra los ficheros con `expires_at` vencido —`PRODUCT_DATA_EXPORT_RETENTION_DAYS`, 7 de serie— y marca las filas. Igual que el paquete de diagnóstico con datos personales: la retención de una copia completa de la plantilla y del registro en el disco del cliente no puede depender de que alguien se acuerde. `docs/cliente/operacion.md` explica cómo sacar el fichero del contenedor (`docker compose cp`) y que el panel lo descarga entero en memoria del navegador, así que por encima de ~1 GB conviene la consola.
6. **Ámbito `settings:*` y `DataExportPolicy` de solo `admin` que además rechaza a todo actor de soporte** (nota 6 del doc 02 §7.3, mismo criterio que el perfil de cumplimiento y los quioscos): llevarse todos los datos es la potestad de quien responde de la instalación, ningún rol la usaría por separado, y el fabricante **jamás** (regla dura 16): un token de soporte con alcance `configuration` pasa el `ability` y la policy lo cierra con `403`. Cada rol, el quiosco, el portal y los tres alcances de soporte tienen su prueba de `403`. **Las tres rutas nunca se degradan** (ADR-019, regla dura 15): con licencia caducada, ausente o ilegible funcionan igual, y hay prueba. Limitador propio `throttle:data-export` (`PRODUCT_DATA_EXPORT_RATE_LIMIT`, 30/min por cuenta y por IP): el panel sondea la lista cada 5 s mientras dura la generación.
7. **Auditoría en la familia `legal_export`**: sujeto nuevo `data_export`, acciones `data_export.requested` (quién la pidió y por dónde), `data_export.generated` (recuentos por fichero, `sha256`, tamaño, `requested_by`) y `data_export.downloaded` (quién y cuándo se llevó el fichero). Comparten familia con `legal_export.generated` porque responden la misma pregunta —«qué ha salido del registro y quién se lo llevó»— y RL-20 es la garantía de continuidad del mismo registro que RL-06 pone a disposición de la Inspección; no es `personal_data_access` (no es la consulta de la ficha de alguien) ni `support_access` (no sale hacia el fabricante). El actor de `generated` es quien la pidió aunque la escriba el trabajador de cola (el evento lleva `requested_by_user_id`; sin nadie detrás, `system`).
8. **Telemetría (punto 14 de «no cubiertos»): un documento JSON por HTTPS, semanal, a un destino que fija el cliente, con un identificador aleatorio y un contenido que es un subconjunto por lista de permitidos del paquete de diagnóstico anonimizado.** `TELEMETRY_ENABLED=false` de serie (Anexo B) y `TELEMETRY_ENDPOINT` vacío de serie: **hacen falta las dos** y además que la licencia liste `telemetry` en `features` (ADR-023: es accesoria y degradable; `Feature::Telemetry` pasa a `implemented()`). Sin cualquiera de las tres, `product:telemetry --send` no construye ni envía nada y lo dice. El envío es `POST` con `Content-Type: application/json`, TLS verificado, sin redirecciones, 3 s de conexión y 10 s en total, **un reintento** a los 5 s y nada más hasta la semana siguiente; el fallo se anota en el log técnico a nivel `notice` (nunca `error`: no es una avería del registro) y en `storage/app/telemetry/state.json` (`last_attempt_at`, `last_success_at`, `last_failure` solo con la clase de la excepción). Programado `weeklyOn(1, '05:40')` en el planificador, **jamás en el camino de una petición** (paso 7). **`installation_id` es un UUID v4 aleatorio** acuñado la primera vez y guardado en ese mismo fichero: no es `license_id` ni deriva de la razón social; identifica la instalación ante el fabricante sin identificar al cliente, y borrar el fichero estrena identidad. **Contenido, campo a campo** (`TelemetryReport`, lista cerrada atada por prueba a la tabla de `configuracion.md` §3 quinquies): `schema_version`, `installation_id`, `sent_at`, `product` (`version`, `php_version`, `database_version`), `license` (`state`, `plan`, `features`, `days_until_expiry`), `scale` (`employees_active` **por tramos** `1-25|26-100|101-250|251-500|501+`, `devices_active`, `departments`), `usage_7d` (contadores agregados que ya calcula `MetricsCollector`: escaneos aceptados y rechazados, lotes sincronizados, incidencias abiertas, informes generados) y `doctor` (solo `ok|warning|failure` por comprobación, sin mensajes). **Nunca** nombres, correos, códigos, horas, rutas del servidor, `APP_URL`, la razón social, `license_id` ni `uuid` de nadie. `php artisan product:telemetry` imprime el documento que **se enviaría**, para que el cliente decida con él delante; `--send` lo envía. Una prueba de arquitectura fija que el único cliente HTTP saliente de `app/` vive en `Product/Infrastructure/Telemetry` (verificación de ADR-020: ningún otro canal sale hacia el fabricante).
9. **Con la telemetría activada y sin salida a internet no cambia nada visible**: la suite lo prueba con un destino inalcanzable y comprueba que `/scan`, el panel y `doctor` responden igual y que no aparece ningún error para el usuario; `doctor` no añade comprobación de telemetría a propósito (un aviso semanal de «no se pudo enviar» sería justo el recordatorio insistente que RF-PD-12 prohíbe).
10. **En el panel, la exportación vive en la pantalla «Licencia»**, en una sección propia «Tus datos son tuyos» (`DataExportPanel.vue` en `features/settings/`): es donde el cliente mira cuando teme quedarse sin producto, y RL-20 es la respuesta a ese temor. Enseña las exportaciones recientes con su estado, un botón «Generar exportación completa» con aviso `role="alert"` de que el fichero contiene **todos** los datos personales de la plantilla y de que la acción queda auditada, sondeo cada 5 s mientras una está en curso, y descarga cuando termina. No hay pantalla de telemetría: se decide en el `.env` con la lista de campos delante (`configuracion.md`), como el resto de los parámetros de soporte.

11. **Lo que corrigieron las revisiones** (`seguridad-cumplimiento` con `/revision-cumplimiento` y `revisor-codigo`, 08-09-2026; todo aplicado y probado): (a) **una exportación interrumpida a mitad** (contenedor parado, cola reiniciada, fallo al cerrar) dejaba la fila en `running` para siempre y, por el índice único, bloqueaba RL-20 por las dos vías; ahora `GenerateDataExportJob::failed()` marca la fila, `complete()` va dentro del `try` y un ZIP sellado al fallar se borra, y una fila `pending|running` más antigua que `PRODUCT_DATA_EXPORT_STALE_AFTER` (3600 s) pasa a `failed`/`stale` al pedir la siguiente y en la purga horaria; (b) **la transacción de exportación no era una instantánea**: en READ COMMITTED cada cursor toma la suya y los CSV podían tener referencias colgantes entre sí; ahora `within()` fija `REPEATABLE READ` (solo cuando `transactionLevel() === 0`: bajo `RefreshDatabase` la transacción es anidada) y `DataExportSnapshotTest` inserta un tramo desde otra conexión a mitad y comprueba que no aparece en ningún fichero; (c) `failure_reason` es un **código estable** (`write_failed`, `database_error`, `stale`, `unexpected`; enum `DataExportFailure`, CHECK, contrato) y no la clase de la excepción, traducido en el panel; (d) `TELEMETRY_ENDPOINT` **exige `https`** (`TelemetryBlockReason::EndpointNotHttps`; un destino `http://` se rechaza como si no hubiera destino) y `product:telemetry` sin banderas **no persiste** el identificador (`provisional()`); (e) `FileTelemetryStateStore` deja `notice` si no puede guardar; (f) quiosco y portal prueban también la descarga; cubo por IP ×4 del de cuenta; `ByteSize::human()` único en `Shared`; prueba que ata cada columna del catálogo a su descripción en `lang/{es,en}`; prueba de volumen que mide el **incremento** de memoria (48 MiB) y no el pico absoluto; `TELEMETRY_ENDPOINT` **no** viaja en el paquete de diagnóstico, a propósito y documentado en `DiagnosticsConfigurationAllowlist`.
...
| Feature + Contrato | `GET /api/v1/data-export` y `GET /api/v1/data-export/{uuid}/download` contra `openapi.yaml`; `409` con una en curso y con una no terminada; `404` con una purgada; **cada descarga auditada** (`data_export.downloaded`) | `->group('RF-PD-14', 'RS-05')` |
| Autorización negativa | 403 para el quiosco, el portal y **los tres alcances de soporte** (regla dura 16) | `->group('RF-PD-14', 'RF-PD-11')` |
| Integración | La purga por caducidad borra el fichero y marca la fila; **ninguna fila se borra** | `->group('RF-PD-14', 'RN-13')` |
| Unitaria | La lista de campos de `TelemetryReport` coincide, campo a campo, con la tabla de `configuracion.md` §3 quinquies | `->group('RF-PD-12')` |
| Arquitectura | El único cliente HTTP saliente de `app/` es el de la telemetría (verificación de ADR-020) | `->group('RF-PD-12')` |
...
- [x] Pruebas unitarias, de integración con volumen, feature, contrato y autorización negativa.
- [x] **Prueba de que la exportación íntegra funciona con licencia caducada.**
- [x] Exportación auditada.
- [x] `TELEMETRY_ENABLED=false` por defecto y contenido documentado campo a campo.
- [x] Textos en español e inglés.
- [x] `docs/cliente/obligaciones-legales.md` recoge RL-20 y qué hace el cliente con la exportación.
...
**Decisiones tomadas al ejecutar la tarea (09-09-2026).** Las que la ficha dejaba abiertas o que el árbol obligó a fijar; cada una está escrita también donde la lee quien la necesita (contrato, `ClientDocumentationTest`, doc 02 §11.6.1, doc 03 §6.5.8, las propias guías):

1. **La hoja del empleado la produce el producto, no un PDF estático del paquete.** La ficha pide que lleve la dirección del portal, que salga en los idiomas activos de la instalación y que se entregue en el mismo acto que la tarjeta; ninguna de las tres cosas la sabe un fichero de `docs/cliente/` (la dirección es de cada instalación, los idiomas son `LOCALE_AVAILABLE` y la marca es de la 5.8). Endpoint **`GET /api/v1/credentials/instructions-sheet?locale=`** (`[scope: credentials:*]`, `CredentialPolicy::printInstructionsSheet`, mismos roles que `print`): devuelve **un PDF A4 de una sola cara** con la marca de la instalación (`BrandingProvider`/`BrandingLogoReader`, como las tarjetas), la dirección del portal (`APP_URL` + `/portal/`, que es donde lo sirve Nginx), los pasos de fichaje con **pictogramas SVG incrustados** (sin red, ADR-016; no capturas: una pantalla de tablet en una hoja de una cara no se lee y caduca con cada versión), y una línea en blanco «Persona de contacto» que RRHH rellena a mano (no hay un ajuste nuevo para eso). Es `GET` porque, a diferencia de `print`, no cambia el estado de nada; `Cache-Control: no-store` igualmente porque lleva la marca del momento. `locale` fuera de `LOCALE_AVAILABLE` → `422`; sin `locale` → `LOCALE_DEFAULT`. Los textos viven en **`lang/{es,en}/instructions-sheet.php`** (array plano `clave => frase`, marcadores `:app_name` y `:portal_url`), y el PDF se renderiza con `spatie/laravel-pdf` sobre Browsershot por un puerto `InstructionsSheetRenderer` con adaptador y doble de pruebas, siguiendo `BrowsershotCardRenderer`/`FakeCardRenderer`. **Una cara** lo verifica una prueba de integración que renderiza con Chromium real y cuenta las páginas del PDF. Sin comando de consola: la hoja la imprime RRHH desde el panel, no IT desde el servidor.
2. **`docs/cliente/hoja-empleado.md` es la guía de RRHH sobre la hoja, y lleva el texto literal.** Dice qué es, cuándo se entrega (con la tarjeta y el PIN, un solo acto: tarea 1.10, paso 4), cómo se imprime desde el tablero de credenciales, y reproduce **el texto de la hoja en ese idioma** para que RRHH sepa qué está entregando sin abrir el PDF. `ClientDocumentationTest` ata cada frase de `lang/<idioma>/instructions-sheet.php` (sin los marcadores) a su guía: si cambia el producto y no la guía, la CI cae. Las capturas del quiosco que ilustran las confirmaciones (`quiosco-fichaje-confirmado.png`, `quiosco-fichaje-pendiente.png`, `quiosco-pin-respaldo.png`) van en esta guía y en la de RRHH, no en la hoja.
3. **Nombres y pares en inglés**, como en la 5.11: `guia-rrhh.md` ↔ `en/hr-guide.md`, `guia-portal-empleado.md` ↔ `en/employee-portal-guide.md`, `hoja-empleado.md` ↔ `en/employee-sheet.md`. El dataset de `ClientDocumentationTest` pasa de cinco a ocho pares y las pruebas de la 5.11 (par existente, mismos comandos, mismos apartados, imágenes que viajan, sin secretos, enlaces que resuelven) se aplican solas.
4. **Capturas sobre los dobles, como la 5.11, y con nombres pactados aquí** para que quien escribe la guía y quien genera la imagen no tengan que verse: panel en `frontend-admin/tests/screenshots/hr-guide.screenshots.ts` sobre `stubManagementApi` (Hotel Marina, Youssef Amrani, Recepción; 1366×768; es/en): `rrhh-01-empleados`, `rrhh-02-alta-empleado`, `rrhh-03-pin-una-vez`, `rrhh-04-ficha-empleado`, `rrhh-05-credenciales`, `rrhh-06-presencia`, `rrhh-07-jornada`, `rrhh-08-correccion`, `rrhh-09-historial-correcciones`, `rrhh-10-incidencias`, `rrhh-11-resolver-incidencia`, `rrhh-12-informe-periodo`, `rrhh-13-exportacion-legal`, `rrhh-14-perfil-cumplimiento` (`.png`, en `docs/cliente/img/<idioma>/`). **El portal no tenía Playwright ni E2E** (solo unitarias): gana `playwright.screenshots.config.ts`, el doble `tests/e2e/support/portal.ts` (nace aquí y queda para un E2E futuro) y `tests/screenshots/portal.screenshots.ts`, a **412×915** porque el empleado lo abre desde el teléfono: `portal-01-acceso`, `portal-02-jornadas`, `portal-03-correccion-visible`, `portal-04-descarga`. Añadir `@playwright/test` al portal toca el `package-lock.json` raíz: **se regenera desde Linux sin `node_modules`** (trampa de `HANDOFF.md`). Quiosco: `tests/screenshots/sheet.screenshots.ts` con las tres confirmaciones de la decisión 2 (1280×800). Mínimos atados por prueba: guía de RRHH ≥ 12 capturas, portal ≥ 4, hoja ≥ 2, en las dos lenguas.
5. **Lenguaje de negocio, verificado.** Ninguna de las seis guías contiene identificadores del código (`WorkDay`, `ShiftEntry`, `ScanEvent`, `daily_totals`, `audit_log`, `occurred_at`, `recorded_at`, `employee_uuid`), y la guía de RRHH nombra **los nueve motivos de corrección con la etiqueta que enseña el panel** (`corrections.reasons.*` de `es.json`/`en.json`, contrastados por prueba, con un ejemplo de cada uno). El glosario es el §1 de la guía de RRHH; las otras dos remiten a él.
6. **Coherencia de producto, verificada por frases prohibidas**, no por palabras sueltas («móvil» es legítimo: el portal se abre desde el móvil): `biometr`, `huella dactilar`/`fingerprint`, `credencial en el móvil`/`tarjeta en el móvil`/`QR en el móvil` y sus equivalentes ingleses, y `PIN … por correo`/`PIN … by email`. La guía del portal dice por qué no hay recuperación por correo (ADR-015) y qué hacer: pedir el restablecimiento a RRHH, que lo entrega en mano.
7. **Los runbooks que citan las fichas 1.10 y 1.13 no existen** (`alta-nuevo-empleado.md`, `tarjeta-perdida-o-rota.md`; el contrato de `print` cita el segundo). El alta completa —ficha, contrato, tarjeta, PIN y hoja en un solo acto— **es el §2 de `guia-rrhh.md`** y no se duplica en un runbook: los runbooks son del IT (§12) y el alta es de RRHH. `tarjeta-perdida-o-rota.md` **sí se escribe** (revocar → reemitir → imprimir → entregar con hoja nueva, y el caso «impresión fallida» de ADR-034), porque lo cita el contrato y lo necesita quien atiende la incidencia; la guía de RRHH §8 lo enlaza. `docs/runbooks/README.md` lo indexa.
8. **Referencia cruzada única con la 5.11 y con los runbooks.** La guía de RRHH no explica copias, actualizaciones, diagnóstico ni purga: enlaza `operacion.md` y `obligaciones-legales.md`. «Llega un requerimiento de Inspección» remite a `requerimiento-inspeccion.md`; «un empleado pide su registro» a `solicitud-derechos-rgpd.md` (y al portal, que es la vía ordinaria de RL-05); «una tarjeta se pierde» a `tarjeta-perdida-o-rota.md`. Desde dónde se entra al portal (`PORTAL_INTERNAL_CIDR`) se explica en `configuracion.md`; la guía del portal lo enlaza.
9. **Ausencias y lo que aún no existe.** La ficha pide documentar «ausencias (3.10)» y el perfil de cumplimiento (5.2). La 3.10 es de la Fase 3, que en el orden real (0 → 1 → 2 → 5 → 3 → 4) va **después**: la guía no documenta ausencias ni promete nada de ellas; **se añade al cerrar la 3.10** (anotado en su ficha del plan 06 y en `HANDOFF.md`). El perfil de cumplimiento sí, con la advertencia de la ficha.
10. **El acto de entrega recuerda las tres cosas.** El diálogo de «Registrar entrega» del tablero de credenciales dice en pantalla que se entregan la tarjeta, el PIN y la hoja (i18n es/en), y el tablero tiene el botón «Hoja de instrucciones» con un idioma por cada valor de `LOCALE_AVAILABLE`. Es la «referencia desde la tarea 1.10» de la DoD, hecha producto y no solo texto.
11. **La validación por lectura** (cuatro recorridos por alguien ajeno siguiendo solo las guías) no la sustituye ningún script y **queda pendiente del usuario**, como la instalación limpia de la 5.11.
12. **Doc 05 no cambia.** Su §10.8 promete cuatro manuales para IT; estos tres son adicionales y no lo contradicen (la propia ficha lo dice). El doc 02 §11.6.1 añade los tres al árbol del paquete.
13. **La guía destapó que el panel no podía corregir (RF-PA-04), y se cierra aquí.** El contrato tiene `addShiftEntry`, `correctShiftEntry` y `voidShiftEntry` desde la 1.15 y el backend los sirve con sus nueve motivos, pero ningún componente de `frontend-admin` los llamaba: `features/workdays/` era de solo lectura y RRHH no tenía forma de añadir, corregir ni anular un tramo sin la API. Una guía que explica «cuándo corregir» sobre un botón que no existe es peor que ninguna guía, y el doc 05 (§«Detalle de jornada», «Exportación para Inspección») da las correcciones por hechas. Se construye en esta tarea: `CorrectionDialog` con los tres modos, motivo del catálogo con su pista, texto libre obligatorio de 20 caracteres en `OTROS`, errores `409`/`422` sin perder el formulario, E2E y la captura `rrhh-08-correccion.png`. Otros dos huecos que la guía encontró se dicen en ella en vez de taparse: **no hay pantalla de contratos** (los endpoints existen; el informe de periodo avisa de «días-persona sin contrato registrado» y el registro legal no se ve afectado) y **la reemisión en un acto (`reissue`) es solo de API** (el camino de RRHH es revocar → emitir → imprimir → entregar). Ambos quedan anotados en «Pendiente» de `HANDOFF.md` como deuda de producto de tareas anteriores. Tres de los ocho tipos de incidencia que enseña el filtro del panel no se abren hoy (pausa no registrada, patrón anómalo, salida sin fichar: productores de la Fase 3); la guía lo dice en una nota.
14. **Lo que corrigieron las revisiones (`revisor-codigo` y `seguridad-cumplimiento`, segunda vuelta con tres agentes).** (a) **Los tres `409` de la corrección de tramos tienen `type` propio en el contrato**: `urn:kronoqr:problem:shift-entry-superseded`, `…:shift-already-open` (RN-01) y `…:overlapping-shift-entry` (RN-02), y el `422` del cambio de jornada `…:correction-would-change-work-date`; el diálogo del panel solo recarga ante el primero y muestra el `detail` de los otros dos sin destruir el formulario —antes anunciaba «este tramo ya no es la versión vigente» para el `409` más probable al añadir un tramo por olvido de salida, que es «turno ya abierto»—. (b) **Un `409` no pierde lo escrito**: el aviso va encima del formulario, no en su lugar. (c) **Vaciar la salida de un tramo cerrado no es un cambio**: el backend no permite retirar una salida (se anula y se da de alta), y el diálogo ya no lo previsualiza como «Sin cerrar» ni lo habilita. (d) El grupo de rutas `/credentials` lleva **`throttle:management`**: la hoja, `print` y `print-batch` lanzan un Chromium por petición y solo los frenaba Nginx por IP (RS-02, regla dura 19). (e) La hora **inexistente** del cambio de horario de verano se rechaza en el diálogo en vez de desplazarse en silencio; la hora repetida de octubre queda fijada por prueba (segunda ocurrencia). (f) `corrections.wouldChangeWorkDate` deja de estar huérfana: el `422` se traduce por `type`. (g) Las frases de la hoja que citan pantallas del quiosco se atan por prueba a `scan.result.*` y `pin.entryButton` de los `locales` del quiosco. (h) La hoja no promete acceso al portal «desde el móvil» sin más: por defecto responde solo dentro de `PORTAL_INTERNAL_CIDR` (RF-ID-08); redacción neutral en `lang/*/instructions-sheet.php` y en las dos guías atadas. (i) Menores: `work_date` obligatoria en el alta, `reason_text` no viaja con un motivo distinto de `OTROS`, `FALLBACK_TIMEZONE` en vez de `''`, nombre del PDF del doble igual al del servidor, criterios del informe traducidos en la captura inglesa.

**Terminado cuando.** Subconjunto aplicable de la DoD del §10.3:

- [ ] Los tres documentos escritos, en español e inglés.
- [ ] Los cuatro recorridos completados por alguien ajeno siguiendo **solo** las guías (decisión 11: pendiente del usuario).
...
**Decisiones tomadas al ejecutar la tarea (09-09-2026).** Las que los documentos dejaban abiertas o que la implementación obligó a fijar; cada una está escrita también donde la lee quien la necesita (contrato, `configuracion.md`, `operacion.md`, runbook `errores-en-el-panel.md`, doc 02 Anexos B/C, doc 07 §6):

1. **Qué es un error y qué no.** Dentro de una petición HTTP, es error lo que **el manejador de excepciones respondería con `5xx`**: el enganche pregunta al propio manejador qué estado daría y descarta todo lo `< 500` (validación, autenticación, autorización, `404`, `429`, los `409`/`422` de las excepciones de dominio traducidas en `bootstrap/app.php`). Fuera de una petición (cola, planificador, consola) todo `Throwable` es error, salvo los internos que Laravel no informa. **Un `409` de «turno ya abierto» no es un error del sistema**: es el sistema funcionando. *(Corregido tras la revisión: la primera versión excluía por espacio de nombres `Domain\Exception`/`Application\Exception`, y 50 de las 96 excepciones de esos espacios no tienen traducción a `4xx`: producían un `500` y quedaban fuera del histórico.)*
2. **Los cuatro orígenes del servidor se captan con un único enganche `reportable` más un contexto de ejecución.** Tres oyentes (`JobProcessing`/`JobProcessed`/`JobFailed`, `ScheduledTaskStarting`/`Finished`, `CommandStarting`/`Finished`) mantienen un contexto «dónde estoy» (`worker` con nombre del *job* y cola, `scheduler` con el comando, `console` con el comando) y, sin ninguno, la petición es `api` con ruta y método. Ningún módulo importa `bootstrap/app.php` (Deptrac): el enganche lo registra `ProductServiceProvider` al resolver `ExceptionHandler`.
3. **Nivel.** `critical` cuando el fallo toca lo que nadie ve o lo que no puede parar: un *job* que **agota sus intentos** (`JobFailed`), cualquier tarea del planificador, cualquier petición a las rutas de fichaje (`/api/v1/scan*`) y los códigos del quiosco que le impiden fichar (`kiosk.camera.*`, `kiosk.scanner.*`, `kiosk.offline.storage_unavailable`, `kiosk.offline.confirm_not_persisted`, `kiosk.roster.decrypt_failed`, `kiosk.pin.seal_failed`). Todo lo demás es `error`. La tabla vive en `ErrorLevel` y el runbook la reproduce. **El nivel de un error de cliente se decide con el origen delante** (`ErrorLevel::forClientCode(ErrorSource, code)`): solo un token de quiosco produce un `critical` de cliente. **El código se valida contra un catálogo cerrado por origen** (`Shared\Domain\ValueObject\ClientErrorCode`, copia literal de los tipos de `errorReporter.ts` y `clientErrors.ts`, atada por prueba de arquitectura): un código fuera del catálogo de su origen es una petición inválida. *(Corregido tras la revisión de seguridad: sin esto, una sesión de portal fabricaba filas `critical` con un código de quiosco y disparaba la alerta al IT del cliente, y un código libre creaba una huella nueva por envío sin techo.)*
4. **La huella es `sha256(source | exception_class | file:line | mensaje normalizado)`**, 64 hexadecimales. La normalización sustituye, en este orden, UUID, instantes ISO-8601 y horas `HH:MM`, direcciones IP, correos, rutas absolutas, cadenas hexadecimales de ocho o más y **todo número** por marcadores (`<uuid>`, `<time>`, `<ip>`, `<email>`, `<path>`, `<hex>`, `<n>`); no toca las palabras. Para los errores de cliente no hay `file:line` (nunca viaja el `stack`, ver `clientErrors.ts`): la huella es `source | código | mensaje normalizado`. La columna `message` guarda el mensaje **saneado** de la primera aparición (no el normalizado, que es ilegible), truncado a 1 000 caracteres; `app_version`, `trace_id`, `device_id` y `employee_uuid` son los de la **última**.
5. **El saneado es de servidor y es de lista de permitidos.** El mensaje pasa por `ErrorMessageSanitizer` (correos → `[email]`, DNI/NIE → `[id]`, teléfonos → `[phone]`, horas y fechas → `[time]`, `Bearer …`, `FH1.…` y `clave=valor` con nombre de secreto → `[secret]`, **todo texto entre comillas simples, dobles o angulares → `'…'`** —es donde las excepciones interpolan valores, y un nombre de persona no se detecta de otra forma—, y **todo lo que siga a `, SQL: ` se corta y el valor de `DETAIL: Key (…)=(…)` se sustituye**). **Las `QueryException`/`PDOException` no usan `getMessage()`**: Laravel interpola los parámetros enlazados sin comillas en el mensaje (`values (Maria, Gonzalez Perez, EMP-0042)`, el bcrypt de un PIN); el enganche compone `SQLSTATE` + mensaje del driver + `getSql()` con sus `?`, que conserva el valor diagnóstico sin el dato. `context` solo admite claves de una lista cerrada, **derivada de lo que los clientes y el servidor emiten de verdad** (los tres reporters usan la clave `message` para el texto del error, que el servidor **eleva a la columna `message`** y no repite en `context`; el resto de claves permitidas está en `ErrorContextAllowlist` y una prueba compara la lista con las claves que aparecen en `errorReporter.ts`, `clientErrors.ts` y las llamadas `.report()` del quiosco) con valores escalares saneados y truncados a 200; `trace_id`, `employee_uuid` y `device_id` tienen columna propia. El cliente sanea también (ya lo hacía), pero el servidor no se fía: la prueba envía PII desde un cliente **y provoca una `QueryException` real con `DETAIL: Key`** y comprueba la tabla. *(Corregido tras las dos revisiones: la primera lista no contenía ninguna clave real de los clientes —los errores llegaban sin mensaje y todos los `web.vue_error` colapsaban en una fila—, y una `QueryException` dejaba nombres y hashes en claro.)*
6. **La escritura es síncrona, atómica y no puede fallar hacia fuera.** `INSERT … ON CONFLICT (fingerprint) DO UPDATE SET occurrences = occurrences + 1, last_seen_at = GREATEST(last_seen_at, EXCLUDED.last_seen_at), first_seen_at = LEAST(…), …` (el `occurred_at` de un cliente se acota por arriba al instante de recepción y **por abajo** a recepción − retención: un reloj de tablet atrasado un año no puede arrastrar un grupo vivo hacia la purga ni reabrirlo con una ocurrencia anterior a su `resolved_at`; se reabre solo si la ocurrencia es posterior) por una **conexión propia** (`error_events`, mismo DSN que `pgsql`, declarada en `config/database.php`) para que no dependa del estado de la transacción que acaba de fallar. Va envuelta: si la base de datos no responde, se registra en el log y se sigue (regla dura 19; un error al guardar el error no puede convertirse en un segundo error). **Un error resuelto que vuelve a ocurrir se reabre** (`resolved_at = NULL`, `resolved_by_user_id = NULL`) conservando `occurrences`: que un fallo dado por arreglado reaparezca es exactamente lo que IT debe ver.
7. **Transporte de cliente, dos canales y ninguno nuevo para el quiosco.** El quiosco envía `client_errors` (máximo 50) **dentro del latido**, como anticipó la 1.8, y la respuesta devuelve `client_errors_accepted` para que `acknowledge(n)` vacíe solo lo confirmado; nunca se abre otra conexión que compita con la cola. Panel y portal usan `POST /api/v1/client-errors` (`202`, `{accepted}`, máximo 50 por envío) con su token de sesión, drenado por `@kronoqr/web-kit/clientErrorTransport` al iniciar sesión, cada 60 s si hay pendientes y en `pagehide` con `keepalive`. **No hay endpoint anónimo**: los errores previos al inicio de sesión esperan en el buffer (techo 50) y una superficie pública que escribe en base de datos sería un vector de denegación. `source` lo decide el **servidor por el tipo de token** (dispositivo → `kiosk`, sesión de gestión → `admin`, sesión de portal → `portal`), nunca el campo `app` del cliente. Un token de quiosco en `/client-errors` recibe `403` (tiene su canal) y `/client-errors` **nunca devuelve el histórico**. Límite `throttle:client-errors`: `PRODUCT_CLIENT_ERRORS_RATE_LIMIT=12` por minuto y token, ×4 por IP.
8. **Autorización.** `GET /diagnostics/errors` y `POST …/resolve` exigen `diagnostics:*` y `ErrorEventPolicy` (`admin`). **Un acceso de soporte lee pero no resuelve**: `view` admite `isSupportActor()` (el alcance `diagnostics` de la 5.9 se concedía «para el paquete anonimizado y los errores») y `resolve` lo rechaza, porque dar un fallo por resuelto en la instalación de un cliente es decisión del cliente. **Resolver no escribe en `audit_log`**: son datos técnicos sin relevancia legal (regla dura 6, al revés); quedan `resolved_at` y `resolved_by_user_id`. Resolver es idempotente (`200` con la misma fila).
9. **Consulta.** `GET /diagnostics/errors?source=&level=&status=open|resolved|all&from=&to=&page=&per_page=`, ordenado por `last_seen_at` descendente, `status=open` por defecto, paginado a 25 (máximo 100), con `meta` como la bandeja de incidencias. Los instantes salen en UTC y el panel los presenta en la zona del centro (regla dura 3).
10. **Comandos.** `product:errors [--since=24h] [--level=] [--source=] [--json] [--lang=]` lista los grupos abiertos y sale `0` (ninguno), `1` (hay `error`) o `2` (hay `critical`), como `doctor`, para que un script pueda preguntar. `product:errors:prune [--dry-run]` borra por `last_seen_at < ahora − ERROR_HISTORY_RETENTION_DAYS` (la misma variable que ya leía `config/compliance.php`), **a diario a las 03:35 UTC**, sin confirmación y sin asiento: es la purga técnica de RL-11, no la de RF-PR-03. `compliance:apply-retention` sigue informando del ciclo corto como hasta ahora; `DatabaseErrorHistoryArchive` no cambia.
11. **Métrica y alerta.** `application_errors_total{source,level}` en Redis (`HINCRBY`, como `scans_total`), añadida a la lista cerrada de `MetricsCollector`. Regla `infra/observability/prometheus/rules/errors.yml`: `increase(application_errors_total{level="critical"}[5m]) > 0` → `ErroresCriticosNuevos`, severidad alta, destinatario IT del cliente, runbook `errores-en-el-panel.md`; la sirve `/metrics` de la 3.1 como `auth.yml`. Las otras dos filas del doc 01 §9.3 que apuntan al mismo runbook (5xx y p95 del fichaje) son reglas de la 3.2: el runbook las cubre ya; las reglas no.
12. **Paquete y exportación.** `ErrorEventsCollector` pasa de `not_installed` a real: resumen por origen y nivel más los grupos del periodo (máximo 500, `last_seen_at` descendente) por `FieldAllowlist`, con `trace_id`. `error_events` **entra en la exportación íntegra** de la 5.10 (son datos del cliente; deja de figurar en `not_installed` y `lang/*/data-export.php` describe sus columnas). `absences` sigue `not_installed`.
13. **Panel.** `features/errors/`, ruta `/errors`, entrada «Errores» con `diagnostics:*`. Lista agrupada con filtros de origen, nivel, estado y periodo; por fila, nivel, origen, mensaje, `occurrences`, primera y última vez (relativa y absoluta), versión y `trace_id` copiable; detalle con clase, `file:line`, contexto y **un texto «qué hacer» por origen y nivel** en español e inglés, escrito para quien no conoce el sistema, que remite a la sección de `operacion.md` y al runbook. Botón «Marcar como resuelto» con confirmación; oculto para un actor de soporte.
14. **Techo de grupos por origen** (revisión de seguridad): el limitador acota peticiones, no filas; con 50 por envío y 12 envíos por minuto una sesión podía crear 600 grupos nuevos por minuto durante 90 días. El catálogo cerrado corta la entropía en el origen, y además `PRODUCT_ERRORS_MAX_OPEN_GROUPS_PER_SOURCE` (500 de serie) pone cota: por encima, la ocurrencia incrementa un grupo de desbordamiento por origen (`code = overflow`, mensaje fijo) en vez de crear fila, y `product:doctor` avisa del tamaño de la tabla. **La alerta cuenta grupos nuevos o reabiertos, no ocurrencias**: `application_errors_total{source,level}` sigue midiendo ocurrencias (doc 02 §8.2) y `application_error_groups_opened_total{source,level}` (nueva, `RETURNING xmax = 0` del upsert) es la serie de `ErroresCriticosNuevos`, para que una cámara rota conocida no mantenga la alerta encendida indefinidamente. **`resolved_by` sale `null` para un actor de soporte** (el paquete ya lo excluía por el mismo motivo; `IncidentUser.name` exige un nombre, así que no cabe un `uuid` sin nombre sin un esquema propio). `product:doctor` gana la sonda `app.error_history` (aviso al 80 % del techo, fallo al techo). `code`, `exception_class` y `file` se truncan en el sumidero a la anchura de su columna: un valor largo no puede hacer fallar el `INSERT` y perder el error.
15. **La «inspección manual de la tabla» tiene versión automática**: `ErrorEventsHaveNoPersonalDataTest` provoca errores por los siete orígenes con mensajes y contextos cargados de nombre, correo, DNI, hora y teléfono, y comprueba que ninguna columna de ninguna fila los contiene. La inspección manual sobre la semilla realista sigue en la DoD.
...
| `BRANDING_LOGO_ROOT` | `/var/kronoqr/branding` | 5.8 | Directorio del contenedor contra el que se valida `BRANDING_LOGO_PATH`, en solo lectura. Sin ese cerco, el endpoint público del logotipo podría servir cualquier fichero del servidor |
| `BRANDING_PATH` | `./branding` | 5.8 | Directorio del servidor del cliente que Compose monta en `BRANDING_LOGO_ROOT`. Relativo al directorio de instalación, como `TLS_CERT_DIR` |
| `PRODUCT_BRANDING_RATE_LIMIT` | `120` | 5.8 | `GET /api/v1/branding` es pública y la piden navegadores al arrancar: un techo por IP holgado para una recepción detrás de un NAT y estricto para un bucle |

`BRANDING_APP_NAME`, `BRANDING_LOGO_PATH` y `BRANDING_ACCENT_COLOR` **no son variables de entorno**: son claves de `installation_settings` (5.1) que se siembran desde el asistente de puesta en marcha (5.5) y se editan desde el panel (5.8). Las variables `BRANDING_NAME` y `BRANDING_ACCENT_COLOR` que el Anexo B listaba se retiraron en la 5.8 (ficha 5.8, «Decisiones tomadas», punto 1).
...
**Cierre ejecutado (09/10-09-2026, rama `chore/cierre-fase-5`).** Los cuatro revisores en paralelo sobre `main` `9d5ec6f` (última tarea integrada: 5.11b, PR #52). Lo que encontraron y se corrigió **antes** de subir `current_phase`, porque ese literal es el registro de que la fase se cerró: (1) `doctor.sh` no viajaba en el paquete (`package.sh` era de la 5.7, el script de la 5.9; ahora manifiesto + `test -x` en ⑧ + guarda en `QualityGatesTest`); (2) la actualización y la vuelta atrás no dejaban asiento en `audit_log` (riesgo del doc 07 fechado «antes del cierre»): `system.updated` y `system.restored_from_backup` (`AuditAction`, familia `InstallationLifecycle`, `SystemEventPayload` con lista cerrada y sin PII, `compliance:record-system-event` y `compliance:audit-chain-head`, enganchados en `update.sh`; el de la vuelta atrás se escribe **sobre la cadena restaurada** con la huella de la descartada); (3) seis claves de `installation_settings` no tenían vía de cambio tras el asistente y la guía decía «desde el panel»: pantalla **«Ajustes operativos»** (`/settings`) y `SettingsSurfaceTest`, que exige a cada `SettingKey` una vista escritora y su fila en la guía; (4) el doc 05 §10.5 prometía que al caducar la licencia no se perdía «nada más» y ADR-023 degrada también la marca blanca: corregido el doc 05, no el ADR; (5) la matriz de trazabilidad llevaba desde la 5.5 sin regenerar (`docs/` va `:ro` en el contenedor) y tres requisitos estaban cubiertos por etiquetas que no probaban lo que decían (RL-16 pasa a `verificacion: revision`, RL-17 gana 15 pruebas de comportamiento, RQ-11 pierde 9 etiquetas derivadas, RF-GP-05 gana su E2E): `TraceabilityMatrixFreshnessTest` impide que vuelva a pasar; (6) **la CI no ejecutaba la suite de integración ni ninguna prueba de frontend** y `e2e.yml` era un marcador: jobs ④ Integración, ⑥ unitarias de los cuatro paquetes y ⑦ E2E de las tres SPA (el portal no tenía ninguna: 27 nuevas con `axe`), más la cobertura RNF-M-01 como job nocturno con `make coverage` arreglado (OOM); (7) el *bind mount* de Docker Desktop oculta 45 ficheros de `Product/Domain/ValueObject` al iterador de PHP: la mutación y la cobertura locales de `Product` medían 52 de 97 ficheros (el 74,85 % del `HANDOFF` era artefacto; el real es 81,77 %) y tres pruebas de arquitectura no los miraban: `SourceDiscoveryTest` lo delata y `ModuleTree` recorre con `scandir`; (8) `DiagnosticsBundleVolumeTest` no poblaba `error_events` (la tabla no existía cuando se escribió); (9) `throttle:management` en `/credentials`, `type` propio para los `409` de la corrección, `completed_at` fuera de la respuesta pública del asistente, `kiosk_pairing_total` en el doc 02 §8.2, `obligaciones-legales.md` alineado con lo que audita el soporte (ventanas de 15 min), `endurecimiento.md` con las cinco superficies sin autenticar, y los seis riesgos aceptados del doc 07 con la fecha vencida re-fechados con dueño. **Resultado de la tabla de arriba:** las ocho comprobaciones verificables por herramienta en verde (CI 34400365952 y la de `main` 34402932670 con ⑧ y ⑧b; salto no consecutivo solo a nivel de integración hasta que `versions.txt` publique 2.2.0); la instalación limpia por una persona ajena siguiendo solo la guía, y los cuatro recorridos de la 5.11b, **siguen pendientes del usuario**. Doc 07: SAMM 1,47 → **1,73**. Deuda anotada en `HANDOFF.md` (MSI de `Workforce` 66,67 % y `Kiosk` 77,70 % por módulo con el global en 82,83 %; catorce rutas de gestión sin zona de límite; `plan_exceeded` por fila en la importación; guarda de autorización negativa por ruta; presupuesto local de la suite unitaria).
...
| # | Tarea | Punto |
|---|---|---|
| 1 | 5.1, 5.8 | **Precedencia entre las variables de entorno del Anexo B y las filas de `installation_settings`** para la misma propiedad (caso claro: `BRANDING_APP_NAME` frente a la clave de marca en base de datos; también `COMPLIANCE_PROFILE` frente a `sites.compliance_profile_id`). La cascada del ADR-017 define los ámbitos `installation` y `site`, pero no sitúa el entorno en ella. ✅ **RESUELTO (07-09-2026, tarea 5.8)** — **manda la base de datos**, como fijó la 5.1: la variable de entorno **no es un escalón** de la cascada. `BRANDING_NAME` y `BRANDING_ACCENT_COLOR` se retiran del Anexo B; la marca la siembra el asistente de puesta en marcha y se edita desde el panel. `COMPLIANCE_PROFILE` sigue siendo solo el valor con el que el instalador siembra `sites.compliance_profile_id`. Detalle en la ficha 5.8, «Decisiones tomadas» |
...
| 6 | 5.4, 5.7, 5.9 | **Los valores concretos de los códigos de salida** de los cinco scripts. El §3.5 exige que estén «documentados en la cabecera del script», pero no fija ninguno, y `install.sh` y `update.sh` deben ser consistentes entre sí. ✅ **RESUELTO** — la tabla común la fijó la 5.4 en `infra/scripts/lib/exit-codes.sh` (0–6) y la 5.7 la respeta sin excepción: `update.sh` nunca sale con `6` (toda verificación fallida deshace) y su `2` cubre también la copia previa fallida, porque la instalación no se ha tocado |
...
| 11 | 5.7 | **Cómo se materializa el «punto de control entre cada migración»** del §11.6.4: si es una copia incremental, un `savepoint`, una marca de versión o un volcado por versión. El §11.6.4 exige el punto de control, no su mecanismo. ✅ **RESUELTO (07-09-2026, tarea 5.7)** — **marca de versión persistida**: un lote de `migrations` por versión intermedia y una línea de punto de control en pantalla y en el informe. La vuelta atrás es siempre restaurar la copia verificada previa, nunca `migrate:rollback` en un cliente. Detalle en la ficha 5.7, «Decisiones tomadas» |
| 12 | 5.9 | **Valores admitidos del campo `scope` de `support_grants`.** El doc 01 §5 lo declara y RF-PD-11 exige «alcance limitado», pero ningún documento enumera los alcances posibles. ✅ **RESUELTO (08-09-2026, tarea 5.9)** — tres valores, cada uno con sus ámbitos de token y el rol con el que actúa ante las policies: `diagnostics` (por defecto: paquete anonimizado y errores), `read_only` (además lee jornadas, plantilla y auditoría; actúa como `auditor`) y `configuration` (además cambia ajustes, perfil y quioscos; actúa como `admin`). Ninguno activa licencias, concede accesos, toca credenciales, corrige fichajes ni incluye datos personales en un paquete. Tabla en el contrato (`POST /api/v1/support/grants`) y en la ficha, «Decisiones tomadas» |
| 13 | 5.9 | **Formato, cifrado y canal de entrega del paquete de diagnóstico.** El §11.6.6 y el doc 05 §10.6 describen el contenido y que «se envía al soporte», pero no el formato del fichero, si va cifrado ni por qué canal viaja. Relevante porque atraviesa la frontera de datos del cliente. ✅ **RESUELTO (08-09-2026, tarea 5.9)** — **un único fichero JSON legible** (`kronoqr-diagnostics-<version>-<UTC>.json`) con `manifest.sha256` del resto del documento; **sin cifrar**, porque el cliente tiene que poder inspeccionarlo antes de enviarlo (la ficha exige esa inspección) y una clave de cifrado compartida con el fabricante sería un secreto más que custodiar; el canal es el del contrato de soporte del cliente, fuera del producto. Detalle en la ficha, «Decisiones tomadas» |
| 14 | 5.10 | **Destino, protocolo y periodicidad de la telemetría** cuando `TELEMETRY_ENABLED=true`, y la lista exacta de campos enviados. RF-PD-12 fija qué **no** puede llevar (datos personales ni de jornada) y que viene desactivada; el resto no está definido. ✅ **RESUELTO (08-09-2026, tarea 5.10)** — **un documento JSON por HTTPS (`POST`), semanal (lunes 05:40 UTC desde el planificador, nunca en una petición), a `TELEMETRY_ENDPOINT` (vacío de serie; lo fija el cliente en el `.env`)**, y solo si además `TELEMETRY_ENABLED=true` y la licencia lista `telemetry` en `features`. Identificador `installation_id` aleatorio (UUID v4, guardado en `storage/app/telemetry/state.json`; ni `license_id` ni razón social). Contenido cerrado y atado por prueba a la tabla de `configuracion.md`: versión del producto, PHP y PostgreSQL; estado, plan y funcionalidades de la licencia; plantilla activa **por tramos**, quioscos y departamentos; contadores agregados de siete días; resultado de `doctor` sin mensajes. Un reintento y silencio hasta la semana siguiente. `php artisan product:telemetry` imprime lo que se enviaría. Detalle en la ficha, «Decisiones tomadas», punto 8 |
...
| 16 | 5.12 | **Cómo llegan al servidor los errores de los tres clientes y qué cuenta como error del servidor.** RF-PD-15 exige captar «los tres clientes» y el paso 6 de la ficha pide «un endpoint de recepción con rate limiting», pero ningún documento fija si es un endpoint por cliente, si el quiosco usa el latido (lo que la 1.8 dejó preparado), si existe un canal anónimo, ni qué excepciones del servidor son errores frente a desenlaces de negocio. ✅ **RESUELTO (09-09-2026, tarea 5.12)** — el quiosco reporta **dentro del latido** (`client_errors`, respuesta `client_errors_accepted`); panel y portal por `POST /api/v1/client-errors` con su sesión, **sin canal anónimo**; el `source` lo decide el servidor por el tipo de token. Del servidor se persiste todo `Throwable` salvo las excepciones de `Domain\Exception` y `Application\Exception` de los módulos y las `HttpException` `< 500`. Detalle en la ficha 5.12, «Decisiones tomadas», puntos 1 y 7 |
| 17 | 5.11b | **Quién produce la hoja de instrucciones del empleado y con qué datos.** La ficha la sitúa como artefacto de `docs/cliente/` («fuente; se genera el PDF de una cara») y a la vez exige que lleve la dirección del portal, que salga en los idiomas activos de la instalación y que se entregue con la tarjeta; ningún documento dice cómo un fichero del paquete sabría nada de eso. ✅ **RESUELTO (09-09-2026, tarea 5.11b)** — la produce el producto: `GET /api/v1/credentials/instructions-sheet?locale=` devuelve un PDF A4 de una cara con la marca, la dirección del portal y los textos de `lang/<idioma>/instructions-sheet.php`; `hoja-empleado.md` es la guía sobre la hoja y reproduce su texto, atado por prueba. Detalle en la ficha 5.11b, «Decisiones tomadas», puntos 1 y 2 |
