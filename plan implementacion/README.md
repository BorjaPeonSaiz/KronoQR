...
| 0-9 | 0.7 | **Valor y gobierno de `quality.current_phase`.** Clave nueva de `backend/config/quality.php`: es lo que fija el alcance del bloqueo de `qa:traceability --check`. **Resuelto en el cierre de la Fase 0**: es un literal versionado, no una variable de entorno, y se actualiza como parte del procedimiento de cierre de cada fase | 🟢 |
...
| 2-6 | 2.4 | ✅ **Lighthouse CI** (`@lhci/cli`) contra la build de producción con datos de semilla de 500 empleados. Es la herramienta estándar para LCP en CI y no exige infraestructura nueva, a diferencia de k6 (que el §9.2 reserva para carga de API). **Nota del cierre de Fase 2 (30-08-2026):** la via implementada mide LCP con `PerformanceObserver` dentro del E2E de Playwright; `@lhci/cli` queda como mejora opcional de `devops-observabilidad` | ✅ |
...
| 3-17 | 3.6 | ✅ ***Workflow* propio: `.github/workflows/load-test.yml`.** No `release.yml` —que sigue siendo un marcador del plan 08— ni `ci.yml`: el pipeline del §10.1 no tiene etapa de carga y **las etapas ①–③ deben seguir por debajo de 4 minutos**. Se dispara a mano (`workflow_dispatch`, con `instances` y `duration`) y en **cada etiqueta `vX.0.0`**. **Decisión 19: ahí NO se juzga el umbral.** El runner reparte su CPU entre el servidor y los once generadores y no llega a 50 fichajes/s (medido: 29 tramos/s con 60 ofrecidos, y con el pool doblado igual), así que lo que juzga es **regresión contra su propia línea base** (`load-tests/k6/baseline.json`: hasta +25 % de p95 y −20 % de tramos/s) e **integridad bajo sobrecarga**. **RNF-P-02 y RNF-P-06 se juzgan solo sobre hardware de referencia con `make load-test`**, paso manual del procedimiento de publicación de una versión mayor (RQ-08). Mide **las imágenes de entrega instaladas desde el paquete**, no la pila de desarrollo, y sube `.results/` como artefacto (nunca `.fixtures/`, que lleva tarjetas y tokens vivos mientras dura la pasada) | ✅ |
