...
5. Implementar el **ámbito por recurso** de RF-ID-03 como filtro en la consulta, no como comprobación posterior: un responsable solo alcanza empleados de su departamento (`departments.manager_user_id`, doc 01 §5.5). No hay eje por centro: hay exactamente uno por instalación (ADR-040).
...
3. Backend: difusión con **Reverb** (doc 02 §3.1, ADR-011) de los eventos de dominio `EmployeeClockedIn` y `EmployeeClockedOut` (doc 01 §5.4) hacia canales privados por departamento —y uno global de la instalación—, autorizados con el ámbito de 2.1. Sin canal por centro (ADR-040).
