...
4. Quiosco: pantalla de diagnóstico con código de servicio (RF-KI-08) que muestra estado de cámara, red, cola, token y **versión desplegada** (doc 02 §10.5). **Nota (10-09-2026):** el «estado de cámara» incluye lo que devuelve `MediaStreamTrack.getSettings()` de la pista que abre `useCamera.ts` —etiqueta del dispositivo, resolución real, `frameRate`, `facingMode`, `focusMode`, `zoom` y `backgroundBlur`— y **avisa sin bloquear** (regla 19) cuando `backgroundBlur === true`, cuando `focusMode` no es `continuous` o cuando la resolución queda por debajo de 1280×720. Origen: una webcam de portátil con Windows Studio Effects (encuadre automático + desenfoque de fondo) entregaba a Chrome un fotograma recortado y borroso sin que nada en la PWA lo dijera (runbook `alta-nuevo-quiosco.md` §6). Tipos: `backgroundBlur` ya está en `lib.dom` de TypeScript 5.9; `focusMode` y `zoom`, en `frontend-kiosk/src/shared/media/media-capabilities.d.ts`.
...
**Decisiones tomadas al ejecutar la tarea (16-09-2026).** Las que los documentos dejaban abiertas o que el estado real del repositorio obligó a fijar. Son la especificación común de los agentes que la ejecutan y quedan escritas también donde las lee quien las necesita (`openapi.yaml`, `configuracion.md`, `operacion.md`, doc 02 §8.2, doc 07 §6). El análisis previo de huecos encontró que **el veredicto de salud ya existe en el dominio** (`KioskHealthRow`/`KioskHealthReport`, tarea 5.11, con los umbrales de `config/kiosk.php`) y solo lo consume la consola; que el latido persiste tres columnas y descarta `oldest_pending_at`; que **la batería no existe en ninguna capa** aunque el doc 05 §5.3 la promete; que no hay ningún canal autenticado por el que el quiosco reciba configuración salvo el latido y el padrón; y que en la tablet no hay gesto, ruta ni código de servicio: `features/diagnostics/` es un README.

1. **Contrato primero y aditivo sobre `/api/v1` (ADR-012).** `KioskHeartbeatRequest` gana `battery_level` (0–100 o `null`) y `battery_charging` (booleano o `null`), los dos opcionales: la Battery Status API solo la ofrece Chrome en Android, que es la tablet del producto, y una tablet que no informa no es una tablet averiada. `KioskHeartbeat` gana `service_code_hash` (decisión 6). `Device` gana `battery_level`, `battery_charging`, `oldest_pending_at` y `health` (decisión 2). `DeviceList` gana `meta` (decisión 3): la frase «sin `meta`» del contrato de la 5.6 se sustituye, no se contradice en silencio.
2. **El veredicto lo calcula el servidor y hay una sola regla.** `GET /api/v1/devices` devuelve por dispositivo `health.verdict` (`ok|warning|failure|revoked`), `health.reason` (`beating|queue_pending|late|silent|awaiting_first_heartbeat|never_seen|revoked|battery_low`) y `health.seconds_since_last_seen`, calculados con `KioskHealthRow`, la misma clase que `kiosk:health`. Así panel, consola y alerta `QuioscoSinLatido` (600 s) cuentan lo mismo, que es lo que pide el paso 3. **Los umbrales siguen en `config/kiosk.php`** (`KIOSK_HEALTH_FRESH_WITHIN_SECONDS`, `KIOSK_HEALTH_SILENT_AFTER_SECONDS`) y no pasan a `installation_settings`: la regla de Prometheus es YAML estático atado por `AlertCatalogueTest`, y un umbral editable desde el panel que no moviera la alerta rompería justo la coherencia que se busca. Se añade `KIOSK_HEALTH_BATTERY_LOW_PERCENT` (15) con el mismo tratamiento.
3. **`DeviceList.meta`** lleva `generated_at` (reloj del servidor: el panel extrapola la antigüedad como hacen los almacenes de presencia, incidencias y errores, nunca con el reloj del navegador), `timezone` (la del centro, `InstallationSiteProvider`, para «la zona horaria mostrada» del paso 2) y `thresholds` (`fresh_within_seconds`, `silent_after_seconds`, `battery_low_percent`) para que la leyenda del panel diga el umbral real de la instalación.
4. **`oldest_pending_at` se persiste** (columna nueva) y se devuelve en `Device`: convierte «37 pendientes» en «el más antiguo es de hace tres horas». **Migración *expand* pura** en un solo despliegue, como `paired_at`: `battery_level SMALLINT NULL` con `CHECK devices_chk_battery_level_range (0..100)`, `battery_charging BOOLEAN NULL`, `oldest_pending_at TIMESTAMPTZ(6) NULL`; sin valor de serie, sin backfill, reversible.
5. **La batería es información y entra en el veredicto solo por abajo.** `KioskHealthRow` añade la razón `battery_low` (veredicto `warning`) cuando `battery_level <= batteryLowPercent` **y** `battery_charging === false`: una tablet colgada de la pared que se descarga es una tablet a la que alguien ha quitado el cargador, y se sabrá antes de que muera. `null` en cualquiera de los dos no avisa. La prioridad de razones queda: revocado → sin latido nunca → silencioso → tardío → batería baja → cola pendiente → latiendo.
6. **Código de servicio (RF-KI-08): configuración de la instalación, entregada al quiosco como huella por el latido.** Clave nueva `SettingKey::KIOSK_SERVICE_CODE` (`optionalText`, patrón `^[0-9]{8,12}$`, vacío de serie = pantalla sin código, impacto `PRESENTATION`), editable en «Ajustes operativos» (`SettingsSurfaceTest` lo exige) y documentada en `configuracion.md` §6.0 ES/EN. **Numérico** porque la tablet solo tiene el teclado en pantalla de `PinNumericKeypad`. El servidor devuelve en cada `KioskHeartbeat` `service_code_hash` = SHA-256 hex de `"{device_uuid}:{código}"` (`null` si no hay código); la tablet lo guarda en `localStorage` y comprueba el código en local con `sha256Hex`, **sin red**, con cinco intentos fallidos → 60 s de espera. Se eligió el latido y no el padrón (que es «dos campos y ni uno más» por RL-12) ni `GET /branding` (público, sin token) ni una variable de compilación (igual para todas las instalaciones, regla 13). **El código nunca aparece en claro** fuera de `/settings`: ni en el asiento de auditoría del cambio (se registra que cambió, no el valor), ni en el paquete de diagnóstico (`DiagnosticsConfigurationAllowlist`), ni en logs (`SettingsTelemetry` ya solo registra claves). **Riesgo aceptado (doc 07 §6):** con acceso físico al almacenamiento de la tablet una huella SHA-256 de un código de 8–12 cifras se fuerza; lo que protege es una pantalla sin datos personales ni token, y el coste de un KDF por latido y por quiosco no se justifica.
7. **Sin huella conocida, la pantalla se abre sin código.** Una tablet sin emparejar no tiene nada que proteger (ni token, ni padrón, ni cola con jornadas) y es justo la que hay que diagnosticar; una emparejada que aún no ha recibido ningún latido tampoco. En ese caso la pantalla lo dice en su cabecera («sin código de servicio configurado») y `product:doctor` gana la sonda `kiosk.service_code` (aviso, nunca fallo) para que el IT lo sepa desde el servidor.
8. **Cómo se abre: pulsación larga de tres segundos sobre el reloj de pared de `ScanView` y de `PairingView`**, un botón real (`kiosk-touch`, `aria-label`, alcanzable con teclado) que visualmente es un reloj. **Corrección al ejecutar:** las dos vistas no tenían ningún reloj visible (el análisis lo daba por hecho), así que el reloj es un elemento nuevo y discreto en la cabecera —`ClockDiagnosticsTrigger.vue`—, revisado por `ui-ux`; sirve de reloj y de disparador, no compite con el visor, no cabe abrirlo por accidente y no cambia el flujo de fichaje (regla 19). Ruta `/diagnostics` con `import()` y **exceptuada del guard del router** que manda a `/pair` sin token (decisión 7). La pantalla tiene «Volver a fichar» a 48 px y **vuelve sola a los 120 s sin interacción**; fichar nunca depende de ella.
9. **Qué muestra la pantalla (todo por `device_id`, nunca nombres, regla 21):** *Cámara* (estado de `useCamera`, etiqueta, resolución real, `frameRate`, `facingMode`, `focusMode`, `zoom`, `backgroundBlur`, linterna; **avisa sin bloquear** cuando `backgroundBlur === true`, `focusMode !== 'continuous'` o resolución < 1280×720, según la nota del paso 4); *Red* (`online|offline`, servidor alcanzable, hora del último latido correcto y desfase de reloj en segundos con su signo); *Cola* (pendientes, más antiguo, `durable` o respaldo en memoria, sincronizando); *Padrón* (`generated_at`, número de entradas, PIN disponible); *Token* (presente o no, caducidad, `device_id`, nombre del quiosco, y como identificador **los ocho primeros hex de `sha256(token)`, nunca el token ni su `id|` de Sanctum**); *Versión* (`APP_VERSION`, estado del *service worker*); *Batería* (nivel y cargando); *Wake lock* (soportado, activo); *Errores pendientes de enviar* (`errorReporter.size()`); *Responsable del tratamiento* (configurado o no, la pista que `privacy.ts` ya dejó). Ningún código nuevo en `ClientErrorCode`: la pantalla no reporta, muestra.
10. **Tres costuras del quiosco que la pantalla necesita y hoy no existen:** `CameraController.settings()` y `capabilities()` (la pantalla abre su propio flujo con `useCamera`, porque `ScanView` se desmonta al navegar, y lo suelta al salir); `persistPairedDevice()` guarda también `expires_at` y `device.name` (claves `kronoqr.kiosk.device_token_expires_at` y `kronoqr.kiosk.device_name`); `OfflineQueueController.rosterGeneratedAt()`; el planificador del latido conserva el último resultado (`lastBeatAt`, `lastSkewSeconds`, `serviceCodeHash`) y la instantánea del latido incorpora la batería desde `navigator.getBattery()` (tipos en `shared/media/battery.d.ts`, no en `lib.dom`).
11. **El panel no virtualiza.** El paso 2 dice «virtualización si hay muchos dispositivos», pero el contrato no pagina por ADR-040 (un hotel, unos pocos quioscos) y la tabla `<table>` semántica se conserva; pasar a `role="table"` con `useVirtualizer` solo para una lista que cabe en pantalla cambiaría selectores de pruebas y E2E sin beneficio. Si algún día `GET /devices` paginara, la virtualización llegaría con la paginación.
12. **Presentación en el panel:** módulo puro `devicePresentation.ts` (patrón `errorPresentation.ts`) con `verdictBadgeClass` (`bg-kq-{success|warning|danger}-soft text-kq-…`, doc 06 regla 5) y `reasonKey`; fila resaltada **con texto e icono, nunca solo por color** (WCAG 1.4.1); antigüedad desde `meta.generated_at`, instante con `formatInstantWithZone` y `meta.timezone`; columnas nuevas batería (`%` y «cargando» / «no informa») y cola más antigua; leyenda con los umbrales de `meta.thresholds`; `useDeviceRows.elapsedSinceHeartbeat` clava a cero un latido «del futuro». **Enlace al runbook** como en `features/errors`: el runbook no viaja al navegador; un bloque «Qué hacer» por veredicto en i18n nombra `docs/runbooks/quiosco-no-responde.md` §2 y el comando `php artisan kiosk:health` para el IT. «Ajustes operativos» gana el campo «Código de servicio del quiosco» con su ayuda.
13. **ADR-023: no degradable.** La salud de los quioscos y la pantalla de diagnóstico son operación del registro legal (un quiosco parado abre incidencias RN-15); sin caso en `Feature`, sin consulta a la licencia, y `KioskHealthDoesNotDependOnLicenseTest` lo demuestra con el precedente de `DiagnosticsDoNotDependOnLicenseTest`.
14. **Instrumentación (única deuda de OTel por módulo):** `Kiosk/Http/Support/KioskTelemetry.php` con el patrón de `SettingsTelemetry` (tracer `kronoqr.kiosk`, spans `kiosk.record_heartbeat` y `kiosk.list_devices`, solo `device_id`/`trace_id`, sin valores); métrica nueva `kiosk_battery_level{device}` (gauge) en `RedisKioskMetrics`, `MetricCatalogue` y doc 02 §8.2; el panel del cuadro «Operación de quioscos» queda para restos.
15. **Pruebas (§9.5):** Feature `DeviceListTest` (forma, `health`, `meta`, orden, revocado sin veredicto de alerta) y latido con batería y `oldest_pending_at` → `RF-PA-07`; autorización negativa por rol y con token de quiosco en `GET /devices` y latido con token de gestión → `RS-04`, `RF-ID-03`; unitarias de `KioskHealthRow` (batería, prioridad de razones) y de `SettingKey`; contrato; `SettingsSurfaceTest` y `DoctorCommandTest` con la clave nueva; sin licencia; E2E panel «un quiosco sin latido aparece marcado» `@RF-PA-07`; E2E quiosco «se abre con el código y muestra cámara, red, cola, token y versión sin revelar el token» `@RF-KI-08` (la cámara falsa de Chromium no trae `focusMode` ni `backgroundBlur`: se afirma que las filas y los avisos aparecen, no valores de hardware); axe 0 críticas/graves en las dos; Vitest ≥ 70 %; paridad ES/EN.
16. **Documentación:** `operacion.md` ES/EN gana el apartado «La pantalla Quioscos del panel» (mismo número de apartados en las dos lenguas, `ClientDocumentationTest`), `configuracion.md` §6.0 la fila de `KIOSK_SERVICE_CODE` y la de `KIOSK_HEALTH_BATTERY_LOW_PERCENT`; `alta-nuevo-quiosco.md` el apartado de la pantalla de diagnóstico y el código de servicio; `quiosco-no-responde.md` §2 arranca por la pantalla del panel; doc 07 §6 el riesgo aceptado de la decisión 6.
17. **Lo que queda fuera y por qué:** el panel Grafana de batería (3.2 restos), la ventana controlada de actualización (3.12), la detección de tablet «del futuro» como incidencia (hoy se clava a cero y se muestra), y mover los umbrales a `installation_settings` (decisión 2).
18. **Lo que corrigieron las revisiones** (`seguridad-cumplimiento`, `revisor-codigo`, `ui-ux` y `qa-testing`, 16-09-2026; todo aplicado en una segunda vuelta de los cuatro agentes): (a) **la exportación íntegra llevaba el código de servicio en claro** en el dataset `installation_settings` mientras tres textos afirmaban lo contrario → las claves `confidential` salen con `value` nulo y `value_redacted = true` (`DataExportCatalog`, `DatabaseDataExportSource` compuesto desde `SettingKey::cases()`, `DataExportOmitsServiceCodeTest` sobre el ZIP entero); (b) **un actor de soporte con alcance `configuration` leía y cambiaba el código** → `SettingResource` devuelve `value: null` + `redacted: true` a cualquier actor de soporte y `PATCH` responde 403 antes de validar (`SettingsPolicy::updateConfidential`, contrato aditivo: `InstallationSetting.value` nullable y `redacted`), en la línea de `DataExportPolicy` y `ComplianceProfilePolicy`; (c) **el bloqueo de cinco intentos vivía en la vista** y «Volver a fichar» lo reiniciaba → persistido en `localStorage` (`kronoqr.kiosk.service_code_lockout`), y el comentario que hablaba de «tiempo constante» sobre un `===` corregido; (d) **abrir el diagnóstico desde `/pair` construía el singleton de la cola sin `onDeviceRevoked`** y una desvinculación posterior ya no devolvía la tablet a `/pair` → `onDeviceRevoked` es ahora una suscripción del controlador y sin token la pantalla no arranca `SyncRunner` (`deviceRevocationAcrossViews.spec.ts`, E2E); (e) **«servidor alcanzable» se deducía de un latido congelado y la tablet dejaba de latir con la pantalla abierta** → la fila lee `onReachability` del `SyncRunner` y la pantalla tiene su propio planificador de latido con un `beat()` al abrir; (f) el temporizador de vuelta se reiniciaba con cualquier toque → solo con controles reales y techo absoluto de 5 min; (g) **el reloj-disparador no era operable con teclado** aunque su comentario lo afirmaba → `keydown`/`keyup`/`blur` sobre el mismo `useLongPress`; (h) **la insignia de veredicto se camuflaba con el tinte de la fila** en `warning`/`failure` → variante sólida `bg-kq-{warning|danger} text-kq-on-…` (doc 06 regla 5); (i) texto «qué hacer» de `queue_pending` que describía `silent`, leyenda que redondeaba 90 s a «2 min» (ahora «1 min 30 s»), bloque «qué hacer» omitido para `ok`; (j) `CameraController.capabilities()` sin consumidor retirado; (k) doc 07 A-9/A-10 y `operacion.md` §12.4/§13.1/§16.5 dicen exactamente lo que el código hace: el código es de la instalación y no del dispositivo; lo único que la pantalla puede enviar es `kiosk.wake_lock.denied`; y una tablet sin ningún latido posterior a la configuración del código abre la pantalla sin él (decisión 7, documentada con honestidad). **Aceptado y anotado como deuda:** `CheckKioskHealthTest` y `KioskHealthRowTest` fijan las mismas reglas desde dos ficheros; la tabla del panel se desplaza en horizontal a 700 px como las demás del panel (no hay patrón apilado en el panel).
...
**Decisiones tomadas al ejecutar la tarea (16-09-2026).** Las que los documentos dejaban abiertas o que el estado real del repositorio obligó a fijar. Son la especificación común de los agentes que la ejecutan y quedan escritas también donde las lee quien las necesita (`openapi.yaml`, doc 01 §4 y §11, doc 02 §8.2, `guia-rrhh.md`). El análisis previo de huecos encontró que **RN-10, RN-11 y RN-12 ya se evalúan en `Attendance\Domain\Policy\AnomalyDetectionPolicy`** con los umbrales de `CompliancePolicy` (Shared) y con RN-12 suspendida en `ComplianceRuleSuspension`; que **el exceso semanal no lo evalúa nadie** (`maximumWeeklyMinutes`, `weekStartsOn` y `holidayCalendar` llevan sin consumidor desde la 5.2, tal como anuncia el docblock del objeto de valor) y que **no tiene `RN-*`** —RF-PA-06 lo nombra, RF-PD-07 lo parametriza y el doc 01 §4 no lo enuncia—; que `Reporting` no puede importar el dominio de `Attendance` (Deptrac, doc 02 §1.6), así que la vista no puede reutilizar `AnomalyDetectionPolicy` ni `WorkDay`; que el cuadro «Negocio» ya grafica `incidents_open` por tipo pero ninguna serie describe el exceso semanal; que el panel ya usa la etiqueta «Cumplimiento» para el **perfil** (`app.nav.compliance`) y que `guards.ts` duplica el orden del menú (deuda anotada en `HANDOFF.md`).

1. **Contrato primero (ADR-013): `GET /api/v1/compliance/summary`, tag `Reporting`.** La consulta vive en `Reporting` (doc 01 §5.1 sitúa `ComplianceView` ahí) y el tag `Compliance` del contrato es «lo que se entrega fuera»; la ruta conserva el nombre del Anexo B. Ámbito **`attendance:read`**, el mismo que la presencia y el detalle de jornada: es una lectura del registro con los umbrales del perfil aplicados, y quien puede leer el registro puede leerlo con una regla encima. Policy `ComplianceSummaryPolicy` registrada contra el objeto de valor `ComplianceSummary` (no contra Eloquent), roles «manager+» = `{admin, rrhh, responsable_departamento}`; el `auditor` recibe `403` como en la presencia (auditar es mirar lo escrito, no la gestión del día). Parámetros: `from` y `to` opcionales (por omisión, los **28 días que terminan hoy** en la zona del centro: cuatro semanas, la ventana que RRHH revisa), `department_id`, `employee_uuid` y `rule`. `422` si el rango supera `REPORTING_COMPLIANCE_MAX_RANGE_DAYS` (92, mismo presupuesto síncrono que el informe por periodo) o `DateRange::MAXIMUM_DAYS`. **Sin paginación**: los hallazgos son escasos —solo los incumplimientos— y el techo lo pone el rango.
2. **El exceso semanal es una regla de negocio nueva: RN-17 «Jornada semanal ordinaria».** Se enuncia en el doc 01 §4 con el patrón de RN-10..12 (límite abierto: 40 h exactas no alertan, 40 h 1 min sí; umbral `max_weekly_hours` del perfil; la semana empieza en `week_starts_on`, ISO 1–7, y se compone de siete fechas civiles `work_date`, que ya están en la zona del centro por RN-05, así que no hay conversión de zona), con su escenario Gherkin en §11 y su fila en el Anexo A (Fase 3). **Es informativa y no abre incidencia**: el art. 34.1 ET fija las 40 h **en cómputo anual**, así que una semana por encima no es por sí sola un incumplimiento; la vista la señala para que RRHH la contraste con el convenio. `ComplianceRule` gana `MaximumWeeklyWorkingTime = 'RN-17'` y un método `opensIncident(): bool` (falso solo para RN-17) del que derivan `ComplianceProfileField::affectsIncidentDetection()` y el asiento `compliance_profile.changed`: cambiar `max_weekly_hours` o `week_starts_on` deja de ser «sin consumidor» (`hasNoConsumerYet()` queda solo para `holiday_calendar`) y el asiento y la pantalla del perfil dicen que afecta **a la vista de cumplimiento** y no a la detección. `holidayCalendar` sigue sin consumidor a propósito: ninguna de las cuatro reglas lee festivos (la semanal es por semana trabajada, no por día laborable) y fingir lo contrario sería prometer un efecto que no existe; lo estrena la 3.10 (ausencias).
3. **Una comparación, un sitio: los predicados viven en `Shared\Domain\ValueObject\CompliancePolicy`.** `restIsInsufficient(int $restMinutes)`, `dailyTimeIsExcessive(int)`, `continuousShiftNeedsBreak(int)` y `weeklyTimeIsExcessive(int)`, con el límite abierto del doc 01 (`<` y `>` estrictos). `AnomalyDetectionPolicy` delega en ellos sin cambiar de comportamiento (sus pruebas y su mutación siguen intactas) y el evaluador de la vista los usa también: el Gherkin «Perfil de cumplimiento distinto» se prueba una vez sobre el objeto de valor y vale para la bandeja y para la vista. No se mueve nada más de `Attendance`: lo que la vista y la bandeja comparten es el umbral y la comparación; lo que **no** pueden compartir es el modelo (el agregado frente a un modelo de lectura), y de eso se ocupa la decisión 5.
4. **La aritmética vive en el dominio de `Reporting` y el SQL trae hechos, no veredictos** (reglas duras 1 y 2). `Reporting\Domain\Policy\ComplianceEvaluation` (pura, sin reloj: recibe `CompliancePolicy` y las filas) evalúa por empleado un modelo de lectura `ComplianceFacts` con una fila por jornada —`work_date`, `first_in_at`, `last_out_at`, `previous_last_out_at` (fin del último tramo de la jornada **anterior**, o nulo), `total_minutes` de `daily_totals`, `has_open_shift`, y el tramo cerrado más largo `{uuid, clocked_in_at, clocked_out_at}`— y devuelve `ComplianceFinding`s. Descanso y tramo continuo se calculan **restando instantes UTC** (RN-09: el cambio de hora no altera nada); el total diario **sale de la proyección** y no se recalcula desde `shift_entries` (regla dura 7, mismo criterio que `DatabasePeriodReportReader`); el total semanal es la suma de siete `total_minutes`. `DatabaseComplianceFactsReader` (`Reporting/Infrastructure/Persistence`) resuelve el `previous_last_out_at` con `lag()` sobre la jornada anterior **partiendo una jornada antes de `from`** y carga los días fuera del rango que hacen falta para completar las semanas del borde (decisión 6); solo tramos vigentes (`closed`, `anomalous`; los abiertos cuentan cero y marcan `has_open_shift`; `voided` y `superseded` nunca); alcance en el `WHERE` (RF-ID-03); `SET LOCAL statement_timeout` de `REPORTING_COMPLIANCE_TIMEOUT_SECONDS` (10) con el mismo `422` del informe por periodo. **Desviación respecto a la ficha:** el hueco no se calcula con operadores de `tstzrange` en SQL porque las reglas duras 1 y 2 piden que la resta ocurra en el dominio; `tstzrange` se usa donde ayuda a acotar (`&&` con la ventana) y la prueba de integración con volumen es la que garantiza los huecos.
5. **Qué cuenta, exactamente lo mismo que la bandeja (paso 4).** RN-10 solo **entre jornadas** (última salida de la jornada anterior frente a primera entrada de esta; sin jornada anterior no se evalúa; un descanso negativo es un solape de RN-02 y no un descanso; cero minutos sí es descanso); RN-11 sobre la suma de tramos **cerrados** (`total_minutes`; un tramo abierto vale cero y la jornada sale marcada `has_open_shift`); RN-12 sobre el tramo cerrado más largo; RN-17 sobre la suma de la semana del perfil. **Las reglas suspendidas no se cuentan**: `ComplianceRuleSuspension` (único sitio) hace que la vista no emita hallazgos de RN-12 mientras la 3.5 no vacíe la lista, y `meta.rules[]` la lista con `evaluated: false` y su umbral para que el panel diga «Pausa en jornada continuada: 6 h según el perfil ES-hosteleria — no se evalúa hasta que exista la pausa declarada». Cuando la 3.5 vacíe la lista, la vista empieza a contarla sin tocar una línea. Una prueba de integración siembra jornadas, ejecuta `DetectAttendanceAnomalies` y la consulta, y afirma que RN-10 y RN-11 señalan **el mismo conjunto** `(empleado, jornada)`.
6. **Semanas completas, aunque el rango las corte.** Toda semana del perfil que toque `[from, to]` se evalúa sobre sus siete días —el lector carga los días de fuera del rango— y el hallazgo dice `week.starts_on`/`ends_on`; una semana con alguna jornada abierta sale `has_open_shift` (el total puede crecer). Evaluar solo la parte dentro del rango habría producido totales que no suman lo que la persona ve en su registro (paso 6).
7. **La respuesta.** `ComplianceSummary{data[], meta}`. Cada `ComplianceFinding`: `rule` (`insufficient_rest`, `daily_excess`, `missing_break`, `weekly_excess`, con `requirement` `RN-10..12`/`RN-17`), `employee` (`IncidentEmployee`: uuid, código, nombre, departamento), `work_date` o `week`, `measured_minutes`, `threshold_minutes`, `difference_minutes` (exceso o déficit, siempre ≥ 1; **minutos enteros, nunca horas con decimales**: las partes suman el total), `shift_entry_uuid` (el que abre la jornada en RN-10, el continuo en RN-12, nulo en RN-11 y RN-17), `has_open_shift` e `incident` (`{id, status}` o nulo: la incidencia de la bandeja para la misma `(empleado, jornada, tipo)` cuando existe, que es la demostración visible de que las dos cuentan lo mismo y el enlace a resolverla). Orden: empleado (apellidos, nombre, uuid), fecha, regla. `meta`: `generated_at` (puerto `Clock`), `time_zone`, `from`, `to`, `profile{id, name, jurisdiction}`, `week_starts_on`, `rules[]` (`rule`, `requirement`, `threshold_minutes`, `evaluated`, `suspension_reason`), `totals{by_rule, employees_affected, employees_evaluated}`, `scope` (`all`|`departments`) y `criteria[]` traducidos, como el informe por periodo: un aviso cuyo criterio no se ve es un aviso que nadie defiende ante un empleado (paso 5).
8. **Es un acceso a datos personales de terceros y deja constancia (RS-05):** `PersonalDataAccessLog::recordDisclosure('compliance_summary', filas, {from, to, department_id?, employee: bool, rule?, scope})` dentro de la consulta, antes de devolver; sin agrupar (no se sondea) y sin nombres (regla dura 21). Telemetría con el patrón de `PeriodReportTelemetry`: `ComplianceSummaryTelemetry`, span `reporting.compliance_summary` con el rango, el número de hallazgos y el alcance, nunca identificadores de persona.
9. **ADR-023: no degradable.** Es una lectura del registro legal contra los umbrales legales; sin caso en `Feature`, sin consulta a la licencia, y `ComplianceSummaryDoesNotDependOnLicenseTest` lo demuestra con el precedente de la 3.3.
10. **Cuadro «Negocio» (paso 7):** serie nueva `compliance_findings_last_week{rule}` (gauge, colector *textfile*, prefijo del catálogo) y `compliance_employees_affected_last_week`, publicadas por `reporting:compliance-metrics` cada noche después de `attendance:detect-incidents`, sobre **la última semana completa del perfil** y con la **misma consulta** que la vista (alcance sin restringir), para que el panel y Grafana no puedan discrepar; panel «Alertas de cumplimiento de la última semana, por regla» en `negocio.json`, fila nueva en doc 02 §8.2 y `MetricCatalogue::textfileSeries()`. Sin alerta: es un indicador de RRHH, no un fallo de operación (§8.4: una alerta sin runbook es ruido).
11. **Panel: carpeta `features/compliance/`** (`ComplianceView.vue`, `ComplianceFindingsTable.vue`, `compliance.api.ts`, `compliancePresentation.ts`, `README.md`), ruta `/compliance` (`name: 'compliance'`, `ATTENDANCE_READ`), entrada de menú «Cumplimiento» después de «Incidencias» —es la primera pantalla de trabajo de un responsable tras la bandeja— y **el perfil pasa a llamarse «Perfil de cumplimiento»** (`app.nav.complianceProfile`; se corrigen `shell.spec.ts`, `compliance-profile.spec.ts` y el texto de «Ajustes operativos» que lo cita). **Se paga la deuda de `guards.ts`:** la lista de secciones se declara una sola vez en `shared/ui/navigation.ts` y la consumen el menú y la guarda. La pantalla carga al abrir (la ventana por omisión está acotada y los hallazgos son escasos, al contrario que el informe por periodo): filtros de periodo, departamento y regla; cuatro tarjetas —una por regla— con el recuento y **el umbral en palabras con el nombre del perfil** («Descanso mínimo entre jornadas: 12 h según el perfil ES-hosteleria»), la suspendida marcada «no se evalúa» con el motivo; tabla agrupada por regla con `HH:MM` desde minutos (`web-kit/workdayTotals`), la zona horaria del centro a la vista, la jornada abierta marcada con texto e icono (WCAG 1.4.1), enlace al registro de la persona (`/employees/{uuid}/workdays`) y a la incidencia cuando `incident` no es nulo; estado vacío «Sin alertas en el periodo» con los criterios; `meta.criteria` tal cual llegan. Nada se calcula en el panel (regla dura 7).
12. **Pruebas (§9.5).** Unitarias: `CompliancePolicyPredicatesTest` (11 h 59 / 12 h 00 en RN-10, 9 h 00 / 9 h 01 en RN-11, 5 h 59 / 6 h 00 / 6 h 01 en RN-12, 40 h 00 / 40 h 01 en RN-17, y el Gherkin «Perfil de cumplimiento distinto» con 10 h frente a 12 h) → `RN-10`, `RN-11`, `RN-12`, `RN-17`, `RF-PD-07`, `RF-PA-06`; `ComplianceEvaluationTest`: turno 22:00→06:00 seguido de entrada a las 18:00 mide 12 h desde el fin real sin partir (`RN-05`, `RN-10`), semana con cambio de hora —descanso entre el sábado 23:00 y el domingo 10:00 del cambio de marzo son 10 h reales y la semana no se desplaza— (`RN-09`), semana que empieza en domingo (`week_starts_on = 7`), jornada abierta marca y no alerta, regla suspendida no emite. Integración: `ComplianceFactsReaderVolumeTest` (500 empleados × 92 días: huecos correctos, jornada previa fuera del rango, `statement_timeout`) → `RN-10`; `ComplianceSummaryMatchesDetectionTest` (decisión 5) → `RN-10`, `RN-11`, `RF-PA-06`. Feature + contrato: `ComplianceSummaryTest` (forma, valores por omisión, `meta.rules` con la suspendida, `incident` enlazado, `criteria` en ES y EN, asiento de divulgación), `ComplianceSummaryScopeTest` (el responsable no ve otro departamento ni en `data` ni en `totals`; `department_id` ajeno → vacío) → `RF-ID-03`; fila nueva en `AuthorizationNegativeTest` (`empleado`, quiosco, `auditor` → `403`) → `RF-ID-03`, `RS-04`; sin licencia (decisión 9). Mutación sobre `ComplianceEvaluation` y los predicados (MSI ≥ 80 %). Panel: `ComplianceView.spec.ts` (Vitest), `compliance.spec.ts` E2E `@RF-PA-06` (umbral y perfil visibles, regla suspendida, estado vacío, enlace a la incidencia) y `accessibility.spec.ts` ampliado; paridad ES/EN.
13. **Documentación.** Doc 01 §4 (RN-17 y nota), §11 (escenario «Semana por encima de la jornada ordinaria») y Anexo A; doc 02 §8.2; `guia-rrhh.md` §4 bis / `hr-guide.md` «La vista de cumplimiento» (mismo número de apartados en las dos lenguas, `ClientDocumentationTest`); `docs/07` §6 no cambia (no hay riesgo nuevo aceptado). Ficha y `HANDOFF.md`.
14. **Lo que queda fuera y por qué:** el descanso **intra**-jornada de RN-10 y la reactivación de `missing_break` (3.5, pausa declarada); los festivos (3.10); la exportación de la vista (3.9, informes en diferido); el resumen semanal por correo (3.12); una alerta de Prometheus sobre la serie nueva (sin runbook no hay alerta).
15. **Lo que corrigieron las revisiones** (`seguridad-cumplimiento`, `revisor-codigo`, `qa-testing` y `ui-ux`, 16-09-2026, en solo lectura y en paralelo; todo aplicado en una segunda vuelta de los agentes de backend, panel y guías reanudados con su contexto): (a) **bloqueante** — `meta.totals` se calculaba sobre `data` ya filtrada por `rule`, así que pulsar una tarjeta ponía a cero las otras tres → totales del periodo entero se filtre o no, contrato corregido en `employees_affected`; (b) **bloqueante** — no había ningún caso del cambio de hora de **octubre** (marzo protege contra un falso negativo; octubre contra un falso positivo: una alerta de descanso a quien sí descansó doce horas) → tres pruebas (descanso, tramo que atraviesa el salto, semana de siete fechas); (c) `employees_evaluated` podía quedar por debajo de `employees_affected` por las semanas del borde → denominador sobre la ventana evaluada; (d) el runbook `brecha-de-seguridad.md` enumeraba nueve conjuntos de divulgación «y no hay más» y ya son diez → fila `compliance_summary` y `DisclosureDatasetsInventoryTest` (Architecture), que deriva los conjuntos del código y exige la fila del runbook; (e) la rama «responsable sin departamento» (`reachesNobody()` → `AND 1 = 0`) no la ejecutaba ninguna prueba y la sesión de portal faltaba en `SelfScopeIsolationTest` → dos casos; (f) **la excepción real a «cuenta lo mismo que la bandeja»**: cuando un tramo suelto dispara RN-08, la bandeja no abre la de RN-11 y la vista sí emite `daily_excess` enlazando esa incidencia → caso en `ComplianceSummaryMatchesDetectionTest` (que esquivaba el caso con `whereNull(shift_entry_id)`) y contrato matizado; (g) **retroactividad**: la vista recalcula siempre con el umbral vigente al consultar y la bandeja no se reprocesa → nota en doc 01 §4, descripción del endpoint y tabla bandeja/vista en la guía de RRHH §4 bis.6; (h) el ejemplo del contrato traía cuatro criterios inventados («desde el lunes» contradecía `week_starts_on`) → los cinco reales de `lang/es/compliance-summary.php`, también en el doble del E2E; (i) `compliance_metrics_week_start_seconds` se emitía sin catalogar → doc 02 §8.2 y `textfileSeries()`; (j) mutantes vivos y huecos de QA: dos personas pegadas al límite (12 h 00 / 11 h 59) en la comparación con la detección, fábrica `missingBreak()` (nunca ejercitada mientras RN-12 esté suspendida), semana cerrada sin `has_open_shift`, `firstInAt` nulo (tramos todos anulados), solape de un segundo, `from` suelto válido, `employee_uuid` inexistente → `200` vacío, comando de métricas idempotente; `ComplianceWeek::touches()` era código muerto → lo usa `weeksTouching()`; (k) panel: la pantalla del perfil seguía diciendo junto a la jornada semanal que «no la aplica ninguna regla» → aviso movido al calendario de festivos y nota «esto mueve la vista de cumplimiento, no la bandeja»; la guía prometía un enlace «al día del aviso» → el nombre lleva al registro con `from`/`to` de la jornada o la semana y `EmployeeWorkDaysView` lee el rango de la URL; `generated_at` en ISO crudo junto a la zona → `formatInstant` (también en el informe por periodo); zona horaria a la vista bajo los filtros; primera columna fija a 400 px; vocabulario unificado con el perfil («Tramo continuo máximo sin pausa», «between working days»), columna «Empleado»; aserción `wrapper.find(...).toBeTruthy()` que no podía fallar → `findAll().length`; `ComplianceFindingsTable` del 40 % al 87 % de ramas; E2E con la tarjeta de RN-17, la insignia «Turno abierto» y la zona horaria.
...
**Decisiones tomadas al ejecutar la tarea (17-09-2026).** Las que los documentos dejaban abiertas o que el estado real del repositorio obligó a fijar. Son la especificación común de los agentes que la ejecutan y quedan escritas también donde las lee quien las necesita (`openapi.yaml`, doc 01 §4 y Anexo B, `configuracion.md`, `hoja-empleado.md`, `guia-rrhh.md`). El análisis previo de huecos encontró que **casi toda la costura de ADR-024 ya existe desde la Fase 1** —`intent` en el contrato de `/scan`, `/scan/pin` y cada elemento de `/scan/batch`, en `scan_events` (con su `CHECK`) y en el registro Dexie de la cola; `ScanAction` y `scan_events.result` con `break_start`/`break_end`; el feedback del quiosco distingue las cuatro acciones (`isArrival`, `scan.action.break_*`); el agregado `WorkDay` ya pasa la prueba del turno 22:00→02:00 · pausa · 02:30→06:00 = 450 min—, pero **el servidor persiste `intent` sin actuar sobre él**: `RegisterScanHandler` decide solo por «hay tramo abierto» y nunca responde `break_start` ni `break_end`, y un `break_end` de madrugada carga la jornada por la fecha del escaneo y **parte el turno**, que es justo lo que ADR-024 prohíbe; el anti-rebote (`DebouncePolicy`) ignora la intención; **no existe ningún ajuste «fichaje de pausa» por centro** (RF-AT-12) ni canal por el que el quiosco lo reciba; el quiosco no ofrece la elección ni avisa del desfase en la pantalla de fichaje (solo lo mide en el latido, con una constante de 15 min, y lo enseña en el diagnóstico); y RN-12 sigue suspendida por una constante (`ComplianceRuleSuspension::SUSPENDED`). **El desfase de reloj, en cambio, ya está resuelto en el servidor**: se mide en cada escaneo, se persiste, marca `flagged_for_review` con el umbral de `ATTENDANCE_MAX_CLOCK_SKEW_MINUTES` y la pasada nocturna abre `clock_skew` (prueba en `IncidentDetectionTest`); lo que falta es el aviso en el quiosco.

1. **Contrato primero y aditivo (ADR-012, ADR-013).** No hay endpoint nuevo. `KioskHeartbeat` gana `break_clocking_enabled` (booleano) y `clock_skew_tolerance_seconds` (entero): el latido es el único canal autenticado que la tablet repite cada minuto (precedente de `service_code_hash`, 3.3), y la tablet guarda los dos en local para que funcionen sin red. `ScanIntent` corrige su descripción: **el servidor honra la intención declarada siempre**, esté o no activado el fichaje de pausa en la instalación (el ajuste gobierna la pantalla del quiosco y la evaluación de RN-12, no la verdad de lo que la persona declaró). `ComplianceRuleStatus.suspension_reason` pasa de `awaiting_declared_break` a **`break_clocking_disabled`** (decisión 8). `WorkDayShiftEntry` gana `opened_by` (`clock_in`|`break_end`) y `closed_by` (`clock_out`|`break_start`|`null` si sigue abierto), derivados de `scan_events.result` de los escaneos que abrieron y cerraron ese tramo (un tramo declarado o corregido a mano sin escaneo va como `clock_in`/`clock_out`), para que el detalle de jornada del panel y del portal enseñen «pausa» donde la hubo (consecuencia de ADR-024: «los informes… la tratan como pausa»). Sin cambio de esquema de `scan_events` ni de `shift_entries`.
2. **Qué es pausa y qué es fin de turno lo decide el dominio (reglas 1 y 2): `Attendance\Domain\Policy\ScanIntentPolicy`.** Entradas: la intención declarada, si la jornada tiene tramo abierto y **cuál fue el último escaneo aceptado** de la persona (`ScanLog::lastAcceptedScanOf()`, puerto nuevo: resultado, `shift_entry_uuid`, `occurred_at`). Tabla de resolución: con tramo abierto, `break_start` → **`BREAK_START`** (cierra), `auto` → `CLOCK_OUT`, `break_end` → `CLOCK_OUT` (contradice el estado: se cierra sin inventar una pausa que nadie declaró); sin tramo abierto, `break_end` → **`BREAK_END`** si el último escaneo aceptado fue `break_start` y `CLOCK_IN` si no hay pausa que continuar; **`auto` → `BREAK_END` si el último escaneo aceptado fue `break_start`** (la persona vuelve del descanso sin pulsar nada: es lo que hace que la vuelta no añada ningún paso, decisión 5) y `CLOCK_IN` si no; `break_start` → `CLOCK_IN` (no hay nada que cerrar). Una intención que contradice el estado **se resuelve por la estructura y se registra tal cual**: `intent` es lo que el quiosco pidió y `result` lo que se decidió (doc 01 §5.5); no se rechaza (regla 19), no se marca y no abre incidencia —una incidencia nueva exigiría tipo nuevo, migración y bandeja, y la divergencia ya queda escrita en `scan_events` para quien la revise—. El feedback del quiosco enseña siempre la acción que el servidor decidió.
3. **Un `BREAK_END` continúa la jornada del `break_start`, en cualquier día natural** (ADR-024 consecuencia 2, RN-05). El handler carga la jornada con `findWorkDayOfShiftEntry(uuid del tramo que cerró el break_start)` (puerto existente) en lugar de por la fecha del escaneo; un `break_end` del 15-03 a las 02:30 sigue en la jornada del 14-03 y el total del 15-03 es cero. Sin cambio en `WorkDay`: `clockIn` sobre la jornada correcta ya hereda `work_date`. `/scan/batch` procesa en orden de `occurred_at` y resuelve elemento a elemento con la misma política, de modo que un lote offline con `break_start` y `break_end` produce la misma atribución que en línea. `worked_minutes` de la respuesta sigue siendo el total de la jornada tras el escaneo, también en `break_start`.
4. **El anti-rebote considera la intención (ADR-024, paso 11)** y `ATTENDANCE_DEBOUNCE_SECONDS` se queda en 60 s, documentado. `DebouncePolicy::suppressorOf()` recibe los escaneos aceptados adyacentes **con su resultado** (`ScanLog::acceptedScansAdjacentTo()` devuelve `AcceptedScan{occurredAt, result}`), y **nunca suprime un escaneo con intención explícita que sea la inversa del escaneo aceptado más cercano**: un `break_end` a los 20 s de un `break_start` mal pulsado reabre el tramo (se pierden veinte segundos, no cuatro horas), y un `break_start` a los 20 s de un `break_end` vuelve a cerrar. Todo lo demás —`auto` tras cualquier cosa, dos `break_start` seguidos— conserva la ventana: el doble escaneo accidental sigue siendo lo que RF-AT-06 descarta. Límites probados en 59 s / 60 s con y sin intención.
5. **La elección en el quiosco: un botón «Pausa» que se arma antes de pasar la tarjeta, no una pregunta después.** Visible solo cuando `break_clocking_enabled` (latido); un toque lo arma («Pasa tu tarjeta para empezar la pausa»), el siguiente escaneo o PIN se encola con `intent: break_start` y el botón se desarma solo a los 10 s o tras el escaneo. **Nadie más añade un paso**: quien entra, sale o vuelve de la pausa solo pasa la tarjeta (`auto`) y el servidor resuelve (decisión 2). Se descartó preguntar «¿salida o pausa?» tras cada escaneo porque el quiosco no sabe, sin red y con varias tablets, si esa tarjeta tiene tramo abierto, y preguntárselo a todo el mundo en un cambio de turno es exactamente el paso que ADR-024 prohíbe añadir. Mismo botón en la vista de PIN. Objetivos ≥ 48 px, texto ≥ 24 px, operable con guantes, `aria-pressed`, anuncio en la región viva; `intent` se escribe **al encolar** (ya lo hace `scanPipeline`: pasa de `'auto'` fijo a la intención armada). Feedback: `break_start` → «Pausa {hora}» y `break_end` → «Vuelta {hora}» (textos que ya existen), con sonido y color de salida/entrada respectivamente y una frase que deje claro que la jornada sigue abierta.
6. **Aviso de desfase en el quiosco (RF-AT-10), sin bloquear ni asustar.** Dos fuentes, un mismo umbral (`clock_skew_tolerance_seconds` del latido, que sustituye a la constante `CLOCK_SKEW_WARNING_SECONDS`): (a) banda discreta y persistente en `ScanView` cuando el latido mide `|desfase| > umbral` («La hora de esta tablet va N min adelantada/atrasada; los fichajes se registran igual y el responsable lo revisará»), (b) en la confirmación de un escaneo respondido en línea, la misma frase si `|recorded_at − occurred_at| > umbral` (los dos instantes ya viajan en `ScanAccepted`; una confirmación de la cola sincronizada horas después no es desfase y no se avisa). Nada cambia en el camino de fichaje. El servidor no cambia: mide, persiste, marca y la nocturna abre `clock_skew` (ya existía); la prueba Feature de esta tarea recorre el Gherkin «Reloj del quiosco desviado» de punta a punta (escaneo con 40 min de adelanto → `200`, `occurred_at` del dispositivo, `flagged_for_review`, `attendance:detect-incidents` → incidencia `clock_skew` asignada).
7. **«Configurable por centro» = una clave de `installation_settings`** (ADR-040: un centro por instalación; RF-PD-01): `SettingKey::ATTENDANCE_BREAK_CLOCKING`, `SettingDefinition::choice('disabled', ['enabled', 'disabled'], SettingImpact::COMPLIANCE_REVIEW)` —`choice` y no un tipo booleano nuevo, que obligaría a ampliar `SettingType`, el contrato y el panel para una sola clave—. **`disabled` de serie**: el doc 05 lo vende como «opcional», y activarlo en una instalación cuya plantilla no ficha la pausa reactivaría RN-12 (decisión 8) contra gente que descansó sin fichar. `OperationalSettings` gana `breakClockingEnabled`. Editable en «Ajustes operativos» con su ayuda (`SettingsSurfaceTest` lo exige), fila en `configuracion.md` ES/EN, `.env.example`; el cambio queda auditado como todos los de impacto `COMPLIANCE_REVIEW` (`/revision-cumplimiento` C bis).
8. **RN-12 se reactiva cuando el fichaje de pausa está activado, y solo entonces.** `ComplianceRuleSuspension` deja de ser una constante y pasa a construirse desde la instalación: `ComplianceRuleSuspension::forInstallation(bool $breakClockingEnabled)` (instancia; `isSuspended()`, `suspended()`), con RN-12 suspendida mientras el ajuste esté en `disabled`. La razón es la de verdad: **«el fichaje de pausa está desactivado en esta instalación»** (`break_clocking_disabled`), no «esperando a la 3.5». Los cinco consumidores la reciben construida por quien tiene `OperationalSettingsProvider`: `DetectAttendanceAnomalies` (ya lo inyecta), `AnomalyType::opensIncident()`→ pasa a recibir la suspensión, `UpdateComplianceProfileHandler`/`ComplianceProfileField` (Product implementa el puerto), `ReadComplianceSummary`/`ComplianceEvaluation` (se le inyecta el puerto) y la pantalla del perfil (lee `ATTENDANCE_BREAK_CLOCKING` por `GET /settings`, que la misma potestad `settings:*` alcanza). Activar el ajuste hace que la siguiente pasada nocturna abra `missing_break` sobre los tramos cerrados de más de 6 h **sin retroactividad más allá de la ventana de detección**, y el asiento de `installation_setting.changed` lo advierte por su impacto; desactivarlo vuelve a suspenderla sin cerrar nada.
9. **RN-10 intrajornada: NO se evalúa, y se corrige la nota del doc 01 §4.** La nota prometía que «cuando exista la pausa declarada, RN-10 evaluará también el hueco intrajornada que no sea pausa». Ejecutarla convertiría la jornada partida —salir a las 15:00 y volver a las 19:00 sin pulsar «Pausa»— en un «descanso insuficiente» de cuatro horas, cuando el art. 34.3 ET mide el descanso **entre jornadas** y un hueco dentro de la misma jornada no lo es, se declare o no. Lo que la pausa declarada aporta es RN-12 (un tramo continuo deja de serlo) y el detalle del registro (decisión 1), no una segunda vara para RN-10. La nota se reescribe con este razonamiento; **es una desviación documentada respecto al doc 01 que el usuario debe conocer** (`HANDOFF.md`).
10. **Idempotencia y concurrencia sin novedades**: el reenvío devuelve la respuesta original con la misma `action` (`break_start` incluido), y la prueba de diez peticiones paralelas con el mismo `scan_id` se repite con `intent: break_start`. Sin migración: `scan_events.result` ya admite los dos valores (`scan_events_chk_result`) e `intent` existe desde la 1.3; **`/migracion-segura` no aplica** y se anota en la ficha (el paso 5 suponía que el `CHECK` no los tenía).
11. **ADR-023: no degradable** (es fichaje; regla dura 15). Sin `Feature`, sin consulta a la licencia, prueba `BreakClockingDoesNotDependOnLicenseTest` con el precedente de la 3.4. Instrumentación sin series nuevas: `scans_total{device,result}` cuenta `break_start`/`break_end` solo (paso 8); `incidents_open{type}` ya cubre `clock_skew` y `missing_break`.
12. **Pruebas (§9.5, cinco niveles más idempotencia concurrente).** Unitarias: `ScanIntentPolicyTest` (tabla completa de la decisión 2), `DebouncePolicyTest` ampliada (59/60 s con `auto`, `break_end` tras `break_start` a 20 s no se suprime, `break_start` tras `break_start` sí), `ReviewPolicyTest` 14:59/15:00/15:01 «en ningún caso se rechaza» (`RF-AT-10`, `RN-15`), `MidnightShiftTest` sigue en verde, RN-12 con y sin pausa a las 3 h (`RN-12`, `RF-AT-12`), `ComplianceRuleSuspensionTest` (activado → nada suspendido; desactivado → RN-12) y sus consumidores (`AnomalyType`, `ComplianceProfileField`, `ComplianceEvaluation`). Integración: `DatabaseConstraintsTest` con pausas de por medio (`RN-01`, `RN-02`), `IncidentDetectionTest` abre `missing_break` solo con el ajuste activado, un lote offline `break_start`+`break_end` sincronizado horas después produce la misma atribución que en línea (`RF-AT-12`, `RF-KI-03`). Feature + contrato: `RegisterScanTest` (`break_start` cierra y responde `break_start`; `auto` tras `break_start` responde `break_end` y continúa la jornada de madrugada; `break_end` sin pausa previa → `clock_in`; reenvío idéntico; sin `intent` → `auto`), `PinScanTest` (mismo con PIN), `ScanBatchTest` (respeta `intent` por elemento en orden de `occurred_at`), `ScanIdempotencyConcurrencyTest` con `break_start` (`RF-AT-07`, `RQ-03`), Gherkin «Reloj del quiosco desviado» de punta a punta (`RF-AT-10`), latido con los dos campos nuevos, `EmployeeWorkDaysTest`/`MyWorkDaysTest` con `opened_by`/`closed_by`, `OperationalSettingsTest` con la clave nueva y su asiento; autorización negativa: `/scan` con token de gestión/portal → `403` (ya existe: `ScanAuthorizationTest`; se añade la variante con `intent`). Quiosco: Vitest del botón «Pausa» (armado, desarme a los 10 s y tras el escaneo, `intent` en la cola, oculto sin ajuste), del aviso de desfase (umbral del latido, no constante) y del feedback; E2E con cámara simulada `@RF-AT-12` (armar «Pausa», escanear → `intent: break_start` en la petición, feedback «Pausa»; segundo escaneo → «Vuelta»; total visible) y `@RF-AT-10` (latido con desfase → banda visible y el fichaje sigue); `offline.spec` conserva `intent` en la cola. Panel y portal: Vitest de la marca de pausa en el detalle de jornada; `accessibility.spec` ampliado en los tres. Mutación sobre `ScanIntentPolicy`, `DebouncePolicy` y `ComplianceRuleSuspension` (MSI ≥ 80 %).
13. **Documentación.** Doc 01 §4 (nota de RN-10 reescrita, nota de RN-12 «reactivada cuando el fichaje de pausa está activado», RN-05 sin cambio), Anexo B (notas de contrato: latido, `WorkDayShiftEntry`, `ScanIntent` honrado siempre); doc 02 Anexo B (`ATTENDANCE_DEBOUNCE_SECONDS` revisado a la luz de la pausa; `ATTENDANCE_BREAK_CLOCKING`); `configuracion.md`/`configuration.md` (clave nueva y efecto sobre RN-12); `hoja-empleado.md`/`employee-sheet.md` §5.1–5.2 (cómo fichar la pausa y qué significa «Pausa»/«Vuelta»); `guia-rrhh.md`/`hr-guide.md` §4.1 (fila `missing_break`), §4 bis (RN-12 «no se evalúa mientras el fichaje de pausa esté desactivado») y §7; `guia-portal-empleado.md`/`employee-portal-guide.md` §2 (la marca de pausa); runbook `quiosco-no-responde.md` o `alta-nuevo-quiosco.md` (qué hacer ante la banda de desfase: poner la tablet en hora). Ficha y `HANDOFF.md`.
14. **Lo que queda fuera y por qué:** una incidencia por intención contradictoria (tipo nuevo; decisión 2); un botón «Fin de pausa» explícito en el quiosco (la vuelta es `auto`, decisión 5); RN-10 intrajornada (decisión 9); la pausa en los informes por periodo y en la exportación legal más allá de las marcas del tramo (la duración ya es correcta por construcción: la pausa no está en ningún tramo); reclasificar un escaneo ya registrado (se corrige con RN-13 como cualquier tramo).
15. **Lo que corrigieron las revisiones** (`seguridad-cumplimiento`, `revisor-codigo`, `qa-testing` y `ui-ux`, 17-09-2026, en solo lectura y en paralelo; todo aplicado en una segunda vuelta de los agentes de dominio, quiosco, panel —con `web-kit` y portal—, documentación y, al final, backend, reanudados con su contexto tras un corte por límite de sesión): (a) **bloqueante, visto por los tres revisores técnicos** — un `break_start` que nadie continuaba absorbía la jornada siguiente entera (`lastAcceptedScanOf()` sin cota: el `auto` del martes resolvía `BREAK_END` sobre el lunes, las ocho horas se cargaban al día equivocado, el martes quedaba a cero y ninguna incidencia lo veía) → **la continuación se acota con el descanso mínimo entre jornadas del perfil** (RN-10, `CompliancePolicy::minimumRestMinutes` por `CompliancePolicyProvider`, regla 14; `ScanIntentPolicy::allowingBreaksShorterThan()`, límite estricto: 11 h 59 continúa, 12 h 00 abre jornada; un escaneo anterior al `break_start` —cola desordenada— tampoco continúa); la **«pausa sin vuelta» no se detecta como incidencia** en esta tarea (tipo nuevo, migración y bandeja) y queda escrito en doc 01 §4, contrato y guía; (b) **bloqueante** — la vuelta de pausa sobre un tramo corregido (`superseded`) caía a `WorkDay::start(fecha civil)`: partía el turno de noche y dejaba `result`/`opened_by = break_end` sobre una jornada sin pausa → puerto nuevo `WorkDayRepository::findWorkDayOfAnyShiftEntry()` (un tramo retirado sigue identificando su jornada; el existente conserva su semántica para las correcciones), degradación explícita a `clock_in` con `flagged_for_review` si el `uuid` no existe, y el lector del detalle sigue la cadena de versiones para que una corrección no borre la marca de pausa; (c) la copia inmutable no distinguía pausa de fin de jornada (`scan_events` no es solo-append ni encadenada) → `ClockingAction` en `EmployeeClockedIn/Out` y `action` en el `payload` de `shift_entry.created/closed`; (d) un token de soporte con alcance `configuration` podía activar `ATTENDANCE_BREAK_CLOCKING` mientras `ComplianceProfilePolicy` se lo niega para el perfil → misma excepción en `SettingsPolicy` (documentada en `operacion.md` §12.4); (e) tres comparaciones del umbral de desfase con `>=` en el latido y `>` en servidor y banda → `exceedsClockSkewTolerance()` con `>` en las tres y pruebas 899/900/901; (f) la intención armada es de la tablet, no de la persona → se desarma al cambiar de pantalla y ante lecturas que no fichan, estado armado con acento, pictograma ⏸ y 24 px, desarme anunciado en la región viva; un PIN rechazado ya no consume la intención (se rearma para el reintento); (g) la regla «esto fue una pausa» estaba duplicada en panel y portal y ya divergía («0 min»/«90 min» frente a «1 h 30 min», `pause*`/`break*`, tokens e icono distintos) → `breakBetween()` y `BreakBadge.vue` en `web-kit` (ADR-036), mismo formato y tokens en las dos SPA; (h) el panel copiaba a mano el catálogo `enabled|disabled` y `Promise.all` bloqueaba el perfil por una lectura informativa → clave tipada con el enum del contrato y `allSettled` que cae a «desactivado»; (i) la nota de RN-10 del doc 01 afirmaba una conclusión legal cerrada («no incumple nada») que el modelo no puede sostener → reescrita en términos de lo que el sistema hace y no hace, «misma jornada» declarado como constructo de RN-05, y la validación remitida a la asesoría del cliente; misma advertencia en la guía de RRHH; (j) docblocks «en cuanto llegue la 3.5», `DeclaredIntent::isExplicit()` muerto, valores por defecto en `OperationalSettings` y `HeartbeatOutcome` (0 violaba `minimum: 60`), `lastAcceptedScanOf()` leído aunque hubiera tramo abierto, `replayDebounce()` reimplementando «el aceptado más cercano», constante `AHORA` duplicada entre suites (rompía `--group` sin `--testsuite`: el comando de verificación de la ficha se corrige), E2E del desarme con 0,7 s de margen, hoja impresa sin citar «Pausa»/«Vuelta» en `InstructionsSheetTextsTest`; (k) huecos de QA cerrados: fila 7 de la tabla con historial, «la bandeja y la vista cuentan RN-12 igual» con el ajuste activado, «desactivar no cierra las incidencias abiertas», tramo `superseded` en `ScanLogTest` y en Feature, carrera de dos tablets con intenciones opuestas. **Aceptado y anotado para el doc 07 en el cierre de fase:** `break_clocking_enabled` y la tolerancia viven en `localStorage` de la tablet (manipulables; consecuencia acotada: el servidor honra la intención y aplica su propio umbral; control compensatorio: modo quiosco sin devtools del runbook); la ventana de 10 s del botón «Pausa» como decisión de producto; que `qa:traceability` no exige aún los requisitos de Fase 3 hasta que `current_phase` suba al cerrarla.
...
php artisan test --testsuite=Unit --group=RF-AT-10 --group=RF-AT-12
php artisan test --testsuite=Feature --group=RF-AT-10 --group=RF-AT-12
...
> **Por qué va partido por suite y no en una sola orden.** `php artisan test --group=…` sin `--testsuite` carga **todas** las suites en el mismo proceso y aborta antes de ejecutar nada: hay una constante `AHORA` declarada en dos ficheros de prueba de suites distintas, y PHP falla al redefinirla. El síntoma es un error de arranque, no un fallo de prueba, así que se confunde con «el filtro no encuentra nada». Con `--testsuite` cada proceso carga solo su suite y el choque desaparece. (El renombrado de la constante lo hace el backend; mientras tanto, y para cualquiera que ejecute esto en una versión anterior, esta es la forma que funciona.)
...
| **Requisitos** | RNF-P-06 (doc 02 §11). RQ-08: «Prueba de carga que valida RNF-P-06 antes de cada versión mayor». Derivada: RN-18 «Fichaje irreconciliable» (decisión 20) |
...
**Decisiones tomadas al ejecutar la tarea (17-09-2026).** Las que los documentos dejaban abiertas o que el estado real del repositorio obligó a fijar. Son la especificación común de los agentes que la ejecutan y quedan escritas también donde las lee quien las necesita (`load-tests/k6/*`, `operacion.md`, `configuracion.md`, doc 02 §3.4, §9.6 y §10.1, doc 07 §6). El análisis previo de huecos encontró que **la herramienta ya existe desde la 1.7** (`load-tests/k6/run.sh`, `scan-p95.js`, `aggregate.js`, `provision-fixtures.php`: N contenedores de k6 —N orígenes, porque el borde y la aplicación limitan por IP a 600 r/m— con credenciales firmadas por el propio servidor y veredicto agregado sobre las muestras crudas), pero mide **un solo escenario** —fichajes válidos— y **casi todos son anti-rebote**: con 120 credenciales por instancia a 10 fichajes/s cada persona repite cada 12 s, dentro de los 60 s de RF-AT-06, así que la mayoría de las respuestas son `debounced` (camino barato, sin tramo ni proyección) y la cifra medida no era la del camino de escritura; además `__ITER` es por VU, de modo que dos VU fichaban a la vez con la misma tarjeta. No hay rechazos, ni reenvíos, ni lotes, ni el endpoint de cumplimiento, ni verificación posterior, ni etiqueta de requisito, ni ejecución en la CI; las dos zonas de Nginx del §7.1 sí están (`kronoqr.conf.template`, `KioskRateLimit`), el pool de FPM lleva un `pm.max_children = 20` «a ajustar en la 3.x» y ningún `lock_timeout` existe en ninguna capa.

1. **`scan-peak.js` sustituye a `scan-p95.js`** (`git mv`; es la ruta literal del §2 del doc 02 y de la ficha). `run.sh`, `aggregate.js` y `provision-fixtures.php` se conservan y amplían; `verify-after-load.php` (nuevo, se ejecuta como `provision-fixtures.php`: dentro del contenedor `app` vía `tinker`) es la verificación posterior. Todo bajo `load-tests/k6/`, sin dependencias nuevas: k6 sigue siendo la imagen `grafana/k6` y el agregado, Node sin paquetes.
2. **Modelo de carga: cada instancia de k6 es un origen (una IP) con su rebanada propia de tarjetas y de dispositivos**, y el pico se compone sumando orígenes, como en el diseño de la 1.7. Presupuesto por instancia **≤ 8 peticiones/s** (480/min: la zona de Nginx es un cubo con fuga —un permiso cada 100 ms, ráfaga 50— y los planificadores de los escenarios se solapan, así que hace falta un 20 % de margen, no el 10 % que bastaría contra la ventana fija de la aplicación), repartido en cinco escenarios de k6 con ejecutor `constant-arrival-rate`: `scan` 6/s (fichajes válidos, `intent: auto`), `resend` 0,5 iteraciones/s (cada iteración: un fichaje y su reenvío idéntico), `reject` 1/s, `batch` un lote de 50 cada 60 s y, en **una instancia aparte de «panel»** (otra IP, zona `api`), `compliance` 0,5/s. Con `INSTANCES=10` de quiosco salen **60 fichajes válidos/s ≥ 50** (RNF-P-06) y `DURATION=120s` por omisión. Los dispositivos se reparten por rebanada como las tarjetas (`DEVICES_PER_INSTANCE=8`, 8/8 = 60 peticiones/min por dispositivo, bajo los 120 de `KIOSK_SCAN_RATE_PER_DEVICE`); `run.sh` calcula y pasa `K6_DEVICES` y `K6_EMPLOYEES` al aprovisionamiento. El índice de iteración es el global del escenario (`k6/execution` → `scenario.iterationInTest`), nunca `__ITER`.
3. **Nadie repite tarjeta dentro de los 60 s del anti-rebote**: el aprovisionamiento garantiza `K6_EMPLOYEES ≥ INSTANCES × (6 × 66 + 60 + 100)` (≈ 5 600 con diez instancias) y **la población de la carga son solo los empleados que el propio script crea** (`full_name` «Carga k6 NNNN», código `K6NNNN`, sin correo ni PII; regla dura 21) en el departamento «Carga k6» del primer centro, que también crea: nunca reutiliza empleados activos ajenos —sobre una base restaurada de producción con códigos importados `K6…` habría revocado tarjetas de personas reales— y **aborta si existe un `K6…` fuera de ese departamento**. Además de negarse en producción exige `K6_ACKNOWLEDGE_TEST_DATABASE=yes`, porque `APP_ENV` no distingue una base vacía de un clon de producción. Al terminar la pasada, `run.sh` revoca las tarjetas y los tokens emitidos, desactiva la cuenta de gestión y borra los fixtures; los empleados sintéticos y su histórico se quedan. Así cada respuesta de `scan` es `clock_in`/`clock_out` de verdad —tramo, `daily_totals` y asiento— y el anti-rebote (`debounced`) queda por debajo del 5 %, que el agregado comprueba. Una instalación sin licencia o por encima de su plan **no bloquea nada** (ADR-019) y el aprovisionamiento lo avisa; **jamás contra producción** (ya lo rechaza).
4. **`resend` (regla dura 8, RQ-03, RF-AT-07):** la misma VU envía un fichaje y, entre 1 y 5 s después, **el mismo cuerpo con el mismo `scan_id` y la misma `Idempotency-Key`**; la comprobación exige el mismo código y un cuerpo idéntico al original. Un reenvío distinto es fallo de la prueba, no «respuesta inesperada».
5. **`reject` (RS-03, regla dura 17): tres clases de rechazo a partes iguales y comparadas entre sí**, no contra el acierto —`ConstantTimeFloor` iguala rechazos entre sí a propósito y un acierto hace más trabajo—: firma alterada (último carácter del payload), **desconocida** (payload bien formado y firmado por el servidor con un secreto que no existe: `unknown_payloads` del aprovisionamiento) y **revocada** (`revoked_payloads`: credenciales emitidas y revocadas al aprovisionar). Se espera `422` `problem+json` con el mismo cuerpo genérico salvo `scan_id`. Veredicto del agregado: el p50 de cada clase no se separa más de 20 ms de las otras y ninguno baja del suelo (`IDENTITY_CREDENTIAL_REJECTION_FLOOR_MS`, 25 ms). Es la «prueba de abuso: saturación del borde» que el doc 07 §5 (fila «Pruebas dirigidas por requisitos») aplazaba a esta tarea.
6. **`batch` (RF-KI-04, §6):** lotes de 50 elementos de tarjetas reservadas al escenario, con `occurred_at` repartidos en los diez minutos anteriores y **desordenados** (el servidor ordena), esperando `207` con 50 resultados aceptados o `debounced`. Un lote por minuto y por instancia (≈ 11 % de los fichajes): un quiosco que vuelve a tener red en mitad del cambio de turno, no todos a la vez.
7. **`compliance` (resto de la 3.4):** `GET /api/v1/compliance/summary` con el periodo por omisión, desde la instancia «panel» con el token de un `responsable_departamento` que emite el aprovisionamiento (usuario «Responsable carga k6», sin segundo factor por rol, alcance el departamento «Carga k6»; el token lo emite el servidor con las *abilities* del rol, nunca a mano). Lo que se mide es su p95 (**informativo**, presupuesto orientativo 2 s: ningún requisito lo fija) y, sobre todo, que **el fichaje sigue dentro de RNF-P-02 con esa lectura en curso**, porque cada lectura deja asiento bajo el candado de la cadena (ADR-010/ADR-027).
8. **Historial para `lastAcceptedScanOf()` (resto de la 3.5):** el aprovisionamiento inserta en bloque `K6_HISTORY_DAYS=365` días × dos escaneos aceptados (`clock_in` 06:00 y `clock_out` 14:00 UTC, `shift_entry_id` nulo como una fila importada, todos anteriores a ayer) para `K6_HISTORY_EMPLOYEES=200` de las tarjetas del escenario `scan` (~150 000 filas, segundos); y `verify-after-load.php` exige con `EXPLAIN (FORMAT JSON)` que las dos consultas calientes del fichaje —el último aceptado y la ventana anti-rebote— **usan `scan_events_employee_id_occurred_at_index` y no recorren la tabla**. Índices nuevos solo si ese plan lo pide: ninguno se añade por intuición.
9. **Verificación posterior (`run.sh` la ejecuta tras el agregado):** (a) `attendance:reconcile --from=ayer --to=hoy` con salida 0 y cero divergencias, y `projection_divergence_total 0` leído del fichero `.prom` del colector textfile; (b) para los empleados de la carga: ningún tramo abierto duplicado, tantos tramos vigentes de hoy como respuestas `clock_in`, y `scan_events` cuadrando con las respuestas (el UNIQUE de `scan_id` es la garantía; aquí se afirma); (c) ningún `full_name` de la carga en los logs del contenedor `app` (regla dura 21); (d) los planes de la decisión 8. Cita en su salida RN-06, RF-AT-07 y RQ-03, que ya tienen prueba Pest en la matriz (`ShiftChangeConcurrencyTest`, `ScanIdempotencyConcurrencyTest`).
10. **Veredictos del agregado, por requisito y con salida distinta de cero:** RNF-P-02 (p95 < 150 ms y p99 < 400 ms sobre `scan` + originales de `resend`, en conjunto y por instancia), RNF-P-06 (tasa de respuestas que producen tramo ≥ 50/s sobre la duración; `429`/`5xx` sobre fichajes válidos son «degradación encolable» —regla 19— y se toleran hasta el 0,5 %; cualquier `4xx` distinto a un fichaje válido es rechazo al empleado y falla), RQ-03 (100 % de reenvíos idénticos), RS-03 (decisión 5). `batch` y `compliance` se informan. Escribe `results/summary.json` legible por máquina y, con `--baseline load-tests/k6/baseline.json`, **avisa** (no falla) si el p95 empeora más de un 25 % sobre la línea base: el presupuesto es la puerta; la línea base, la tendencia.
11. **Etiquetas de requisito en k6: tercer formato del §9.6.** Cada escenario de `options.scenarios` lleva `tags: { requirements: 'RNF-P-06 RNF-P-02 RQ-08' }` (etiqueta nativa de k6, así viaja también en cada muestra del CSV y el agregado agrupa por ella). `TagScanner` gana la herramienta `k6` (`quality.test_paths.k6 = load-tests/k6`, ficheros `*.js`, nombre = clave del escenario, referencia `load-tests/k6/scan-peak.js:NN`), la matriz y el resumen la enumeran («Pest N, Playwright N, k6 N») y el doc 02 §9.6 documenta el formato. `scan` → RNF-P-06, RNF-P-02, RQ-08; `resend` → RQ-03, RF-AT-07; `reject` → RS-03; `batch` → RF-KI-04; `compliance` → RF-PA-06. `--check` no cambia de alcance (`current_phase` sigue en 5).
12. **Decisión 3-17 del plan: workflow propio, `.github/workflows/load-test.yml`**, no `release.yml` (que sigue siendo un marcador del plan 08) ni `ci.yml` (las etapas ①–③ deben seguir por debajo de 4 min y el pipeline no tiene etapa de carga: §10.1). Disparadores: `workflow_dispatch` (entradas `instances`, `duration`) y `push` de etiquetas **`v*.0.0`** (RQ-08: red de seguridad si alguien etiqueta una mayor sin haberla corrido; el procedimiento del plan 08 la ejecuta a mano antes de etiquetar). **Mide las imágenes de entrega**, no la de desarrollo: construye con `make build-ci-images` (caché `gha`), arma el paquete e instala como la etapa ⑧ (`prepare-package-env.sh`, `install.sh`), copia `load-tests/k6/*.php` al contenedor y ejecuta `run.sh` contra esa pila (`run.sh` admite `K6_COMPOSE_ARGS` para apuntar al compose del paquete; por omisión el de desarrollo). Sube `results/` como artefacto. El runner público tiene 4 vCPU/16 GB: el veredicto ahí es bloqueante para el workflow y **la cifra válida para el cliente sigue siendo la del hardware Linux de referencia**, como ya decía el cierre de la Fase 1.
13. **Pool de PHP-FPM configurable sin tocar la imagen (ADR-017):** `PHP_FPM_MAX_CHILDREN` (por omisión **20**, el servidor mínimo del §11.6.2; recomendado **40** con 4 núcleos y 8 GB: ~60 MB por trabajador) rendido por `entrypoint.sh` en `www.conf` para el rol `fpm`, con `start_servers`, `min_spare_servers` y `max_spare_servers` derivados (20 %, 10 % y 30 %). En `.env.example`, en las dos plantillas de compose y en `configuracion.md` ES/EN; con prueba de que el rendido produce un pool válido (`php-fpm -t`).
14. **`lock_timeout` e `idle_in_transaction_session_timeout` por `PGOPTIONS`** (libpq los lee del entorno del contenedor, pdo_pgsql incluido): `DB_LOCK_TIMEOUT=5s` y `DB_IDLE_IN_TRANSACTION_TIMEOUT=60s` en `.env.example`, inyectados como `PGOPTIONS="-c lock_timeout=… -c idle_in_transaction_session_timeout=…"` **solo al servicio `app`** (el que sirve peticiones: fichaje, panel, portal, y las migraciones vía `compose run app`) en `compose.dev.yaml` y `compose.prod.yaml`; no a `horizon`, `scheduler` ni `reverb`, porque `pg_dump` de la copia de seguridad y las tareas nocturnas largas no deben llevar los topes (revisión de seguridad). Convierten «todo el fichaje espera a una transacción colgada» en «algunos fallan y el quiosco encola» (regla 19), y valen también para el DDL de las migraciones (patrón de `/migracion-segura`). Sin `statement_timeout` global: los informes ya tienen el suyo. Los dos valores se validan en el entrypoint (`^[0-9]+(ms|s|min|h)?$`; inválido → salida 2 con «Que hacer»). Prueba: libpq honra `PGOPTIONS` (Integration, con `SHOW`) y los dos compose lo declaran en `app` y solo en `app` (Architecture).
15. **`make load-test`** envuelve `run.sh` (`INSTANCES`, `DURATION`) y aparece en `make help`; la ficha de la 1.7 y el cierre de la Fase 1 ya remitían aquí.
16. **Documentación:** `operacion.md`/`operation.md` §17 «Dimensionado del servidor y prueba de carga» (qué significa RNF-P-06 para un hotel —el borde admite una ráfaga de 50 y después 10 fichajes/s por origen, y el servidor sostiene 50/s con varios orígenes—, cómo correr la prueba contra un entorno de pruebas y **nunca contra producción** —crea empleados y credenciales—, cómo leer el veredicto, los mandos de las decisiones 13 y 14 y la línea base); `configuracion.md`/`configuration.md` con las tres variables nuevas; `instalacion.md`/`installation.md` §1 remite al dimensionado; doc 02 §3.4 (pool), §9.6 (k6) y §10.1 (el workflow de carga queda fuera de las ocho etapas, como la cobertura); doc 07 fila «Pruebas dirigidas por requisitos» y §6 (residual: el runner no es el hardware de referencia); plan README 3-17 ✅.
17. **Línea base:** `load-tests/k6/baseline.json` se escribe con el `summary.json` de la primera ejecución del workflow en el runner (commit propio, antes de la CI manual final) e incluye `git_sha`, fecha, runner e instancias. La ejecución local en Docker Desktop (~450 ms de p95 desde la 1.7) es informativa y no se versiona.
18. **Lo que corrigieron las revisiones (17-09-2026; `seguridad-cumplimiento`, `revisor-codigo`, `qa-testing` y `devops-observabilidad` en solo lectura).** Cuatro bloqueantes: **(a)** la población de la carga se identificaba solo por el prefijo `K6` del código de empleado —sobre una copia restaurada de producción con códigos importados `K6…` el script habría revocado tarjetas de personas reales, emitido otras y escrito fichajes en su registro— → decisión 3 reescrita: departamento «Carga k6» creado por el script, abortar si hay un `K6…` fuera de él, y confirmación explícita `K6_ACKNOWLEDGE_TEST_DATABASE=yes` además de negarse en producción; **(b)** el paso «Carga k6» del workflow corría sin `sudo` contra el `.env` del paquete, que `install.sh` deja a `root:root 0600` → `chown` al usuario del runner tras instalar; **(c)** `grafana/k6` corre con uid 12345 y en un runner Linux no puede escribir en `.results/` → `--user "$(id -u):$(id -g)"`; **(d)** los `429` de la pasada local estaban sin diagnosticar → el log de Nginx los atribuye a `limit_conn conn_per_ip 64` (19 182) y no a `limit_req` (451): artefacto de la saturación local (p95 44 s × 9 r/s ≈ 400 conexiones por origen); `run.sh` cuenta ahora las dos causas en la ventana de la pasada y las escribe en `summary.json`. Importantes: **`PGOPTIONS` alcanzaba a `pg_dump` y a las tareas nocturnas** (`scheduler`/`horizon`) → decisión 14 acotada al servicio `app`; el presupuesto de 9 r/s por origen se razonaba contra una ventana fija cuando Nginx es un cubo con fuga → decisión 2: 6 fichajes/s y 10 orígenes; RS-03 juzgaba el suelo sobre el p50 (bajo cola, cualquier suelo «se cumple») → sobre el mínimo de cada clase; `wait … || true` tiraba los `check()` de contrato de k6 → el agregado los lee del CSV; RNF-P-02 se medía mezclando los `429` de 0,5 ms → solo respuestas atendidas; RQ-03 salió verde con 16 pares de 458 → suelos relativos y salida 2 por debajo; el `EXPLAIN` de la verificación estaba sobre SQL transcrito a mano → SQL capturado del puerto real con `DB::listen`, y prueba de Integración `ScanLogIndexUsageTest`; `batch` no ejercitaba el orden por `occurred_at` (50 tarjetas distintas) → pares del mismo empleado invertidos; sin `verify-audit-chain` tras la carga → añadido (RS-07); sin cierre de la pasada (tarjetas y tokens vivos en disco y en el contenedor) → `trap` que revoca, desactiva y borra; sin prueba de `aggregate.js` → `aggregate.test.js` con `node --test`; nada garantizaba que un escenario etiquetado estuviera activo → `ScanPeakScenariosTest` (Architecture); la matriz decía «verifica en cada cambio» para k6 → cada entrada lleva herramienta y frecuencia, `TraceabilityCommandTest` deja de citar RNF-P-06, y `TagScanner` distingue el `->skip(<condición>)` del incondicional; `DB_LOCK_TIMEOUT` sin validar tumbaba todas las conexiones con `5 s` → validado en el entrypoint con «Que hacer», y techo de 500 para `PHP_FPM_MAX_CHILDREN`; `.results/` es oculto y `upload-artifact` lo excluía → `include-hidden-files`; la guía prometía «no publica si no se cumple» → «se mide antes de cada versión mayor como paso del procedimiento». **Defecto de producto destapado por la carga, fuera del alcance previsto y corregido aquí:** `attendance:reconcile` leía jornada y proyección sin instantánea ni candado y, si coincidía con fichajes en curso, pisaba `daily_totals` con la lectura vieja (tres empleados en la pasada; corregido en `ReconcileDailyTotals::correct()` releyendo bajo `FOR UPDATE` y escribiendo solo si la divergencia persiste, con prueba de concurrencia que fallaba antes). Deuda anotada en `HANDOFF.md`: elemento de lote con `503` (`ClockOutBeforeClockIn`) que la cola reintentaría para siempre; `[global]` del pool rendido frente al `php-fpm.conf.default` de la imagen base; `make help` roto en Git Bash por una línea anterior.
19. **El runner mide regresión, no el umbral (18-09-2026, tras las primeras ejecuciones reales de `load-test.yml`).** Medido en el runner de GitHub (4 vCPU, servidor y once generadores en la misma máquina): a 12 fichajes/s, p50 86 ms y p95 157 ms; a 60/s ofrecidos, **29 tramos/s sostenidos con p95 26 s** (5 218 rechazos por `limit_conn`, 0 por `limit_req`), y con `PHP_FPM_MAX_CHILDREN=40` exactamente igual (27,8/s): el cuello es la CPU compartida, no el pool. La verificación posterior salió íntegra en todas las pasadas (sin duplicados, proyección cuadrada, cadena intacta, regla 21 limpia; RQ-03 y RS-03 verdes cuando hay muestras). Conclusión: **un veredicto de umbral en el runner sería rojo para siempre y se dejaría de mirar**. Se corrige la decisión 12: en el workflow, `K6_LATENCY_VERDICT=baseline` hace que RNF-P-02 y RNF-P-06 se juzguen **contra la línea base del propio runner** (`load-tests/k6/baseline.json`: p95 de escritura ≤ +25 % y tramos/s ≥ −20 %, con los mismos parámetros; sin línea base o con parámetros distintos, «no evaluable»). **El perfil del runner son 3 instancias (18/s, ~60 % de su capacidad medida)**, no 10: a 60/s el runner se satura y deja RQ-03 y RS-03 sin muestras suficientes (78 pares comparables de 605; 0 de 3 clases de rechazo con 20 respuestas 422), así que el veredicto sería «no fiable» para siempre; a 18/s todo es medible y la línea base se toma con ese perfil, tal cual se instala (20 trabajadores). Las 10 instancias quedan para `make load-test` en hardware de referencia, y los demás veredictos (contrato, RQ-03, RS-03, verificación posterior, regla 21) siguen siendo bloqueantes. **El umbral de RNF-P-02/06 solo se juzga en el hardware de referencia del §11.6.2 con `make load-test`**, paso manual del procedimiento de publicación de una versión mayor (plan 08). Doc 02 §10.1, `operacion.md` §17, doc 07 A-11 y el README del plan (3-17) lo dicen así. Además, el workflow gana la entrada `fpm_max_children` para contrastar perfiles, y el aprovisionamiento completa la puesta en marcha mínima (centro y departamento con los casos de uso del asistente) cuando la instalación es limpia, sin cerrar el asistente ni asignar perfil.
20. **Tarea ad hoc derivada: «fichaje irreconciliable», RN-18 (18-09-2026, rama `feat/fichaje-irreconciliable`).** La carga destapó que un elemento de lote con `occurred_at` anterior al inicio del tramo abierto lanzaba `ClockOutBeforeClockIn`, caía en el `catch` de carreras, salía como `503` por elemento y el quiosco lo reintentaba para siempre sin que quedara fila en `scan_events` (regla dura 19 incumplida por no registrar). Es regla de negocio nueva y se ejecuta con la skill `/nueva-regla-de-negocio`: **RN-18** en doc 01 §4, §5.5, §11 y §13 y en `docs/requisitos.yaml`; la decisión vive en el agregado (`WorkDay::outOfOrderScanFor(acción, instante)`, resolución explícita, no en el `catch`) y cubre **las dos formas** —salida que no es posterior a la entrada del turno abierto (RN-03, límite estricto) y entrada que caería dentro o antes de un tramo ya cerrado (RN-02, intervalo `[inicio, fin)`; los tramos abiertos quedan fuera: son la carrera de RN-01)—, ampliación decidida en la revisión; `scan_events.result = rejected_out_of_order` con `flagged_for_review` (migraciones *expand* de los dos `CHECK`); **`422` con el cuerpo genérico** (ADR-012: los quioscos desplegados solo borran ante 200/422; un `409` nuevo no curaría nada) también por elemento del lote; incidencia **`out_of_order_scan`** (media) abierta por la pasada nocturna leyendo la columna **acotada por `recorded_at`** (los elementos atascados que drenan tras actualizar llegan con días de retraso y deben abrir incidencia), una por persona y jornada, resuelta con una corrección de RN-13; quiosco sin cambios de código (E2E «la cola llega a cero»), panel con la etiqueta del tipo, guía de RRHH §4.4 y runbook `cola-offline-atascada.md` §4; sin retroactividad (los elementos atascados se vacían solos en el siguiente reintento tras actualizar el servidor). Dictamen y decisiones en Engram (`attendance/fichaje-irreconciliable`).
...
1. Generar el vídeo `qr-video.y4m` con corrección de errores nivel Q (RF-QR-05, `QR_ERROR_CORRECTION=Q` del Anexo B). **«QR real» es el formato real emitido por el producto**, codificado por el **mismo codificador ZXing** que usa el quiosco para leerlo (`frontend-kiosk/scripts/generate-qr-fixture.mjs`), no un payload firmado por una clave viva: el job ⑦ corre sin backend (decisión 2) y no hay comando que emita uno. Lo que ata el fixture al producto es `backend/tests/Architecture/QrFixturePayloadShapeTest.php` (`@RQ-04`, `@RF-QR-05`), que firma un token aleatorio con `QrSigningKey::sign()` y comprueba que el payload por omisión del generador tiene exactamente esa forma —cuatro segmentos `FH1.<key_id>.<token>.<sig>`, 2 / 22 / 16 caracteres, alfabeto base64url sin relleno— y que el parser del producto lo acepta como bien formado. Si el formato cambia, la prueba señala el fixture en lugar de dejar el E2E en verde grabando un QR que ya no existe.
...
3. Escenario **QR degradado**: vídeo con el QR **parcialmente ocluido**, «para verificar que el nivel de corrección de errores Q cumple lo prometido» (§9.4). El doc 05 §5.2 se lo promete al cliente en estos términos: «la tarjeta sigue leyéndose con hasta un 25 % de deterioro». Esta prueba es la que sostiene esa frase. **Al ejecutar (18-09-2026): esa promesa se retira del doc 05 por decisión del usuario** —el producto no documenta tolerancia al desgaste; la tarjeta que no se lee se repone y mientras tanto se ficha con el PIN—; la prueba queda como regresión del decodificador, no como sostén de una promesa (decisión 3).
...
9. Integrar en el **job ⑦ de `.github/workflows/ci.yml`** (etapa ⑦ del §10.1, ~5 min). **No hay `e2e.yml` ni se crea** (decisión 7): el pipeline único se decidió al cerrar la Fase 5 y el árbol del §2 ya no lo lista.
...
- `frontend-kiosk/e2e/fixtures/qr-video.y4m`, el degradado (oclusión contigua, caso peor) y el desgastado (deterioro repartido por palabras de código, con la fracción medida en el README de fixtures y sin promesa al cliente, decisión 3); los tres se generan, no se versionan.
- `backend/tests/Architecture/QrFixturePayloadShapeTest.php`: el payload del fixture tiene la forma del que emite `QrSigningKey::sign()` (decisión 2).
- `frontend-kiosk/tests/e2e/`, `frontend-admin/…`, `frontend-portal/…`.
- Configuración de Playwright con las banderas del §9.4.
- Job ⑦ de `.github/workflows/ci.yml` (no existe `e2e.yml`).
...
**Decisiones tomadas al ejecutar la tarea (18-09-2026).** Las que los documentos dejaban abiertas o que el estado real del repositorio obligó a fijar. Son la especificación común de los agentes que la ejecutan y quedan escritas también donde las lee quien las necesita (`playwright.config.ts` del quiosco, `frontend-kiosk/e2e/fixtures/README.md`, doc 02 §9.1 y §10.1, doc 05 §5.2, `docs/requisitos.yaml`). El análisis previo de huecos encontró que **esta tarea es de cierre, no de construcción**: la cámara simulada con las banderas literales del §9.4 existe desde la 1.8 (`frontend-kiosk/playwright.config.ts`, tres proyectos de Chromium: `kiosk-qr`, `kiosk-qr-degraded` y `kiosk-layout` con vídeo en blanco), los `.y4m` los genera `scripts/generate-qr-fixture.mjs` con el mismo ZXing que lee el quiosco y nivel Q (2,64 MiB cada uno, ignorados por git y regenerados antes de cada ejecución), el ciclo offline con `occurred_at` original está en `offline.spec.ts` (`@RQ-05`), axe corre en las tres SPA con `wcag2a/aa/21a/21aa/22aa` filtrando críticas y graves, y hay **230 pruebas E2E** (quiosco 71, panel 130, portal 29; 226 etiquetadas) que el job ⑦ de `ci.yml` ejecuta en 3,5 min de reloj. Lo que no existe o está mal: **`RQ-04` figura cubierto por 14 pruebas que no tocan la cámara** (`SetupWizardTest:527` y 13 de accesibilidad del asistente en `frontend-admin/tests/e2e/accessibility.spec.ts`), **no hay ninguna E2E del bloqueo del PIN** (`@RS-12` con 0 usos en Playwright; `support/pin.ts` ni modela el `429`), el QR del fixture es el ejemplo del doc 02 §5.1 con firma inválida (el gancho `KIOSK_E2E_QR_PAYLOAD` nunca se conectó), **el 25 % de deterioro que promete el doc 05 §5.2 no lo sostiene la prueba** (`degraded.spec.ts` tapa un 7,8 % del área con un cuadrado contiguo y el decodificador falla a partir del 10,2 %), el doble canal sonoro del doc 01 §6.5 solo tiene prueba unitaria, el paso 9 de esta ficha pide `e2e.yml` cuando la decisión de un único `ci.yml` ya está tomada, y el código de pruebas arrastra cinco `waitForTimeout`, bucles y `if` en cuerpos de prueba, dos pruebas sin etiqueta y `@axe-core/playwright` sin declarar en el portal.

1. **`RQ-04` se reetiqueta donde está la cámara.** Las 13 pruebas del asistente pasan a `@RF-PD-03` (lo que verifican) y `SetupWizardTest:527` pierde `RQ-04`; `@RQ-04` va a las pruebas del quiosco que corren con `--use-file-for-fake-video-capture` (`scan.spec.ts`, `degraded.spec.ts`, el proyecto `kiosk-qr`). Sin esto, al pasar `current_phase` a 3 la puerta daría verde con evidencia falsa.
2. **«QR real» significa el formato real, no una firma verificada por un servidor que el job ⑦ no tiene.** Se conserva la decisión de la 1.8 de que ⑦ corre sin backend (`page.route`): levantar PostgreSQL y PHP solo para firmar un payload rompería el presupuesto y no añadiría nada que el E2E pueda comprobar. Lo que sí se ata: una prueba de contrato en Pest (`QrFixturePayloadShapeTest`, `->group('RQ-04','RF-QR-05')`) firma con una clave de prueba y afirma que un payload emitido por `QrSigningKey::sign()` tiene exactamente la forma del payload por omisión del generador (cuatro segmentos `FH1.<key_id>.<token>.<sig>`, longitudes de base64url); si el formato cambia, la prueba señala el fixture. El gancho `KIOSK_E2E_QR_PAYLOAD` se queda documentado y sin conectar: no existe ningún comando de consola que emita un payload firmado (comprobado), y crearlo solo para el E2E sería superficie nueva sin uso de producto. La ficha corrige su redacción en el paso 1.
3. **Deterioro repartido: las dos cosas, y con la cifra que el decodificador sostiene, no la del folleto.** El generador gana un modo de degradación **repartida** (`KIOSK_E2E_QR_WEAR`: módulos invertidos al azar con semilla fija, determinista) que produce `qr-video-worn.y4m` y un cuarto proyecto `kiosk-qr-worn` con `worn.spec.ts` (`@RF-QR-05`, `@RQ-04`); la oclusión contigua del 28 % del lado se queda como caso peor. **Medido al ejecutar (18-09-2026): el 25 % «repartido» NO decodifica nunca.** El 25 % del nivel Q es la redundancia de Reed-Solomon, y ante errores de posición desconocida (un módulo sucio no es un borrado conocido) el decodificador corrige como máximo la mitad: con 196 palabras de código (versión 7, Q) decodifica hasta 22 corrompidas (11,2 %) y falla desde 24 (12,2 %), medido con `@zxing/library` replicando el orden de colocación. El fixture queda en `0.10` (20/196, con margen) y el `README.md` de fixtures lo documenta con las cifras. **Decisión del usuario (18-09-2026): el producto NO documenta ninguna tolerancia al desgaste** —el soporte físico (papel, cartón, plástico) lo elige el cliente y no se controla—: una tarjeta que no se lee se repone y, mientras tanto, se ficha con el PIN de respaldo (RF-AT-11). La promesa del 25 % se retira de doc 05 §5.2, ADR-005, doc 04, doc 02 §5.1 y doc 01 (corrección editorial, no de decisión); las cifras medidas viven solo en `frontend-kiosk/e2e/fixtures/README.md` como evidencia de la prueba. **Además, la revisión de seguridad encontró que la medición inicial estaba hecha sobre un símbolo que no existe** (el generador contaba la zona tranquila: 45 módulos → símbolo real de 37 = versión 5 con 134 palabras, no versión 7 con 196; cinco celdas invertidas caían en el margen y cuatro sobre patrones estructurales): se rehace mapeando al símbolo real y con asertos en el generador.
4. **Bloqueo del PIN (RS-12, RF-AT-11): E2E nuevas contra el comportamiento real, que es el seguro.** Corrección al redactar esta decisión: el contrato (`/api/v1/scan/pin`) ya dice que el bloqueo es una de las cinco causas que comparten el **mismo `422` genérico**; `PinVerification::locked()` no expone `retryAfterSeconds` y `AuthFailureReason::LOCKED` solo se emite en el panel. Un `429` con `Retry-After` en el quiosco sería el oráculo que la regla dura 17 prohíbe, así que no hay cuenta atrás ni `page.clock`. Corrección tras la revisión QA: `support/pin.ts` NO modela ningún bloqueo (el doble de siempre, `stubPinScanApi(page, 'rejected')`, ya es el comportamiento real: un `422` idéntico), solo gana el helper `consecutiveRejections(page, n)` para que el bucle no viva en el cuerpo de la prueba; `pin-lockout.spec.ts` se queda con UNA prueba (`['@RS-03', '@RS-12', '@RF-AT-11']`: tras N rechazos consecutivos el cuerpo sigue siendo el genérico y el quiosco no exhibe ningún oráculo de bloqueo —ni minutos ni «bloqueado»—) más un **control negativo** con el oráculo prohibido en el doble (que demuestra que la aserción puede fallar); las dos pruebas iniciales que eran subconjunto de esa o duplicaban `pin.spec.ts` se borran. RS-12 habla literalmente del PIN **del portal**: la E2E que recorre ese bloqueo de extremo a extremo es `frontend-portal/tests/e2e/login.spec.ts` (`rateLimited`), que gana la etiqueta `@RS-12`; en Pest lo cubre `PortalPinLockoutTest`. `src/` intacto.
5. **Doble canal sonoro (doc 01 §6.5, RF-KI-06): E2E que afirma que suena.** Se instala con `page.addInitScript` un doble de `AudioContext`/`HTMLMediaElement.play` que registra las llamadas, y `scan.spec.ts` afirma que el fichaje aceptado y el rechazo disparan el canal sonoro con tonos distintos, además del visual. Es la única forma de probarlo sin altavoz. **Al ejecutar se descubrió que el fichaje con tarjeta nunca sonaba diferenciado**: `scanPipeline` pinta siempre «pendiente» primero (RNF-P-03) y `useScanSession.settle()` no reproducía el desenlace, así que los tonos `entry`/`exit`/`error` eran código muerto para el QR. Se corrige en esta tarea con la opción B de `ui-ux`: tono de error al asentarse un rechazo (`rejected`/`unreadable`), sin doble pitido en aceptado; unitaria de `useScanSession` ampliada y E2E que lo afirma.
6. **El ciclo offline «completo» sigue siendo del lado del quiosco en ⑦**; la consolidación en servidor con el `occurred_at` original ya la prueban `ScanBatchTest` y `ScanIdempotencyConcurrencyTest` (Feature). La ficha lo dice; no se levanta backend en ⑦.
7. **Paso 9 corregido: no hay `e2e.yml`.** ⑦ vive en `ci.yml` (decisión del cierre de la Fase 5, cabecera de `ci.yml`); el artefacto de la ficha pasa a «job ⑦ de `ci.yml`». El árbol del doc 02 §2 ya no lo lista (comprobado).
8. **§9.1 «~25 escenarios» se reinterpreta, no se recorta la suite:** doc 02 §9.1 pasa a decir «~25 recorridos críticos de usuario más la rejilla de accesibilidad y las variantes por pantalla»; la lista de recorridos críticos se escribe en la ficha, mapeada a specs, y es lo que la CI no puede perder.
9. **Presupuesto de ⑦: 300 s por app sigue siendo aviso, no fallo**, con `workers: 1` y `fullyParallel: false` en el quiosco (la cámara falsa y el bucle de decodificación son estado global). El cuarto proyecto y las pruebas nuevas dejan el quiosco por debajo de ~210 s. **`retries` pasa a 0 en las tres apps**: una prueba intermitente es un defecto y se arregla, no se reintenta (criterio §9.2 «cero pruebas intermitentes»); si aparece, se gana la carrera declarando la condición (como `delayCameraStart()`), nunca con un `sleep`.
10. **Deuda del §3.5 que esta tarea paga:** los cinco `waitForTimeout` se sustituyen por esperas por condición o `page.clock` (el de la pulsación larga de 3 s del diagnóstico se queda con `page.clock.runFor` si `keydown`/`keyup` lo admiten; si no, queda documentado como gesto); los `if` y bucles con lógica salen del cuerpo de las pruebas a *helpers* o a *datasets*; las dos pruebas de `shell.spec.ts` quedan SIN etiqueta a propósito (corrección al ejecutar: no existe requisito con identificador para la navegación del panel —RF-PA-01 es la presencia en vivo—; inventarlo falsearía la matriz; darlo de alta en doc 01 es otra tarea); `@axe-core/playwright` se declara en `frontend-portal/package.json`.
11. **Vitest 5 se migra dentro de esta tarea, como primer paso y con su propio agente**, porque el lock hay que regenerarlo una sola vez desde Linux sin `node_modules` (`node:24-alpine`, `npm install --package-lock-only --ignore-scripts`) y el portal necesita ese mismo lock para declarar axe. Los cuatro motivos del HANDOFF ya no son los reales (Node 24 y Vite 8 cumplen; `sequential` y `toHaveTextContent` no se usan); lo que hay que revisar es: mocks limpiados por defecto antes de cada prueba (46 `toHaveBeenCalledTimes`, 3 `vi.mock`), `@sinonjs/fake-timers` nuevo (44 `vi.useFakeTimers`), aserciones asíncronas sin `await` (14 `.resolves`/`.rejects`) y 9 `.each(`. Se levanta el `ignore` de `dependabot.yml`. Se verifica con ① y ⑥ de los cuatro paquetes y el guarda «mantiene en el lock los binarios nativos» de `QualityGatesTest`.
12. **Comprobaciones en tablet real siguen siendo manuales** (una mano, guantes, luz baja, TalkBack, `getSettings()` con `focusMode`/`zoom`/`backgroundBlur`) —más las tres que añadió la revisión de `ui-ux`: sonido con extractores de cocina a volumen de despliegue, una tarjeta físicamente desgastada de una temporada (qué ve la persona cuando no decodifica nada), y legibilidad del texto de confirmación a contraluz—: se recogen en la ficha y en `HANDOFF.md`, no se fingen en CI.
13. **Trazabilidad:** `current_phase` no cambia (sigue en 5 hasta cerrar la Fase 3). Los seis requisitos de la Fase 3 sin prueba (RF-GP-04, RF-IN-06/07, RF-KI-07, RF-PR-05, RN-16) son de 3.9-3.12 y se anotan; `RS-11` con una sola prueba sin relación se anota para la 3.8.

**Recorridos críticos (lo que la CI no puede perder).** Es la lista que el §9.1 llama «~25 escenarios» y que la decisión 8 reinterpreta: recorridos de usuario, no pruebas. Cada uno se mapea a la *spec* real que lo cubre hoy —`frontend-kiosk/tests/e2e/`, `frontend-admin/tests/e2e/`, `frontend-portal/tests/e2e/`— y lo que no existe se dice, con el requisito que se queda sin recorrido. Son 27 filas para un «~25»: la cifra del §9.1 es un orden de magnitud, no un contrato.

| # | Recorrido | Spec | Etiquetas | Estado |
|---|---|---|---|---|
| **Quiosco** | | | | |
| 1 | Fichar acercando la tarjeta a la cámara, sin tocar la pantalla | `kiosk/scan.spec.ts` (proyecto `kiosk-qr`) | `@RF-KI-01` `@RF-KI-02` `@RQ-04` | Hecho (`@RQ-04` añadido, decisión 1) |
| 2 | Nunca bloquea al empleado: confirma en < 300 ms, sin red, con anti-rebote y con rechazo genérico | `kiosk/scan.spec.ts` | `@RF-AT-05` `@RF-KI-02` | Hecho (en el quiosco no queda ninguna prueba sin etiqueta) |
| 3 | PIN de respaldo cuando no hay tarjeta, con red y sin ella | `kiosk/pin.spec.ts` | `@RF-AT-11` `@RQ-05` `@RS-03` | Cubierto |
| 4 | Bloqueo del PIN por intentos: el rechazo sigue siendo el genérico sin cuenta atrás (RS-03), control negativo con el oráculo prohibido en el doble, y vuelta al servicio | `kiosk/pin-lockout.spec.ts` | `@RS-12` `@RF-AT-11` | **Hecho** (decisión 4) |
| 5 | Ciclo offline completo: fichar sin red, cola en IndexedDB, reconectar y consolidar con el `occurred_at` original | `kiosk/offline.spec.ts` | `@RQ-05` `@RF-KI-03` `@RF-KI-04` | Cubierto |
| 6 | Tarjeta con una zona tapada de una pieza: oclusión contigua, el caso peor | `kiosk/degraded.spec.ts` (`kiosk-qr-degraded`) | `@RF-QR-05` `@RQ-04` | Hecho (`@RQ-04` añadido, decisión 1) |
| 7 | Tarjeta desgastada con deterioro **repartido** por palabras de código (fracción medida en el README de fixtures; sin promesa al cliente por decisión del usuario) | `kiosk/worn.spec.ts` (`kiosk-qr-worn`) | `@RF-QR-05` `@RQ-04` | **Hecho** (decisión 3) |
| 8 | Emparejar una tablet nueva y que su primer fichaje viaje firmado con ese token | `kiosk/pairing.spec.ts` | `@RF-PD-06` | Cubierto |
| 9 | Abrir el diagnóstico con el código de servicio, también sin red, y volver al fichaje | `kiosk/diagnostics.spec.ts` | `@RF-KI-08` `@RF-PD-06` | Cubierto |
| 10 | Fichar una pausa y seguir fichando con el reloj desviado, con aviso y sin bloqueo | `kiosk/break.spec.ts` | `@RF-AT-12` `@RF-AT-10` | Cubierto |
| 11 | La tablet con la marca del cliente, y que sobreviva a quedarse sin red | `kiosk/branding.spec.ts` | `@RF-PD-08` `@RF-KI-03` | Cubierto |
| **Panel** | | | | |
| 12 | Entrar con contraseña y segundo factor, incluida el alta del TOTP la primera vez | `admin/login.spec.ts`, `admin/two-factor.spec.ts` | `@RF-ID-01` `@RF-ID-02` `@RS-06` | Cubierto |
| 13 | Presencia en vivo que se actualiza sola, y que sigue actualizándose con Reverb caído | `admin/live-presence.spec.ts` | `@RF-PA-01` `@RF-PA-02` | Cubierto |
| 14 | De la plantilla a la ficha y de la ficha al registro horario de una persona | `admin/workdays-journey.spec.ts` | `@RF-GP-01` `@RF-PA-03` | Cubierto |
| 15 | Corregir una jornada: añadir un tramo, cambiar la hora y anular, con motivo e historial | `admin/corrections.spec.ts` | `@RF-PA-04` `@RN-13` | Cubierto |
| 16 | Bandeja de incidencias: filtrar, resolver y llegar hasta ella desde el detalle de la jornada | `admin/incidents.spec.ts` | `@RF-PA-05` `@RN-18` | Cubierto |
| 17 | Informe del periodo y su exportación en los tres formatos | `admin/period-report.spec.ts`, `admin/period-report-export.spec.ts` | `@RF-IN-01` `@RF-IN-02` `@RF-IN-04` | Cubierto |
| 18 | Exportación íntegra de los datos del cliente, de la petición a la descarga | `admin/data-export.spec.ts` | `@RF-PD-14` `@RL-20` | Cubierto |
| 19 | Credenciales: registrar la entrega, descargar la hoja en cada idioma y seguir una rotación de clave | `admin/credential-instructions-sheet.spec.ts`, `admin/credential-rotation.spec.ts` | `@RL-05` `@RF-QR-07` `@RF-QR-08` | Cubierto |
| 20 | Quioscos: vincular, desvincular y encontrar uno sin latido con lo que hay que hacer | `admin/devices.spec.ts` | `@RF-PD-06` `@RF-PA-07` | Cubierto |
| 21 | Cumplimiento: leer los hallazgos y cambiar el umbral del perfil con su aviso | `admin/compliance.spec.ts`, `admin/compliance-profile.spec.ts` | `@RF-PA-06` `@RF-PD-07` | Cubierto |
| 22 | Asistente de puesta en marcha de la instalación vacía al panel listo, abandonando y retomando | `admin/setup-wizard.spec.ts` | `@RF-PD-03` `@RF-PD-04` `@RF-GP-05` | Cubierto |
| 23 | Operación del IT del cliente: ajustes operativos, marca, paquete de soporte e histórico de errores | `admin/settings.spec.ts`, `admin/branding.spec.ts`, `admin/support.spec.ts`, `admin/errors.spec.ts` | `@RF-PD-01` `@RF-PD-08` `@RF-PD-09` `@RF-PD-11` `@RF-PD-15` | Cubierto |
| 24 | Licencia: activarla fuera del asistente, renovarla y ver la degradación honesta al caducar | — | `@RF-PD-04` `@RF-PD-05` | **Pendiente**: solo hay pasada de `axe` sobre la pantalla (`admin/accessibility.spec.ts`); ningún recorrido la activa ni la caduca, y la **regla dura 15** (la caducidad jamás bloquea el fichaje) no tiene E2E ninguno. Propuesta: `admin/license.spec.ts` con tres pruebas `@RF-PD-04 @RF-PD-05` (activar fuera del asistente, renovar, caducar y ver qué se degrada) más una del quiosco que fiche con la licencia caducada (`producto-licencia` + `qa-testing`) |
| **Portal** | | | | |
| 25 | Entrar con el código de empleado y el PIN, y quedarse fuera tras varios fallos | `portal/login.spec.ts` | `@RF-ID-06` `@RF-ID-07` `@RL-05` | Cubierto; el bloqueo se prueba pero no lleva `@RS-12` |
| 26 | Ver mis jornadas: totales en horas y minutos, corrección visible, turno abierto y datos ajenos inalcanzables | `portal/my-records.spec.ts` | `@RF-ID-05` `@RF-ID-07` `@RL-04` `@RL-05` `@RN-13` | Cubierto |
| 27 | Descargar mi histórico del periodo que elija | `portal/my-export.spec.ts` | `@RF-ID-05` `@RL-05` | Cubierto |

**Lo que NO está en la tabla y también es obligatorio.** No son recorridos y por eso el §9.1 los cuenta aparte: la **rejilla de accesibilidad** (`accessibility.spec.ts` de las tres aplicaciones, más las pasadas de `axe` incrustadas en `pin.spec.ts`, `pairing.spec.ts` y `diagnostics.spec.ts`), las **variantes por pantalla** (idioma, marca del cliente y del producto, rol sin ámbito, sin red) y las pruebas que vigilan un presupuesto o una costura técnica: `kiosk/qr-decode-budget.spec.ts` (`@RNF-P-03`), `kiosk/traceparent.spec.ts`, `kiosk/heartbeat-errors.spec.ts`, `kiosk/layout.spec.ts`, `admin/shell.spec.ts` y `portal/client-errors.spec.ts`. El **doble canal visual y sonoro** del doc 01 §6.5 pertenece a esa rejilla, no a un recorrido, y hoy solo tiene prueba unitaria (decisión 5).

**Comprobaciones en tablet real (manuales).** La decisión 12 las deja fuera de la CI a propósito: se hacen sobre la tablet del quiosco con la tarjeta impresa, antes de dar la tarea por cerrada, y se repiten en cada alta de quiosco (`docs/runbooks/alta-nuevo-quiosco.md`). No se fingen con un doble.

- **Con una sola mano**, la tablet en su soporte: acercar la tarjeta y fichar sin sujetar nada más.
- **Con guantes** (cocina, limpieza, cámara frigorífica): el botón de PIN y el teclado numérico siguen siendo pulsables — los 48 px del doc 01 §6.5 se miden con el dedo enguantado, no se estiman.
- **Con luz baja** y con contraluz: hasta qué distancia decodifica, y que el aviso de confirmación siga siendo legible.
- **Con TalkBack activado**: la confirmación se anuncia, el foco no se pierde al cambiar de pantalla y el sonido no es el único canal.
- **`getSettings()` de la pista de vídeo en la pantalla de diagnóstico**: `focusMode`, `zoom` y `backgroundBlur`. Un `backgroundBlur: true` o un `zoom` distinto de 1 explican un QR recortado o borroso que **no es del código** (la trampa de «Windows Studio Effects» del portátil; en tablet se mira igual antes de acusar al producto).

El resultado se anota en la ficha y en `HANDOFF.md`. Si una de estas comprobaciones falla, es un defecto del producto, no una anécdota del aparato.
...
- Autorización negativa por endpoint y rol; asiento `access.denied` en `audit_log` cuando la denegación es por alcance (`ScopeGuard`), no por rol ni por ámbito de token (decisión 13) → `->group('RS-05', 'RF-ID-03')`.
...
**Decisiones tomadas al ejecutar la tarea (22-09-2026).** Las que los documentos dejaban abiertas o que el estado real del repositorio obligó a fijar. Son la especificación común de los agentes que la ejecutan.

1. **La revisión externa la hace un tercero; esta tarea la prepara, no la sustituye.** RS-11 exige una revisión *externa* antes de la primera versión comercial y el repositorio no puede fabricarla. Lo que esta tarea entrega es (a) la revisión interna previa al mismo nivel (ASVS 2, con nivel 3 en auditoría) y con el mismo formato de hallazgo, (b) el paquete para el revisor, (c) la evidencia automática del §9.2, (d) las correcciones de los hallazgos internos con su prueba de no regresión, y (e) la programación anual. RS-11 queda **preparada y pendiente del tercero**, y así lo dice el informe, doc 07 §6 y `HANDOFF.md`. Nada se declara «superado» sin el informe externo.
2. **Los informes de seguridad viven en `docs/seguridad/`** (nuevo; el «⚠️ decidir» de la ficha): `revision-interna-asvs-<AAAA-MM>.md` (hallazgos y cierre), `paquete-revisor.md` (índice del paquete: qué documentos, qué evidencia, qué NO se entrega) y `evidencia/` con la salida de las herramientas del paso 4, sin secretos. **Nunca entra en el paquete que se entrega al cliente**: `package.sh` copia solo `docs/cliente` y `docs/runbooks`, y `QualityGatesTest` lo ata con una guarda para que nadie lo añada por descuido. El detalle explotable de un hallazgo abierto se redacta al mínimo hasta que se corrige.
3. **`seguridad-cumplimiento` no escribe: enuncia, enruta y verifica.** Se resuelve la contradicción que la propia ficha señala a favor del doc 03 §1.2: el agente hace la revisión previa (pasos 1 y 2), redacta el informe, asigna cada hallazgo a un agente y verifica el cierre (paso 7); las correcciones las ejecutan `backend-laravel`, `devops-observabilidad`, `producto-licencia`, `frontend-*` y `qa-testing` escribe las pruebas que faltan. Doc 02 §11 corrige la columna a «`seguridad-cumplimiento` (preparación y verificación) + agente de cada hallazgo».
4. **Cada hallazgo corregido lleva una prueba que falla antes de la corrección**, en el nivel del §9.5 y con el `->group()` del requisito; el informe registra el nombre de la prueba junto al cierre. Un hallazgo sin prueba se queda abierto en el informe, no se cierra «por inspección».
5. **La evidencia automática del paso 4 se genera con los objetivos del `Makefile` que ya usa la CI** (`deps-audit-php`, `deps-audit-js`, `sast`, `sast-community`, `trivy-fs`, `secrets-scan`; `trivy-image` sobre las imágenes de entrega la aporta el job `security`), y su salida resumida se guarda en `docs/seguridad/evidencia/` con fecha y commit. No se crea herramienta nueva para esto.
6. **DAST: se aplaza con dueño y fecha, no se finge.** Doc 07 §6 lo asignaba a la 3.8 sin tarea. `qa-testing` lo midió contra la superficie real: una API con *Bearer* de Sanctum y tres SPA del mismo origen, donde un ZAP *baseline* encuentra sobre todo cabeceras que RS-09 ya ata con prueba. Se anota en doc 07 §6 como pendiente de `devops-observabilidad` para el cierre de la Fase 3 (`make dast` manual, fuera de `ci.yml` por presupuesto), y el paquete del revisor lo declara como no cubierto. Si el revisor externo lo exige como entregable, se adelanta.
7. **RS-11 en la trazabilidad se sostiene con evidencia documental, no con una prueba de negocio.** La etiqueta `RS-11` de `CorruptSettingsDoNotBlockClockingTest` es un error (la prueba es de RF-PD-01 y regla dura 15) y se retira; la sostiene una prueba de arquitectura `SecurityReviewEvidenceTest` (`->group('RS-11')`) que exige que exista el informe de revisión interna con sus secciones, el índice del paquete del revisor, la fecha de la próxima revisión en doc 02 §11.6.5, y que ningún hallazgo `BLOQUEANTE` del informe figure sin cierre. Es el mismo patrón que `ClientDocumentationTest` usa para RL-21.
8. **La repetición anual se anota en doc 02 §11.6.5** («la revisión de seguridad externa se repite cada 12 meses y en cada versión mayor; próxima: antes de la primera versión comercial, y a los 12 meses de esa fecha») y en `docs/seguridad/paquete-revisor.md`. No hay tarea programada en código: es un compromiso de publicación, no un cron.
9. **`docs/runbooks/brecha-de-seguridad.md` ya existe** (RL-15, 72 h): se revisa que enlace con el informe y con `triaje-hallazgos-seguridad.md`; no se reescribe.
10. **Las filas de doc 07 §6 con vencimiento en esta tarea o en el cierre de la Fase 3 se dictaminan aquí**, una por una, y las que sean hallazgo se corrigen en esta tarea con su prueba (las catorce rutas de gestión sin zona de límite, A-8 TLS sin verificación, amplificación de asientos de exceso de plan, promesa de `obligaciones-legales.md`), o se renuevan con fecha y motivo. **Baja de cuentas de gestión** («antes de la primera versión comercial»): ver la decisión 16.
11. **Zona de límite de aplicación en toda ruta autenticada, y una prueba que enumera el router.** Son **dieciséis** rutas, no catorce (doc 07 §6 se corrige con la lista real): el grupo `employees:*` entero (13 rutas; solo `/employees/import` la tenía), `GET /reports/legal-export`, `GET /auth/me` y `POST /auth/logout`. Todas reciben `throttle:management` salvo `logout`, que queda **exenta en una lista cerrada con motivo** (solo revoca el token del que llama, no lleva `ability:` y acepta a propósito la sesión pendiente de 2FA: un techo ahí impide cerrar sesión y no frena ningún abuso); `/health` y `/ready` siguen fuera porque no llevan `auth:sanctum` y una sonda de vida no debe depender de Redis. La prueba `RouteRateLimitZonesTest` (Feature, como `SupportScopeRoutesTest`: Architecture no arranca el framework) recorre `Router::getRoutes()` y falla hoy en las dieciséis; la acompaña una de comportamiento en `ManagementRateLimitTest` (agotar el cupo con `GET /employees` y recibir `429` en `GET /reports/legal-export`) y otra que exige que cada zona usada tenga su `RateLimiter::for()`.
12. **`POST /scan/batch` gana su prueba de autorización negativa** (regla dura 18, sin excepciones): `ScanBatchAuthorizationTest` con el modelo exacto de `ScanAuthorizationTest` (sin token, cada rol de gestión, cuenta `kiosk` que no es dispositivo, quiosco sin `scan:write`, sesión de portal; la denegación no revela nada del lote). Y los tres datasets de `AuthorizationNegativeTest` que solo se probaban contra `rrhh` (emparejamiento, perfil de cumplimiento, ajustes) pasan por los seis actores; las seis pruebas del dataset grande ganan la etiqueta `RS-05` que la ficha exige.
13. **El asiento de auditoría del 403 se corrige en el documento, no en el código.** Doc 02 §9.4 y esta ficha decían «403 y su registro en auditoría»; el producto escribe `access.denied` solo cuando la denegación es **por alcance** (`ScopeGuard`), y `EmployeeWorkDaysAuthorizationTest` afirma a propósito que los 403 por rol o por ámbito de token no escriben: RS-05 registra divulgaciones, no intentos, y un asiento por cada 403 de rol sería una vía de amplificación bajo el candado de ADR-010. El §9.4 y la ficha pasan a decir «403, y asiento en `audit_log` cuando la denegación es por alcance».
14. **A-13 (RN-18) recibe la medición que prometía, informativa.** `scan-peak.js` gana el escenario `reject-out-of-order` (`RS-03 RN-18`) y `aggregate.js` publica la separación de medianas frente a las tres clases de credencial **sin veredicto bloqueante**: A-13 está aceptado sin suelo de tiempo y lo que faltaba era la cifra para revisar la aceptación en el cierre de fase, no una puerta. `ScanPeakScenariosTest` ya vigila que el escenario nuevo tenga requisito y `exec`.
15. **La contradicción §11 ↔ doc 03 y la cifra «catorce» de doc 07 se corrigen en el mismo commit**, como manda el orden de autoridad de `CLAUDE.md`: arreglar la contradicción forma parte de la tarea.
16. **Baja de cuentas de gestión y rotación de contraseña (H-03, H-13): por consola en esta tarea, la pantalla queda como decisión de producto.** `identity:deactivate-user` (con motivo, asiento `user.deactivated`, la sesión abierta deja de valer en la petición siguiente porque el callback de Sanctum ya consulta `is_active`) e `identity:reset-password` (asiento, contraseña de un solo uso mostrada una vez, como hace `identity:create-user`), más `needsRehash` en el acceso de gestión y en el del portal dentro de la transacción del acceso correcto. Es el mismo canal por el que hoy se crean cuentas y se restablece el 2FA, así que no abre superficie HTTP nueva ni exige contrato; la fila 17 de `endurecimiento.md` deja de enseñar `psql` y pasa a enseñar el comando. La pantalla de cuentas del panel (y su endpoint) es una decisión de producto que sigue en `HANDOFF.md` → «Decisiones de producto abiertas».
17. **SRI (H-06): se retira la promesa del doc 02 §7.1** con el motivo escrito en la misma tabla (CSP estricta sin `unsafe-inline`, mismo origen, sin CDN), que es lo que doc 07 §6 recomendaba y esta revisión suscribe. Decisión reversible: si algún día un asset sale del origen, SRI vuelve como requisito y no como promesa.
18. **Señales que faltaban (H-10):** alerta de saturación del borde sobre los `429` (la que doc 01 §8.1 decía que llegaba con la 3.2) y alerta de rechazos de firma QR (`scans_total{result="rejected_signature"}`, destinatario seguridad), las dos con runbook. **`T1550.001` no gana asiento**: un 403 por ámbito es un hecho que controla el atacante y cada asiento pasa por el candado global (ADR-037); la detección de ese vector queda en la métrica y en la prueba de autorización negativa, y doc 01 §8.1 lo dice así.
19. **El modelo de amenazas tenía once filas (H-12):** se corrige el «doce» de esta ficha. Las candidatas a fila nueva (manipulación del reloj del quiosco, repudio de la lectura de datos por un responsable) se anotan en `HANDOFF.md` como decisión del usuario, no se inventan. **Resuelto en los restos (22-09-2026):** el usuario decidió **añadir** la fila «Manipulación del reloj del quiosco» (`T1070.006`; mitigación RF-AT-09/RF-AT-10, anti-rebote y RN-16; detección por la incidencia `clock_skew`, sin alerta a propósito) y **no añadir** el repudio de la lectura de datos, que RS-05 ya cubre con `personal_data.accessed`. El modelo vuelve a tener doce filas, ahora de verdad: doc 01 §8.1, informe §4 (fila 12) y H-12 lo recogen. **Desde la 3.10 son trece**: su revisión de cumplimiento añadió la fila de divulgación del dato de salud del registro de ausencias (`T1530`), con su riesgo aceptado A-17 en doc 07 §6 por la retención sin plazo de `absences`.
20. **Lo que corrigieron las revisiones** se añade aquí al cerrar, con la prueba de cada hallazgo, en el §8 del informe.
21. **H-14, hallado por la evidencia de la CI y corregido en la misma tarea.** La CI posterior a la fusión de la 3.7 cayó en ⑧b U3 y la anterior había pasado con el mismo código: `update.sh` y el paso U3 de `ci.yml` preguntaban «¿la versión restaurada conoce `compliance:record-system-event`?» con una tubería `artisan list --raw | grep -q` que, bajo `pipefail`, responde «no» cuando es «sí» (EPIPE tras el acierto). Desde el cierre de la Fase 5 ninguna vuelta atrás escribía el asiento `system.restored_from_backup` (RF-PD-10, RS-07, regla dura 6) y la CI lo tapaba porque sus dos copias se equivocaban a la vez. Se corrige con `kq_app_knows_command` (`infra/scripts/lib/app-commands.sh`) en los dos sitios, cuatro pruebas en `UpdateScriptTest` y una guarda que prohibe reintroducir la tubería; el ciclo completo lo demuestra ⑧b en la CI de esta rama. La deuda hermana (`update.sh:1874`, `install.sh:1294`, `doctor.sh:266` con `2>/dev/null || true`) queda anotada, no tocada: está en el camino crítico de la actualización y merece tarea propia.
...
**Decisiones tomadas al ejecutar la tarea (22-09-2026).** Las que los documentos dejaban abiertas o que el estado real del repositorio obligó a fijar. Son la especificación común de los agentes que la ejecutan y quedan escritas también donde las lee quien las necesita (`openapi.yaml`, doc 01 §5.5, doc 05 §9, `configuracion.md`, `guia-rrhh.md`). El análisis previo de huecos encontró que **no existe nada de ausencias en ninguna capa**: `absences` figura en el doc 01 §5.5 con seis columnas, en el Anexo B como CRUD y en `DataExportCatalog::notInstalled()` como conjunto «no instalado»; el informe por periodo (2.8) ya cuenta `days_without_activity` con `generate_series` —«para un informe de absentismo, omitir los días sin actividad es un error»— y ese número es hoy, de hecho, el absentismo bruto; `holiday_calendar` del perfil se guarda y se audita desde la 5.2 sin que lo lea nadie (`ComplianceProfileField::hasNoConsumerYet()`, el texto «todavía no lo aplica ninguna regla» del panel y de `configuracion.md` y la promesa del doc 05 §9 de que «lo estrenará la gestión de ausencias»); la importación de plantilla (5.5) ya tiene el patrón de dos fases sin guardar el fichero (`EmployeeImportSource` + `SimpleExcelImportSource`, `mode: validate|apply` con `confirm_checksum`); el precedente de versión sin sobrescribir es `shift_entries` (`superseded`/`superseded_by_id`, ADR-026 y ADR-035) y el de «anular es un hecho con nombre y no un `DELETE`» es `POST /shift-entries/{uuid}/void`; y el cuadro «Negocio» lleva un panel de texto que dice que el absentismo «depende de la tarea 3.10».

1. **Alcance: registrar hechos, no aprobarlos.** Una ausencia registrada es un hecho, sin estado «pendiente» ni «aprobada» ni botón de aprobar (doc 05 §8: «sin flujo de aprobación», que es Fase 4). Solo días completos: `starts_on` y `ends_on` son fechas civiles inclusivas (doc 01 §5.5), sin medias jornadas. Se admite registrar hacia atrás —una baja se conoce después— y hacia delante —las vacaciones—. El portal del empleado **no** muestra ausencias en esta tarea (anotado en `HANDOFF.md` → «Decisiones de producto abiertas»).

2. **Módulo y modelo.** `Workforce` (doc 02 §1.6 le atribuye «contratos, ausencias»). Dominio puro: `Absence` (`uuid` público v7, `employeeUuid`, `type`, `startsOn`, `endsOn`, `note`, `status`, `version`, `supersedesUuid`), con `covers(day)` inclusivo en los dos extremos, `overlaps(other)`, `days()`, y `correctedWith(...)` que devuelve la versión siguiente; `AbsenceType` cerrado en inglés como `IncidentType`: `vacation`, `sick_leave`, `leave`, `other` (el catálogo comercial es «vacaciones, baja, permiso»; `other` existe para el permiso que no es ninguno y **exige `note`**); `AbsenceStatus`: `active`, `superseded`, `voided`. Los tipos **no son configurables**: un catálogo cerrado es lo que hace comparables los informes de dos clientes y no contradice ADR-017 (es como el catálogo de incidencias). Una ausencia que no toca ni un día de la relación laboral (termina antes de `hired_at` o empieza después de `terminated_at`) se rechaza con `422`; el solape parcial se admite y el informe solo cuenta los días de alta.

3. **Esquema `absences`, y las columnas que el doc 01 no enumeraba.** `id`, `uuid` (UNIQUE), `employee_id` (FK `RESTRICT`), `type` (`CHECK` del catálogo), `starts_on`, `ends_on` (`CHECK ends_on >= starts_on`), `note` (`TEXT` nulo, ≤ 500), más las que hacen cumplir la regla dura 5: `status` (`CHECK`), `version` (≥ 1), `supersedes_id` y `superseded_by_id` (FK a la propia tabla, nulas), `change_reason` (obligatoria a partir de la versión 2: `CHECK ((version = 1) = (change_reason IS NULL))`), `voided_at`, `voided_by_user_id`, `void_reason` (`CHECK ((status = 'voided') = (voided_at IS NOT NULL))`, ídem `superseded` ↔ `superseded_by_id`), `created_at`, `created_by_user_id` (nula en importación y semilla, como en contratos). **El solape lo impide PostgreSQL**: `absences_no_overlap EXCLUDE USING gist (employee_id WITH =, daterange(starts_on, ends_on, '[]') WITH &&) WHERE (status = 'active')`, la misma técnica que `employment_contracts_no_overlap`, y la comprobación en PHP existe solo para explicar el choque. **Al ejecutar, la exclusión quedó `DEFERRABLE INITIALLY IMMEDIATE`**: corregir inserta la versión nueva y cierra la anterior en la misma transacción, y ningún orden pasa sin diferir (la fila vieja tiene que apuntar a una que aún no existe, y si se inserta antes las dos están activas sobre los mismos días); PostgreSQL no admite `DEFERRABLE` en un `CHECK`, así que la única pieza que podía ceder era la exclusión. El alta normal sigue fallando en su propia sentencia y se traduce a `409`; `supersedeWith()` del repositorio hace `SET CONSTRAINTS … DEFERRED` → insertar → cerrar → `IMMEDIATE`, que valida ahí mismo. Índices `(employee_id, starts_on)` y parcial `(starts_on, ends_on) WHERE status = 'active'` para el informe. Migración `2026_09_22_100000_absences.php` (nombre corto por el precedente de contratos) con `down()` probado. Se anota en el doc 01 §5.5.

4. **Corregir es una versión nueva; anular es un hecho.** `PATCH` crea una fila `version + 1` con `uuid` nuevo y `supersedes_id`, y la anterior pasa a `superseded` con `superseded_by_id`: el único `UPDATE` sobre una fila anterior son esas dos columnas de estado, como en `shift_entries` (ADR-026, ADR-035); el resto —tipo, fechas, nota, autor, momento— no se toca nunca. `POST …/void` marca `voided` con autor, momento y motivo y **no** crea versión (no hay versión posterior de un hecho que no pasó). `change_reason` y `void_reason` son texto libre de 3 a 500 caracteres: no hay catálogo de motivos como en las correcciones del registro, porque no hay nueve causas tipificadas que defender ante la Inspección. Nunca `DELETE`, en ninguna capa.

5. **Contrato, en `/absences` de primer nivel** (Anexo B del doc 01) y **antes que el código**. `GET /api/v1/absences` con `from`/`to` (opcionales: por omisión el primer día del mes en curso en la zona del centro y un año menos un día después; máximo dos años), `employee_uuid`, `department_id`, `type`, `status` (`active` por omisión; `all` incluye supersedidas y anuladas), `Page`/`PerPage` como `/employees`; devuelve las ausencias cuyo intervalo toca el rango, **con el alcance dentro del `WHERE`** (RF-ID-03). `POST /api/v1/absences` (`employee_uuid`, `type`, `starts_on`, `ends_on`, `note`) → `201`; `409` si solapa con otra activa; `422` si el empleado no existe (va en el cuerpo, no en la ruta). `GET /api/v1/absences/{uuid}` → la ausencia con `history` (las versiones anteriores, de la más antigua a la más reciente). `PATCH /api/v1/absences/{uuid}` (`type`, `starts_on`, `ends_on`, `note` opcionales, `reason` obligatorio) → `200` con **la versión nueva** (`uuid` nuevo, `supersedes_uuid` el de la ruta); `409` si la de la ruta ya no es `active` o la nueva solapa. `POST /api/v1/absences/{uuid}/void` (`reason`) → `200`; `409` si no estaba activa. `POST /api/v1/absences/import` multipart con `file`, `mode: validate|apply` y `confirm_checksum`, calcado de `/employees/import`: columnas `employee_code`, `type`, `starts_on`, `ends_on`, `note` (alias en castellano `codigo`, `tipo`, `desde`, `hasta`, `nota` y el tipo también por su nombre en castellano: `vacaciones`, `baja`, `permiso`, `otro`), informe fila a fila, se aplican las válidas y se rechazan las demás (empleado desconocido, fechas incoherentes, solape con una activa o con otra fila del fichero); una fila idéntica a una ausencia activa ya registrada sale como `unchanged` y no como solape, para que reimportar sea seguro. Ámbitos: lectura con `employees:read` (policy `viewAny`: `admin`, `rrhh`, `responsable_departamento`), escritura con `employees:*` (policy: `admin`, `rrhh`); `throttle:management` en todas y la zona de `/employees/import` en la importación. `note` **solo la reciben `admin` y `rrhh`**: para el responsable el campo no viaja (así no se confunde con «sin nota»). El tipo sí lo ve el responsable: quien organiza el turno tiene que saber quién falta y por qué categoría, y la guía dice qué no escribir en la nota.

6. **Auditoría (regla dura 6) y dato de salud (regla dura 21).** Tres acciones nuevas en `AuditAction`: `absence.registered`, `absence.corrected`, `absence.voided`, escritas por un listener síncrono de `Compliance` (`RecordAbsenceChange`) a partir de tres eventos de `Workforce`; la importación escribe un `absence.registered` por ausencia aplicada con `source: import` y la huella del fichero, sin asiento resumen. El payload lleva `absence_uuid`, `employee_uuid`, `type`, `starts_on`, `ends_on`, `version`, `has_note` (booleano) y, al corregir, `supersedes_uuid`, el **antes** completo (`previous_type`, `previous_starts_on`, `previous_ends_on`) y `reason`; al anular, `reason`. **`note` no entra nunca en el asiento**: puede llevar un diagnóstico, y el trail no lo necesita para reconstruir el cambio. El tipo sí entra —sin él el asiento no describe el hecho— porque `audit_log` es el registro legal con acceso restringido, no un log técnico. En logs técnicos, mensajes de excepción y `error_events` no aparece ni el tipo ni la nota: las excepciones nombran el `uuid` y nada más, y una prueba de arquitectura comprueba que ningún fichero de ausencias de `Workforce` toca `Log::` ni `logger(`.

7. **Efecto en el informe por periodo (RF-GP-04), con la definición escrita una vez.** `PeriodReportRow` gana tres enteros obligatorios: `absence_days` (días-persona de alta cubiertos por una ausencia activa, tenga o no actividad), `holiday_days` (de alta, festivo del perfil y **no** cubierto por ausencia: si coinciden, gana la ausencia) y `unjustified_absence_days` (de alta, sin actividad, sin ausencia y sin festivo). «De alta» es entre `hired_at` y `terminated_at`, como `days_without_contract`. La definición canónica es `Reporting\Domain\Policy\AbsenteeismRule` (pura, unitaria, mutada) y el SQL la reproduce con `FILTER`; una prueba de integración compara las dos sobre el mismo caso, como el prorrateo de contratos. El calendario de festivos entra por `CompliancePolicyProvider` (puerto de `Shared`) resuelto en `GeneratePeriodReport` y pasado a la consulta como parámetro: el dominio no consulta configuración (regla dura 14). Con eso **`holiday_calendar` estrena consumidor**: `hasNoConsumerYet()` deja de ser cierto para nadie, el texto del panel y de `configuracion.md` pasan de «todavía no lo aplica ninguna regla» a «lo aplica el informe por periodo: los festivos no cuentan como absentismo», y `affectsIncidentDetection()` sigue siendo falso (ningún festivo abre ni cierra incidencias). `meta.criteria` gana tres líneas, en el idioma de la petición: que las ausencias registradas cuentan como justificadas, cuántos festivos del perfil caen en el periodo, y **el límite honesto**: «el producto no conoce el cuadrante; los días de descanso semanal cuentan como no justificados y hay que contrastarlos con el calendario de turnos». Las tres columnas salen también en CSV, XLSX y PDF. **No hay desglose por tipo en el informe**: una fila por departamento con «días de baja médica» es un agregado de dato de salud que nadie ha pedido; el detalle por tipo vive en la pantalla de ausencias, con su alcance.

8. **Instrumentación.** Un gauge `absences_current{type}`: personas de alta con una ausencia activa que cubre **hoy** (fecha civil del centro), por tipo; lo publica `Reporting` (`PublishAbsenceMetrics`, mismo patrón que `PublishComplianceMetrics`, misma cadencia programada) y el cuadro «Negocio» gana el panel «Personas ausentes hoy, por tipo» y reescribe su panel de texto: el absentismo ya tiene serie; trabajadas frente a contratadas (3.13) e impuntualidad (sin tipo de incidencia) siguen sin ella. Fila nueva en el doc 02 §8.2. Sin alerta: no hay runbook que la sostenga.

9. **Panel.** Funcionalidad propia `frontend-admin/src/features/absences/` —no dentro de `employees/`, porque la pantalla es transversal a la plantilla— con ruta `/absences` y entrada «Ausencias» en el menú tras «Empleados», visible con el ámbito de lectura de plantilla (el responsable la ve, acotada). Lista con filtros (periodo, departamento, tipo), registro con buscador de persona (código o nombre, con la API de plantilla que ya existe), **corrección que enseña «de qué valor a cuál» antes de confirmar** (doc 03 §4.3) y pide motivo, anulación con motivo, historial de versiones de una ausencia, y carga por fichero en dos pasos calcada de la del asistente. Ningún botón de aprobar. `PeriodReportTable` gana las tres columnas y el perfil de cumplimiento cambia el aviso del calendario. i18n ES/EN, AA verificada con axe en el recorrido nuevo, E2E `absences.spec.ts` (`@RF-GP-04`) con la API simulada de `support/admin.ts` **con estado**: tras registrar una baja de tres días, el informe del mismo periodo devuelve `absence_days: 3` y `unjustified_absence_days` tres menos, y los días anterior y posterior no cambian. Sin captura nueva en la guía en esta tarea (se regenera con las demás en la siguiente versión menor, resto de la 5.11).

10. **Documentación.** Doc 01 §5.5 (columnas nuevas de `absences` y su porqué, como en contratos); doc 05 §9 (el calendario de festivos ya lo aplica el informe); `configuracion.md` y `en/configuration.md` §2.4 (fila y párrafos del calendario; los cuatro tipos, que no se configuran, en una tabla corta que remite a la guía de RRHH); `guia-rrhh.md` y `en/hr-guide.md` con un apartado nuevo **«5 bis. Ausencias: vacaciones, bajas y permisos»** entre correcciones e informes, con los mismos subapartados en las dos lenguas (`ClientDocumentationTest` los cuenta): qué es y qué no (no hay aprobación), registrar, corregir y anular (antes → después, motivo, nada se borra), carga por fichero (columnas y alias), qué cambia en el informe (las tres columnas y el límite del cuadrante), quién ve qué (el responsable solo su departamento y sin nota; la baja médica es dato de salud: **no escribas el diagnóstico en la nota**) y qué queda anotado. `DataExportCatalog` gana el conjunto `absences` (con `note`: es la exportación de los datos del propio cliente) y `notInstalled()` queda vacío; los textos que decían `not_installed` se actualizan.

11. **Pruebas, además de las exigidas por la ficha.** `AuthorizationNegativeTest` con las seis rutas (auditor, token de portal y de quiosco en todas; responsable en las cuatro de escritura); `RouteRateLimitZonesTest` en verde con las zonas declaradas; contrato en `OpenApiContractTest`; `->group('RF-GP-04')` en todo, `RN-13` en la versión, `RS-05` en los asientos, `RF-ID-03` en el alcance; mutación sobre `Absence`, `AbsenceType` y `AbsenteeismRule` con MSI ≥ 80 %; unitarias de Vitest en los componentes nuevos del panel.

12. **Fuera de alcance, anotado en `HANDOFF.md`:** flujo de aprobación (Fase 4); ausencias por horas; tipos configurables; ausencias en el portal del empleado; aviso al responsable cuando se registra una ausencia de su gente; interacción con la detección de incidencias (un fichaje en día de ausencia no abre nada: es un hecho del registro, no una anomalía); cuadro de la 3.13.
